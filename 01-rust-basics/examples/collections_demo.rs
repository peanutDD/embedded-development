...
fn collection_techniques_demo() {
  ...
  // 查找及格的学生
  let passed: Vec<_> = students.iter().filter(|(_, score)| *score >= 80).collect();
  ...
  // 分区操作
  let (passed, failed): (Vec<(&str, i32)>, Vec<(&str, i32)>) =
    students.iter().partition(|(_, score)| *score >= 80);
