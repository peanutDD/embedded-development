...
fn calculator(expression: &str) -> Result<f64, MathError> {
    ...
    match operator {
        ...
        _ => Err(MathError::InvalidInput(format!("不支持的操作符: {}", operator))),
    }
}
...
mod tests {
    ...
    fn test_safe_sqrt() {
        ...
        assert!(matches!(safe_sqrt(-4.0), Err(MathError::NegativeSquareRoot)));
    }
    ...
    fn test_calculator() {
        ...
        assert!(matches!(calculator("10 / 0"), Err(MathError::DivisionByZero)));
        assert!(matches!(calculator("invalid"), Err(MathError::InvalidInput(_))));
    }
