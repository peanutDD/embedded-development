...
fn exercise_18(strings: Vec<String>) -> Vec<String> {
    // TODO: 在这里实现你的代码
    let mut result: Vec<String> = strings.into_iter()
        .filter(|s| s.len() > 3)
...
fn exercise_20(grades: Vec<(String, u32)>) -> Vec<String> {
    // TODO: 在这里实现你的代码
    let mut high_achievers: Vec<(String, u32)> = grades.into_iter()
        .filter(|(_, grade)| *grade >= 80)
        .collect();
    
    high_achievers.sort_by(|a, b| b.1.cmp(&a.1));
    
    high_achievers.into_iter()
        .map(|(name, _)| name)
        .collect()
}
...
mod challenge_tests {
    ...
    fn test_challenge_sort() {
        ...
        assert_eq!(challenge_sort(numbers.clone(), true), vec![1, 1, 2, 3, 4, 5, 6, 9]);
