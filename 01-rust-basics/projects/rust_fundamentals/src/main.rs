  ...
  fn handle_command(&self, input: &str) -> Result<bool> {
    ...
    match parts[0] {
      ...
      "create_user" => {
        ...
        let id = self
          .state
          .create_user(parts[1].to_string(), parts[2].to_string())?;
      ...
      "assign_task" => {
        ...
        let task_id: u64 = parts[1]
          .parse()
          .map_err(|_| AppError::Business("无效的任务ID".to_string()))?;
        let user_id: u64 = parts[2]
          .parse()
          .map_err(|_| AppError::Business("无效的用户ID".to_string()))?;
...
mod tests {
  ...
  fn test_task_creation_and_assignment() {
    ...
    let user_id = state
      ...
      .unwrap();
    let task_id = state
      .create_task("测试任务".to_string(), "这是一个测试任务".to_string())
      .unwrap();
  ...
  fn test_task_priority_filtering() {
    let state = AppState::new();
    let task1_id = state
      .create_task("普通任务".to_string(), "描述".to_string())
      .unwrap();
    let task2_id = state
      .create_task("高优先级任务".to_string(), "描述".to_string())
      .unwrap();

    state
      .update_task(task2_id, |task| {
        task.set_priority(Priority::High);
        Ok(())
      })
      .unwrap();
