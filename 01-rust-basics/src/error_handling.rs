...
impl ErrorAccumulator {
    ...
    pub fn add(&mut self, error: Error) -> Result<()> {
        self.errors.push(error).map_err(|_| Error::InsufficientResources)
    }
...
pub mod utils {
    ...
    pub fn safe_index<T>(slice: &[T], index: usize) -> Result<&T> {
        slice.get(index).ok_or(Error::InvalidArgument("Index out of bounds"))
    }
}
...
mod tests {
    ...
    fn test_error_context() {
        ...
        assert_eq!(format!("{}", context_error), "network operation: Operation timed out");
    }
