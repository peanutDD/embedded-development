...
fn main() -> ! {
  ...
  output!(
    "  优化级别: {}",
    toolchain_check::check_optimization_level()
  );
  output!("");
  ...
  output!(
    "  半主机支持: {}",
    if feature_check::check_semihosting() {
      "✓ 启用"
    } else {
      "✗ 禁用"
    }
  ...
  output!(
    "  STM32F4支持: {}",
    if feature_check::check_stm32f4() {
      "✓ 启用"
    } else {
      "✗ 禁用"
    }
