...
use rtt_target::{rtt_init_print, rprintln as rtt_rprintln};
...
fn test_memory_inspection() {
    ...
    // 定义一些测试数据
    let test_array: [u32; 8] = [0x12345678, 0x9ABCDEF0, 0xFEDCBA98, 0x76543210,
    ...
    debug_output!("测试结构体地址: 0x{:08X}", &test_struct as *const _ as usize);
    ...
    for region in &memory_regions {
        debug_output!("  {}: 0x{:08X} - 0x{:08X} ({} bytes)", 
                     region.name, region.start_addr, region.end_addr(), region.size);
    }
}
...
fn test_performance_analysis() {
    ...
    debug_output!("  平均每次调用: {} 周期", perf_counter.elapsed_cycles() / 100);
}
