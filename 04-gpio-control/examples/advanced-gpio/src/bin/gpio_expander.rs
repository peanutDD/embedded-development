...
use advanced_gpio::{DriveStrength, ExpanderType, GpioExpander, PinConfig, PowerState, SlewRate};
...
impl GpioExpanderSystem {
  ...
  fn update_statistics(&mut self) {
    // 计算活跃设备数
    self.system_state.active_devices = self
      .expander_manager
      .expanders
      .iter()
  ...
  pub fn initialize(&mut self) -> Result<(), SystemError> {
    ...
    for expander in &mut self.expanders {
      for pin in 0..expander.pin_count {
        ...
        if self
          .pin_mapping
          .insert(virtual_pin_id, virtual_pin)
          .is_err()
        {
          return Err(SystemError::ResourceExhausted);
        }
  ...
  fn calculate_performance_metrics(&mut self) {
    if !self.performance_monitor.response_times.is_empty() {
      ...
      self
        .performance_monitor
        .performance_metrics
        .average_response_time = sum / count;
