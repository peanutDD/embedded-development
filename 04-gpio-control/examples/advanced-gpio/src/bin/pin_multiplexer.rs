...
use advanced_gpio::{PinConfig, PinMultiplexer, SwitchType};
...
impl PinMultiplexerSystem {
  ...
  fn update_statistics(&mut self) {
    // 计算活跃设备数
    self.system_state.active_multiplexers = self
      .mux_manager
      .multiplexers
      .iter()
  ...
  pub fn initialize(&mut self) -> Result<(), SystemError> {
    ...
    for mux in &mut self.multiplexers {
      for channel in 0..mux.channel_count {
        ...
        if self
          .channel_mapping
          .insert(virtual_channel, channel_info)
          .is_err()
        {
          return Err(SystemError::ResourceExhausted);
        }
...
impl CrossbarManager {
  ...
  fn establish_connection(
    ...
    // 建立连接
    self.crossbar_matrix.connections[request.source_port as usize][request.target_port as usize] =
      true;
