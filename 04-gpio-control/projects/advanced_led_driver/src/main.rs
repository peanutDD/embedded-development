...
impl PwmController {
    ...
    pub fn fade_to(&mut self, target_percentage: f32, steps: u16, delay_ms: u32) {
        ...
        rprintln!("PWM渐变: {:.1}% -> {:.1}% ({}步)", current, target_percentage, steps);
...
impl RgbLed {
    ...
    /// 颜色渐变
    pub fn fade_to_rgb(&mut self, target_r: f32, target_g: f32, target_b: f32, steps: u16, delay_ms: u32) {
        ...
        rprintln!("RGB颜色渐变: ({:.2},{:.2},{:.2}) -> ({:.2},{:.2},{:.2})", 
                 current.red, current.green, current.blue, target_r, target_g, target_b);
...
impl EffectEngine {
    ...
    pub fn breathing_effect(&mut self, led: &mut RgbLed, color: PredefinedColor) {
        ...
        // 使用正弦波产生呼吸效果
        let phase = (self.time_counter as f32 * self.effect_speed * 0.01) % (2.0 * core::f32::consts::PI);
    ...
    pub fn blink_effect(&mut self, pwm: &mut PwmController, frequency: f32) {
        ...
        if self.time_counter % 100 == 0 {
            rprintln!("闪烁效果: {}Hz, 当前状态: {}", frequency, if brightness > 0.0 { "ON" } else { "OFF" });
        }
    }
...
impl PerformanceMonitor {
    ...
    pub fn report_performance(&mut self, current_time: u32) {
        if current_time - self.last_report_time >= 10000 { // 每10秒报告一次
            rprintln!("=== LED驱动性能报告 ===");
