...
impl TimingTester {
  ...
  /// 频率响应测试
  pub fn frequency_response_test(
    &self,
    frequencies: &[f32],
  ) -> heapless::Vec<FrequencyResponse, 16> {
    rprintln!("频率响应测试");
  ...
  pub fn clock_jitter_analysis(&self, clock_freq_mhz: f32) -> ClockJitterAnalysis {
    ...
    let total_rms_jitter = (thermal_jitter * thermal_jitter
      ...
      + supply_noise_jitter * supply_noise_jitter)
      .sqrt();
  ...
  pub fn timing_margin_analysis(&self, target_frequency_mhz: f32) -> TimingMarginAnalysis {
    ...
    let clock_jitter = self
      .clock_jitter_analysis(target_frequency_mhz)
      .rms_jitter_ps
      / 1000.0;
  ...
  fn calculate_phase_noise_jitter(&self, frequency_mhz: f32) -> f32 {
    ...
    let jitter_ps =
      10.0_f32.powf(phase_noise_dbc / 20.0) / (2.0 * core::f32::consts::PI * frequency_mhz) * 1e6;
    jitter_ps
  }
