...
impl ElectricalTester {
  ...
  pub fn measure_input_threshold(&self, rising_edge: bool) -> VoltageMeasurement {
    let threshold_type = if rising_edge {
      "上升沿"
    } else {
      "下降沿"
    };
  ...
  pub fn measure_input_impedance(&self) -> ImpedanceMeasurement {
    ...
    let impedance_magnitude =
      (base_resistance * base_resistance + capacitive_reactance * capacitive_reactance).sqrt();
  ...
  /// 温度特性测试
  pub fn temperature_characterization(
    &mut self,
    temp_range: (f32, f32),
  ...
  fn calculate_switching_power(&self, load_capacitance: f32) -> f32 {
    // P = C * V² * f
    load_capacitance * self.system_voltage * self.system_voltage * self.test_frequency * 1e6
    // 转换为微瓦
  }
