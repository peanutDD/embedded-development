...
use stm32f4xx_hal::{
    pac,
    prelude::*,
    gpio::{Input, Output, PushPull, PullUp, OpenDrain, Analog, Pin},
    timer::{Timer, Counter},
};
...
impl GpioAnalyzer {
    ...
    fn analyze_electrical_characteristics(&mut self) {
        ...
        // 输出电平分析
        rprintln!("输出电平特性:");
        rprintln!("  VOH(min): {:.2}V - 输出高电平最小值", self.electrical.voh_min);
        rprintln!("  VOL(max): {:.2}V - 输出低电平最大值", self.electrical.vol_max);
        rprintln!("  噪声容限(高): {:.2}V", self.electrical.voh_min - self.electrical.vih_min);
        rprintln!("  噪声容限(低): {:.2}V", self.electrical.vil_max - self.electrical.vol_max);
        
        // 输入电平分析
        rprintln!("输入电平特性:");
        rprintln!("  VIH(min): {:.2}V - 输入高电平最小值", self.electrical.vih_min);
        rprintln!("  VIL(max): {:.2}V - 输入低电平最大值", self.electrical.vil_max);
        rprintln!("  输入阈值范围: {:.2}V - {:.2}V", 
                 self.electrical.vil_max, self.electrical.vih_min);
        
        // 驱动能力分析
        rprintln!("驱动能力:");
        rprintln!("  IOH(max): {:.1}mA - 输出高电平最大电流", self.electrical.ioh_max);
        rprintln!("  IOL(max): {:.1}mA - 输出低电平最大电流", self.electrical.iol_max);
        ...
        self.test_results.insert("voh_min", self.electrical.voh_min).ok();
        self.test_results.insert("vol_max", self.electrical.vol_max).ok();
    }
    ...
    fn analyze_timing_characteristics(&mut self) {
        ...
        // 计算最大工作频率
        let min_period_ns = self.timing.setup_time_ns + self.timing.hold_time_ns + 
    ...
    fn analyze_power_characteristics(&mut self) {
        ...
        self.test_results.insert("static_power", self.power.static_current_ua * 3.3).ok();
    }
    ...
    fn perform_gpio_tests(&mut self) {
        ...
        self.test_results.insert("switch_cycles", switch_cycles as f32).ok();
        self.test_results.insert("read_cycles", read_cycles as f32).ok();
    }
