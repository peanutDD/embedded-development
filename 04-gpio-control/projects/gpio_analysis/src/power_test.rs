...
impl PowerTester {
    ...
    pub fn measure_static_power(&self, pin_state: bool) -> PowerMeasurement {
        rprintln!("测量静态功耗 (引脚状态: {})", if pin_state { "高" } else { "低" });
    ...
    /// 温度功耗特性分析
    pub fn temperature_power_profile(&mut self, temp_range: (f32, f32)) -> heapless::Vec<TemperaturePowerProfile, 16> {
        rprintln!("温度功耗特性分析 ({:.1}°C 到 {:.1}°C)", temp_range.0, temp_range.1);
    ...
    /// 功耗优化建议
    pub fn power_optimization_recommendations(&self, analysis: &PowerAnalysis) -> PowerOptimizationReport {
        rprintln!("生成功耗优化建议");
    ...
    /// 电池寿命估算
    pub fn battery_life_estimation(&self, battery_capacity_mah: f32, analysis: &PowerAnalysis) -> BatteryLifeEstimate {
    ...
    fn calculate_switching_power(&self, frequency_hz: f32, load: &LoadType) -> f32 {
        ...
        // P = C * V² * f
        capacitance * self.supply_voltage * self.supply_voltage * frequency_hz * 1e6 // μW
    }
