...
pub fn log_error(error: &HalError, context: &str) {
    match error {
        HalError::Hardware(hw_err) => {
            log::error!("[{}] 硬件错误: {} - {}", context, hw_err, error.recovery_suggestion());
        }
        HalError::Configuration(cfg_err) => {
            log::warn!("[{}] 配置错误: {} - {}", context, cfg_err, error.recovery_suggestion());
        }
        HalError::Resource(res_err) => {
            log::info!("[{}] 资源错误: {} - {}", context, res_err, error.recovery_suggestion());
        }
        HalError::Timing(tim_err) => {
            log::error!("[{}] 时序错误: {} - {}", context, tim_err, error.recovery_suggestion());
        }
    }
}
