...
fn main() -> ! {
  ...
  let mut matrix_controller = LedMatrixController::new(
    row_pins, col_pins, latch_pin, clock_pin, enable_pin, reset_pin,
  );
  ...
  // 创建按钮管理器
  let mut button_manager = ButtonManager::new(mode_button, brightness_button, speed_button);
  ...
  loop {
    ...
    if button_manager.is_brightness_pressed() {
      brightness = if brightness >= BRIGHTNESS_LEVELS {
        1
      } else {
        brightness + 1
      };
    ...
    if button_manager.is_speed_pressed() {
      animation_speed = if animation_speed >= 5 {
        1
      } else {
        animation_speed + 1
      };
