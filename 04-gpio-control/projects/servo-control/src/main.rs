...
use stm32f4xx_hal::{
    pac,
    prelude::*,
    timer::{Timer, Channel, PwmChannel},
    ...
    PwmChannel<T, C>: embedded_hal::pwm::SetDutyCycle,
{
    ...
    pub fn set_angle(&mut self, angle: f32) {
        ...
        let pulse_us = self.min_pulse_us + 
            ((clamped_angle / self.max_angle) * pulse_range as f32) as u16;
        ...
        defmt::info!("舵机角度设置为: {:.1}°, 脉宽: {}μs", clamped_angle, pulse_us);
    }
    ...
    pub fn smooth_move_to(&mut self, target_angle: f32, dt_ms: u32) {
        ...
        if fabsf(angle_diff) <= max_step {
            ...
            let step = if angle_diff > 0.0 { max_step } else { -max_step };
...
impl ServoSequence {
    ...
    pub fn add_step(&mut self, servo_id: u8, angle: f32, duration_ms: u32, delay_ms: u32) -> Result<(), ()> {
    ...
    pub fn update(&mut self, dt_ms: u32) -> f32 {
        ...
        match self.mode {
            ...
            SystemMode::Sweep => {
                ...
                let angle_offset = self.sweep_amplitude * sinf(2.0 * 3.14159 * self.sweep_frequency * self.sweep_time);
        ...
        if current_time - self.last_stats_time >= 5000 {
            if self.update_count > 0 {
                ...
                defmt::info!(
                    ...
                    avg_time, self.max_update_time, self.update_count
                );
...
fn main() -> ! {
    ...
    // 创建按钮管理器和系统状态
    let mut button_manager = ButtonManager::new(mode_button, angle_button, speed_button, sequence_button);
