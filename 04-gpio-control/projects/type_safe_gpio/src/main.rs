...
use typenum::{Unsigned, U0, U1, U2, U3, U4, U5};
...
/// 为有效引脚实现验证器
impl PinValidator<0> for Pin<0, Unconfigured> {
  const IS_VALID: bool = true;
}
impl PinValidator<1> for Pin<1, Unconfigured> {
  const IS_VALID: bool = true;
}
impl PinValidator<2> for Pin<2, Unconfigured> {
  const IS_VALID: bool = true;
}
impl PinValidator<3> for Pin<3, Unconfigured> {
  const IS_VALID: bool = true;
}
impl PinValidator<4> for Pin<4, Unconfigured> {
  const IS_VALID: bool = true;
}
impl PinValidator<5> for Pin<5, Unconfigured> {
  const IS_VALID: bool = true;
}
