...
fn show_stats() {
  unsafe {
    ...
    write!(
      stats_str,
      "  TX overflows: {}\r\n",
      TX_OVERFLOW_COUNT.load(Ordering::Relaxed)
    )
    .ok();
    write!(
      stats_str,
      "  RX overflows: {}\r\n",
      RX_OVERFLOW_COUNT.load(Ordering::Relaxed)
    )
    .ok();
...
fn show_buffer_status() {
  unsafe {
    if let (Some(ref tx_prod), Some(ref rx_prod)) = (TX_PRODUCER.as_ref(), RX_PRODUCER.as_ref()) {
