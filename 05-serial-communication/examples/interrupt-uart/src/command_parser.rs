...
fn cmd_status(_args: &[&str]) -> CommandResult {
    unsafe {
        ...
        write!(status, "  LED: {}\r\n", if SYSTEM_STATE.led_state { "ON" } else { "OFF" }).ok();
        write!(status, "  Counter: {}\r\n", SYSTEM_STATE.counter).ok();
        write!(status, "  Debug Mode: {}\r\n", if SYSTEM_STATE.debug_mode { "ON" } else { "OFF" }).ok();
        write!(status, "  Auto Response: {}\r\n", if SYSTEM_STATE.auto_response { "ON" } else { "OFF" }).ok();
...
fn cmd_get(args: &[&str]) -> CommandResult {
    ...
    unsafe {
        match args[1] {
            ...
            "auto" => {
                send_string("auto = ");
                send_string(if SYSTEM_STATE.auto_response { "on" } else { "off" });
                send_string("\r\n");
            }
            _ => {
                send_string("Error: Unknown parameter '");
...
fn handle_tab_completion(command_buffer: &mut Vec<u8, COMMAND_BUFFER_SIZE>, cursor_pos: &mut usize) {
    let cmd_str = core::str::from_utf8(command_buffer).unwrap_or("");
    let commands = ["help", "status", "set", "get", "echo", "led", "debug", "reset", "version", "clear", "auto"];
    
    let matches: Vec<&str, 16> = commands.iter()
