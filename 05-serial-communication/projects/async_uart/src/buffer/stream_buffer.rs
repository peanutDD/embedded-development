...
use super::{Buffer, BufferStats, OverflowStrategy, ReadableBuffer, WritableBuffer};
    ...
    match self.write(buf) {
      ...
      Err(BufferError::Full) => {
        ...
        match self.config.overflow_strategy {
          ...
          OverflowStrategy::Error => Poll::Ready(Err(BufferError::Full.into())),
        }
      }
      Err(e) => Poll::Ready(Err(e)),
    }
  ...
  fn read(&mut self, buf: &mut [u8]) -> Result<usize> {
    ...
    if read_pos < write_pos {
      ...
      if to_read <= first_part {
        ...
        let new_read_pos = if read_pos + to_read == self.size {
          0
        } else {
          read_pos + to_read
        };
...
impl WritableBuffer for StreamBuffer {
  fn write(&mut self, buf: &[u8]) -> Result<usize> {
    ...
    if write_pos < read_pos {
      ...
      self
        .write_pos
        .store(write_pos + copy_len, Ordering::Release);
      ...
      if to_write <= end_space {
        ...
        let new_write_pos = if write_pos + to_write == self.size {
          0
        } else {
          write_pos + to_write
        };
...
mod tests {
  ...
  fn test_stream_buffer_backpressure() {
    let config = StreamBufferConfig::new(100).watermarks(20, 80);
    let mut buffer = StreamBuffer::new(config).unwrap();
  ...
  fn test_stream_buffer_overflow_strategies() {
    // 测试丢弃最旧数据策略
    let config = StreamBufferConfig::new(10).overflow_strategy(OverflowStrategy::DropOldest);
    let mut buffer = StreamBuffer::new(config).unwrap();
