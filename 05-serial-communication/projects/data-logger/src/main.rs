...
use stm32f4xx_hal::{
    ...
    pac::{self, interrupt, ADC1, DMA1, DMA2, USART1, TIM2},
    prelude::*,
    rcc::RccExt,
...
fn main() -> ! {
    ...
    for i in 0..ADC_CHANNELS {
        adc.configure_channel(&adc_pins[i], stm32f4xx_hal::adc::config::Sequence::One, SampleTime::Cycles_3);
    }
...
impl PerformanceMetrics {
    ...
    pub fn calculate_rates(&mut self, elapsed_ms: u32) {
        if elapsed_ms > 0 {
            ...
            self.throughput_bps = (self.buffers_processed as f32 * BUFFER_SIZE as f32 * 2.0 * 8.0 * 1000.0) / elapsed_ms as f32;
        }
    }
}
