...
fn write_single_coil(frame: &[u8]) -> Option<Vec<u8, MAX_FRAME_SIZE>> {
    ...
    // 回显请求（成功响应）
    let mut response = Vec::new();
    for &byte in frame.iter().take(frame.len() - 2) { // 除了CRC
...
/// 创建异常响应
fn create_exception_response(slave_addr: u8, function_code: u8, exception_code: u8) -> Vec<u8, MAX_FRAME_SIZE> {
    let mut response = Vec::new();
