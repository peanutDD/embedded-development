    ...
    pub fn encode(&self, data: &[u8]) -> Result<Vec<u8, 512>, PhysicalLayerError> {
        ...
        match self.modulation {
            ModulationType::NRZ => {
                // NRZ编码：直接传输
                for &byte in data {
                    encoded.push(byte).map_err(|_| PhysicalLayerError::BufferOverflow)?;
                }
            },
            ModulationType::Manchester => {
                ...
                for &byte in data {
                    for bit_pos in 0..8 {
                        ...
                        if bit == 1 {
                            // 1 -> 01
                            encoded.push(0).map_err(|_| PhysicalLayerError::BufferOverflow)?;
                            encoded.push(1).map_err(|_| PhysicalLayerError::BufferOverflow)?;
                        } else {
                            // 0 -> 10
                            encoded.push(1).map_err(|_| PhysicalLayerError::BufferOverflow)?;
                            encoded.push(0).map_err(|_| PhysicalLayerError::BufferOverflow)?;
            ...
            ModulationType::DifferentialManchester => {
                ...
                for &byte in data {
                    for bit_pos in 0..8 {
                        ...
                        if bit == 1 {
                            // 1: 电平不变，然后跳变
                            encoded.push(last_level).map_err(|_| PhysicalLayerError::BufferOverflow)?;
            ...
            ModulationType::FourBFiveB => {
                ...
                for &byte in data {
                    ...
                    encoded.push(high_encoded).map_err(|_| PhysicalLayerError::BufferOverflow)?;
                    encoded.push(low_encoded).map_err(|_| PhysicalLayerError::BufferOverflow)?;
                }
            },
        }
        ...
        match self.modulation {
            ModulationType::NRZ => {
                ...
                for &byte in encoded_data {
                    decoded.push(byte).map_err(|_| PhysicalLayerError::BufferOverflow)?;
                }
            },
            ModulationType::Manchester => {
                // 曼彻斯特解码
...
impl DataLinkLayer {
    ...
    pub fn encapsulate(&mut self, data: &[u8], dest_addr: u8) -> Result<DataFrame, DataLinkError> {
        ...
        for &byte in data {
            payload.push(byte).map_err(|_| DataLinkError::BufferOverflow)?;
        }
    ...
    /// 计算错误检测码
    fn calculate_error_check(&self, header: &FrameHeader, payload: &[u8]) -> Result<u32, DataLinkError> {
...
impl NetworkLayer {
    ...
    /// 分片数据包
    pub fn fragment_packet(&mut self, packet: &NetworkPacket, mtu: u16) -> Result<Vec<NetworkPacket, 8>, NetworkError> {
        if packet.payload.len() <= mtu as usize {
            // 不需要分片
            let mut result = Vec::new();
            result.push(packet.clone()).map_err(|_| NetworkError::BufferOverflow)?;
            return Ok(result);
        }
        ...
        for i in 0..total_fragments {
            ...
            for j in offset..end {
                fragment_data.push(packet.payload[j]).map_err(|_| NetworkError::BufferOverflow)?;
            }
            ...
            fragments.push(fragment).map_err(|_| NetworkError::BufferOverflow)?;
        }
    ...
    /// 重组分片
    pub fn reassemble_fragments(&mut self, fragment: NetworkPacket) -> Result<Option<NetworkPacket>, NetworkError> {
        ...
        // 获取或创建分片缓存
        let fragment_buffer = self.fragmentation.fragment_cache
            .entry(fragment_id)
        ...
        fragment_buffer.fragments.push(fragment_info)
           ...
           fragment_buffer.received_length >= fragment_buffer.total_length {
            ...
            for fragment in &fragment_buffer.fragments {
                for &byte in &fragment.data {
                    reassembled_data.push(byte).map_err(|_| NetworkError::BufferOverflow)?;
                }
            }
...
impl TransportLayer {
    ...
    /// 建立连接
    pub fn establish_connection(&mut self, remote_address: NetworkAddress, remote_port: u16, local_port: u16) -> Result<ConnectionId, TransportError> {
    ...
    /// 发送数据
    pub fn send_data(&mut self, connection_id: ConnectionId, data: &[u8]) -> Result<TransportSegment, TransportError> {
        ...
        let mut payload = Vec::new();
        for &byte in data {
            payload.push(byte).map_err(|_| TransportError::BufferOverflow)?;
        }
        ...
        if self.reliability_config.enable_retransmission {
            connection.retransmit_queue.push(segment.clone())
    ...
    /// 接收数据
    pub fn receive_data(&mut self, segment: TransportSegment) -> Result<Option<Vec<u8, 256>>, TransportError> {
        let connection_id = ConnectionId {
    ...
    /// 关闭连接
    pub fn close_connection(&mut self, connection_id: ConnectionId) -> Result<TransportSegment, TransportError> {
        let connection = self.connections.get_mut(&connection_id)
            .ok_or(TransportError::ConnectionNotFound)?;
...
impl ApplicationLayer {
    ...
    fn handle_http(data: &[u8]) -> Result<Vec<u8, 256>, ApplicationError> {
        ...
        for &byte in http_response {
            response.push(byte).map_err(|_| ApplicationError::BufferOverflow)?;
        }
    ...
    /// 处理应用数据
    pub fn process_data(&mut self, session_id: SessionId, data: &[u8]) -> Result<Vec<u8, 256>, ApplicationError> {
        let session = self.sessions.get(&session_id)
            .ok_or(ApplicationError::SessionNotFound)?;

        let handler = self.protocol_handlers.get(&session.protocol)
...
impl ProtocolStack {
    ...
    /// 发送数据（从应用层到物理层）
    pub fn send(&mut self, data: &[u8], dest_addr: NetworkAddress, dest_port: u16, local_port: u16) -> Result<Vec<u8, 512>, StackError> {
        // 应用层处理
        let session_id = self.application.create_session(8080)
            .map_err(StackError::Application)?;
        
        // 传输层处理
        let connection_id = self.transport.establish_connection(dest_addr, dest_port, local_port)
            .map_err(StackError::Transport)?;
        let transport_segment = self.transport.send_data(connection_id, data)
            .map_err(StackError::Transport)?;
        ...
        for &byte in &header_bytes {
            network_payload.push(byte).map_err(|_| StackError::BufferOverflow)?;
        }
        ...
        for &byte in &network_packet.payload {
            datalink_payload.push(byte).map_err(|_| StackError::BufferOverflow)?;
        }
        
        let frame = self.data_link.encapsulate(&datalink_payload, dest_addr.0 as u8)
        ...
        for &byte in &frame.payload {
            physical_data.push(byte).map_err(|_| StackError::BufferOverflow)?;
        }

        let encoded_data = self.physical.encode(&physical_data)
    ...
    pub fn receive(&mut self, encoded_data: &[u8]) -> Result<Vec<u8, 256>, StackError> {
        // 物理层处理
        let decoded_data = self.physical.decode(encoded_data)
            .map_err(StackError::Physical)?;
        ...
        let datalink_payload = self.data_link.decapsulate(&dummy_frame)
            .map_err(StackError::DataLink)?;
