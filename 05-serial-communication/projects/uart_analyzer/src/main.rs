...
impl UartTimingAnalyzer {
  ...
  pub fn measure_baud_rate(&mut self, bit_transitions: &[u32]) -> f32 {
    ...
    self.baud_rate_accuracy.baud_rate_error = ((measured_baud_rate
      - self.baud_rate_accuracy.nominal_baud_rate as f32)
      / self.baud_rate_accuracy.nominal_baud_rate as f32)
      * 100.0;
  ...
  pub fn analyze_jitter(&mut self, edge_times: &[u32]) -> JitterReport {
    ...
    // 计算RMS抖动
    let mean_variation: f32 =
      period_variations.iter().sum::<f32>() / period_variations.len() as f32;
    let variance: f32 = period_variations
      .iter()
      .map(|&x| (x - mean_variation).powi(2))
      .sum::<f32>()
      / period_variations.len() as f32;
  ...
  fn calculate_jitter_spectrum(&self, variations: &[f32]) -> Vec<f32, 16> {
    ...
    for i in 0..16 {
      let freq_component = variations
        .iter()
        .enumerate()
        .map(|(idx, &val)| {
          val
            * (2.0 * core::f32::consts::PI * i as f32 * idx as f32 / variations.len() as f32).cos()
        })
        .sum::<f32>()
        / variations.len() as f32;
...
impl UartProtocolAnalyzer {
  ...
  fn verify_stop_bits(&self, data: &[u8]) -> bool {
    let stop_bit_start = 1
      + self.frame_format.data_bits as usize
      + if matches!(self.frame_format.parity, ParityType::None) {
        0
      } else {
        1
      };
  ...
  pub fn get_error_rate(&self) -> ErrorRateReport {
    ...
    ErrorRateReport {
      ...
      total_error_rate: ((self.error_statistics.framing_errors
        ...
        + self.error_statistics.overrun_errors) as f32
        / total as f32)
        * 100.0,
    }
  }
