...
use stm32f4xx_hal::{
  dma::{
    Channel4, MemoryToPeripheral, PeripheralToMemory, Stream0, Stream1, StreamsTuple, Transfer,
  },
  gpio::{gpioa::*, gpiob::*, Alternate, Output, PushPull},
  pac::{self, interrupt, TIM2, USART1, USART2},
  prelude::*,
  rcc::Clocks,
...
// DMA传输类型
type DmaTransferRx =
  Transfer<Stream0<pac::DMA2>, Channel4, SerialRx, PeripheralToMemory, &'static mut [u8]>;
type DmaTransferTx =
  Transfer<Stream1<pac::DMA2>, Channel4, SerialTx, MemoryToPeripheral, &'static mut [u8]>;
...
impl WirelessBridge {
  ...
  fn check_system_health(&mut self) -> SystemHealth {
    ...
    // 检查错误率
    let total_packets =
      self.statistics.serial_to_wireless_packets + self.statistics.wireless_to_serial_packets;
    if total_packets > 0 {
...
impl ProtocolHandler {
  ...
  fn process_outgoing(&mut self, packet: &Packet) -> Result<Vec<u8, BUFFER_SIZE>, ProtocolError> {
    ...
    output
      .extend_from_slice(&self.sequence_number.to_le_bytes())
      .ok();
    output
      .extend_from_slice(&self.session_id.to_le_bytes())
      .ok();
    output
      .extend_from_slice(&(packet.payload.len() as u16).to_le_bytes())
      .ok();
  ...
  fn create_heartbeat_packet(&mut self) -> Vec<u8, BUFFER_SIZE> {
    ...
    packet
      .extend_from_slice(&get_system_time().to_le_bytes())
      .ok();
  ...
  fn create_init_packet(&mut self) -> Vec<u8, BUFFER_SIZE> {
    ...
    packet
      ...
      .ok();
    packet
      .extend_from_slice(&PROTOCOL_VERSION.to_le_bytes())
      .ok();
