...
const TIMER_CONFIGS: [TimerConfig; 5] = [
  TimerConfig {
    frequency_hz: 1,
    description: "1Hz - 慢闪 (1秒周期)",
  },
  TimerConfig {
    frequency_hz: 2,
    description: "2Hz - 中等闪烁 (0.5秒周期)",
  },
  TimerConfig {
    frequency_hz: 5,
    description: "5Hz - 快闪 (0.2秒周期)",
  },
  TimerConfig {
    frequency_hz: 10,
    description: "10Hz - 高频闪烁 (0.1秒周期)",
  },
  TimerConfig {
    frequency_hz: 100,
    description: "100Hz - 超高频 (10ms周期)",
  },
];
...
fn main() -> ! {
  ...
  for (index, config) in TIMER_CONFIGS.iter().enumerate() {
    ...
    // 运行一段时间
    let run_duration_ms = if config.frequency_hz >= 10 {
      3000
    } else {
      5000
    };
  ...
  loop {
    ...
    if current_ticks >= last_report_time + 10 {
      // 2Hz * 5秒 = 10 ticks
...
fn run_timer_demo(duration_ms: u32) {
  ...
  loop {
    ...
    // 根据频率计算经过的毫秒数
    // 这里需要知道当前频率，简化处理
    if elapsed_ticks >= (duration_ms as u64 * 2 / 1000) {
      // 假设2Hz
      break;
    }
...
fn display_statistics(config: &TimerConfig) {
  ...
  if system_ticks > 0 {
    let actual_frequency =
      interrupt_count as f32 / (system_ticks as f32 / config.frequency_hz as f32);
    rprintln!(
      "      实际频率: {:.2}Hz (目标: {}Hz)",
      actual_frequency,
      config.frequency_hz
    ...
    let error_percent =
      ((actual_frequency - config.frequency_hz as f32) / config.frequency_hz as f32) * 100.0;
