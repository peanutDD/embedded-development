...
use stm32f4xx_hal::{
    ...
    timer::{Timer, Event},
    gpio::{gpiob::{PB0, PB1, PB2, PB3}, Output, PushPull},
};
...
static G_EVENT_PRODUCER: Mutex<RefCell<Option<Producer<TimerEvent, 32>>>> = Mutex::new(RefCell::new(None));

// 统计计数器
static G_COUNTERS: Mutex<RefCell<InterruptCounters>> = Mutex::new(RefCell::new(InterruptCounters::new()));
...
fn main() -> ! {
    ...
    loop {
        ...
        // 主循环状态LED闪烁（表示系统运行）
        if main_loop_counter - last_status_toggle >= 50 { // 500ms
            cortex_m::interrupt::free(|cs| {
