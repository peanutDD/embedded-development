...
use stm32f4xx_hal::{
    prelude::*,
    stm32,
    gpio::{gpioa::*, gpiob::*, gpioc::*, Input, PullUp, Output, PushPull, Edge},
    interrupt,
    syscfg::SysCfg,
};
...
static INTERRUPT_MANAGER: Mutex<RefCell<Option<ExternalInterruptManager>>> = Mutex::new(RefCell::new(None));
...
fn main() -> ! {
    ...
    cortex_m::interrupt::free(|cs| {
        ...
        INTERRUPT_MANAGER.borrow(cs).replace(Some(interrupt_manager));
    });
...
impl MultiButtonManager {
    ...
    pub fn get_press_count(&self, button_id: usize) -> u32 {
        if button_id < 8 {
            self.buttons[button_id].as_ref().map_or(0, |info| info.press_count)
        } else {
            0
        }
    }
