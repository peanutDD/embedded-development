...
use cortex_m::peripheral::{DCB, DWT, NVIC};
use cortex_m_rt::entry;
use panic_halt as _;
use stm32f4xx_hal::{
  dwt::{Dwt, DwtExt},
  gpio::{gpioa::*, gpiob::*, Alternate, Input, Output, PullUp, PushPull, AF1},
  interrupt,
  prelude::*,
  stm32,
  timer::{Event, Timer},
...
static LATENCY_TESTER: Mutex<RefCell<Option<InterruptLatencyTester>>> =
...
impl InterruptLatencyTester {
  ...
  fn convert_to_nanoseconds(&mut self) {
    let cycles_to_ns =
      |cycles: u32| -> u32 { ((cycles as u64 * 1_000_000_000) / self.cpu_frequency as u64) as u32 };
