...
// 全局变量
static PRIORITY_SCHEDULER: Mutex<RefCell<Option<PriorityScheduler>>> =
...
impl PriorityTask {
  /// 创建新的优先级任务
  pub fn new(id: u32, priority: Priority, period_ms: u32, handler: fn(), state: TaskState) -> Self {
    Self {
      id,
      priority,
  ...
  pub fn add_owned_mutex(&mut self, mutex_id: u32) -> Result<(), &'static str> {
    self
      .owned_mutexes
      .push(mutex_id)
      .map_err(|_| "Too many owned mutexes")
  }
...
impl PriorityMutex {
  ...
  pub fn add_waiting_task(&mut self, task_id: u32) -> Result<(), &'static str> {
    self
      .waiting_tasks
      .push(task_id)
      .map_err(|_| "Too many waiting tasks")
  }
...
impl PriorityScheduler {
  ...
  pub fn create_mutex(&mut self, mutex_id: u32) -> Result<u32, &'static str> {
    ...
    self
      .mutexes
      .insert(mutex_id, mutex)
      .map_err(|_| "Failed to create mutex")?;
    Ok(mutex_id)
  }
  ...
  pub fn get_statistics(&self) -> PrioritySchedulerStatistics {
    ...
    let blocked_tasks = self
      .tasks
      .iter()
      .filter(|t| t.state == TaskState::Blocked || t.state == TaskState::WaitingForMutex)
      .count() as u32;
