...
impl Ord for DeadlineTask {
  fn cmp(&self, other: &Self) -> core::cmp::Ordering {
    ...
    self
      .task
      .absolute_deadline
      .cmp(&other.task.absolute_deadline)
  }
}
...
impl RealTimeScheduler {
  ...
  pub fn get_statistics(&self) -> RealTimeSchedulerStatistics {
    let hard_rt_tasks = self
      .tasks
      .iter()
      .filter(|t| t.task_type == TaskType::HardRealTime)
      .count() as u32;
    let soft_rt_tasks = self
      .tasks
      .iter()
      .filter(|t| t.task_type == TaskType::SoftRealTime)
      .count() as u32;
    let non_rt_tasks = self
      .tasks
      .iter()
      .filter(|t| t.task_type == TaskType::NonRealTime)
      .count() as u32;
