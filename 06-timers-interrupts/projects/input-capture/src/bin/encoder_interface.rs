...
use libm::{atan2f, fabsf, fmaxf, fminf, sqrtf};
use panic_halt as _;
...
impl VelocityCalculator {
  ...
  fn update(&mut self, timestamp: u32, position: i32) -> f32 {
    ...
    if dt > 0.0 {
      ...
      // 低通滤波
      self.last_velocity =
        self.filter_alpha * raw_velocity + (1.0 - self.filter_alpha) * self.last_velocity;
...
impl EncoderInterfaceManager {
          ...
          EncoderMode::Quadrature => {
            ...
            if self.statistics.position_updates % 10 == 0 {
              rprintln!(
                "位置: {}, 角度: {:.1}°, 转数: {:.3}",
                new_position,
                self.decoder.get_angle_degrees(),
                self.decoder.get_revolutions()
              );
            }
          }
          EncoderMode::Velocity => {
            ...
            if self.statistics.position_updates % 20 == 0 {
              rprintln!(
                "速度: {:.2} 脉冲/秒, {:.1} RPM",
                velocity,
                self.velocity_calc.get_rpm(self.decoder.resolution)
              );
            }
          }
  ...
  fn process_index_pulse(&mut self, timestamp: u32) -> Result<(), &'static str> {
    ...
    rprintln!(
      ...
      self.statistics.total_revolutions
    );
...
fn main() -> ! {
  ...
  // 创建事件队列
  let (event_producer, mut event_consumer) = unsafe { ENCODER_EVENT_QUEUE.split() };
  ...
  loop {
    ...
    if encoder_a_state != last_encoder_a || encoder_b_state != last_encoder_b {
      if let Err(error) =
        encoder_manager.process_quadrature_signals(current_time, encoder_a_state, encoder_b_state)
      {
    ...
    while let Some(event) = event_consumer.dequeue() {
      match event {
        ...
        EncoderEvent::DirectionChange(direction) => match direction {
    ...
    if system_tick % 5000 == 0 {
      ...
      rprintln!(
        "总转数: {:.3}",
        encoder_manager.statistics.total_revolutions
      );
      ...
      rprintln!(
        "测量时间: {} ms",
        encoder_manager.statistics.measurement_time_ms
      );
      ...
      rprintln!(
        "  最大速度: {:.2} 脉冲/秒",
        encoder_manager.statistics.max_velocity
      );
      rprintln!(
        "  最小速度: {:.2} 脉冲/秒",
        encoder_manager.statistics.min_velocity
      );
      rprintln!(
        "  平均速度: {:.2} 脉冲/秒",
        encoder_manager.statistics.avg_velocity
      );
