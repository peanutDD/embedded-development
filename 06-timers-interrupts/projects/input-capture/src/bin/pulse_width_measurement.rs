...
impl PulseWidthMeter {
      ...
      EdgeType::Falling => {
        ...
        if let Some(start_time) = self.pulse_start_time {
          ...
          return Some(PulseWidthResult {
            ...
            frequency_hz: if self.last_period > 0.0 {
              1_000_000.0 / self.last_period
            } else {
              0.0
            },
  ...
  fn get_statistics(&self) -> PulseStatistics {
    ...
    if !self.period_buffer.is_empty() {
      ...
      stats.avg_frequency = if stats.avg_period > 0.0 {
        1_000_000.0 / stats.avg_period
      } else {
        0.0
      };
    }
...
impl PulseWidthManager {
  fn new(timer_frequency: u32, event_producer: Producer<'static, PulseWidthEvent, 64>) -> Self {
    Self {
      ...
      match self.current_mode {
        ...
        PulseMode::ContinuousPulse => {
          ...
          if self.statistics.total_pulses % 10 == 0 {
            rprintln!(
              "连续脉冲测量 - 脉宽: {:.2} us, 频率: {:.2} Hz",
              pulse_result.pulse_width_us,
              pulse_result.frequency_hz
            );
          }
        }
        PulseMode::DutyCycle => {
          rprintln!(
            "占空比测量 - 占空比: {:.1}%, 周期: {:.2} us",
            pulse_result.duty_cycle * 100.0,
            pulse_result.period_us
          );
        }
        ...
        PulseMode::Statistical => {
          ...
          if self.statistics.total_pulses % 50 == 0 {
            ...
            rprintln!(
              "统计分析 - 平均脉宽: {:.2} us, 抖动: {:.3} us",
              stats.avg_pulse_width,
              stats.pulse_width_jitter
            );
          }
        }
      }
...
fn main() -> ! {
  ...
  // 创建事件队列
  let (event_producer, mut event_consumer) = unsafe { PULSE_EVENT_QUEUE.split() };
  ...
  loop {
    ...
    while let Some(event) = event_consumer.dequeue() {
      match event {
        ...
        PulseWidthEvent::MeasurementComplete(result) => {
          rprintln!(
            "测量完成 - 脉宽: {:.2} us, 占空比: {:.1}%, 频率: {:.2} Hz",
            result.pulse_width_us,
    ...
    if stats.sample_count > 10 {
      ...
      if precision_ratio < 0.01 {
        // <1% 抖动
        precision_led.set_high();
      } else if precision_ratio < 0.05 {
        // <5% 抖动
    ...
    if system_tick % 5000 == 0 {
      ...
      rprintln!(
        "总测量次数: {}",
        pulse_manager.statistics.total_measurements
      );
      rprintln!(
        "成功测量: {}",
        pulse_manager.statistics.successful_measurements
      );
      rprintln!("失败测量: {}", pulse_manager.statistics.failed_measurements);
      rprintln!("总脉冲数: {}", pulse_manager.statistics.total_pulses);
      rprintln!(
        "测量时间: {} ms",
        pulse_manager.statistics.measurement_time_ms
      );
      ...
      if stats.sample_count > 0 {
        rprintln!(
          "统计信息 - 样本数: {}, 平均脉宽: {:.2} us, 抖动: {:.3} us",
          stats.sample_count,
          stats.avg_pulse_width,
          stats.pulse_width_jitter
        );
        rprintln!(
          "脉宽范围: {:.2} - {:.2} us, 平均频率: {:.2} Hz",
          stats.min_pulse_width,
          stats.max_pulse_width,
          stats.avg_frequency
        );
      }
    }
