...
use input_capture::{
  CaptureEvent, CaptureStatistics, CountMode, EdgeType, FrequencyMeter, MeasurementResult,
  PulseCounter, RotaryEncoder, RotationDirection, UltrasonicSensor,
};
...
fn main() -> ! {
  ...
  loop {
    ...
    // 检查按钮状态
    if current_time > button_debounce_time + 50000 {
      // 50ms防抖
...
fn process_pulse_event(timestamp: u64) {
  free(|cs| {
    if let Some(ref mut data) = SHARED_DATA.borrow(cs).borrow_mut().as_mut() {
      data
        .pulse_counter
        .add_pulse(timestamp as u32, EdgeType::Rising)
        .ok();
    }
  });
}
...
fn process_encoder_update(a_state: bool, b_state: bool, timestamp: u64) {
  free(|cs| {
    if let Some(ref mut data) = SHARED_DATA.borrow(cs).borrow_mut().as_mut() {
      data
        .rotary_encoder
        .update(a_state, b_state, timestamp as u32);
    }
  });
}
