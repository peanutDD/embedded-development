...
use stm32f4xx_hal::{
    ...
    stm32,
    timer::Timer,
    gpio::{gpioa::{PA0, PA1}, Input, PullUp},
    gpio::{gpiob::{PB0, PB1, PB2, PB3, PB4, PB5}, Output, PushPull, Input as GpioInput},
    gpio::gpioc::PC13,
};
...
fn display_encoder_status(
    ...
    match mode {
        ...
        DisplayMode::Velocity => {
            ...
            position_led.set_state(((counter / blink_rate) % 2 == 0).into()).ok();
        }
