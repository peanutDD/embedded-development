...
use input_capture::{FrequencyMeter, CaptureEvent, EdgeType, CaptureMode, MeasurementResult};
        ...
        let measured_frequency = match self.current_mode {
            ...
            FrequencyMode::HighPrecision => {
                ...
                if freq1 > 0.0 && freq2 > 0.0 {
                    ...
                    if phase_diff < freq1 * 0.01 { // 1%误差内
                        (freq1 + freq2) / 2.0
                    } else {
                        freq1 // 使用主通道
                    }
    ...
    fn update_statistics(&mut self, frequency: f32, dt_ms: u32) {
        ...
        if frequency > 0.0 {
            ...
            if self.statistics.avg_frequency == 0.0 {
                ...
                self.statistics.avg_frequency = self.statistics.avg_frequency * (1.0 - alpha) + frequency * alpha;
            }

            // 计算频率稳定性（标准差的简化估计）
            let deviation = fabsf(frequency - self.statistics.avg_frequency);
            self.statistics.frequency_stability = self.statistics.frequency_stability * 0.9 + deviation * 0.1;
        } else {
...
fn main() -> ! {
    ...
    loop {
        ...
        while let Some(event) = event_consumer.dequeue() {
            match event {
                ...
                FrequencyEvent::MeasurementComplete => {
                    ...
                    rprintln!("测量完成 - 频率: {:.2} Hz, 周期: {:.2} us", 
                             result.frequency_hz, result.period_us);
                }
        ...
        if system_tick % 5000 == 0 {
            ...
            rprintln!("成功测量: {}", freq_manager.statistics.successful_measurements);
            rprintln!("失败测量: {}", freq_manager.statistics.failed_measurements);
            rprintln!("频率范围: {:.2} - {:.2} Hz", 
            ...
            rprintln!("频率稳定性: {:.3} Hz", freq_manager.statistics.frequency_stability);
            rprintln!("测量时间: {} ms", freq_manager.statistics.measurement_time_ms);
            ...
            let result = freq_manager.get_measurement_result();
            rprintln!("当前测量 - 频率: {:.2} Hz, 周期: {:.2} us, 占空比: {:.1}%", 
                     result.frequency_hz, result.period_us, result.duty_cycle * 100.0);
        }
