...
use input_capture::{CaptureEvent, EdgeType, FrequencyMeter, MeasurementResult};
use nb::block;
...
fn main() -> ! {
  ...
  loop {
    ...
    // 检查按钮状态
    if system_time_ms > button_debounce_time + 50 {
      // 50ms防抖
    ...
    if state.measurement_active {
      ...
      if system_time_ms % 1 == 0 {
        ...
        if let Some(period_ticks) = input_capture.process_capture(capture_value) {
          // 将定时器周期转换为时间
          let period_us =
            (period_ticks as f64 * 1_000_000.0 / config.timer_frequency as f64) as u32;
