...
use stm32f4xx_hal::{
    ...
    gpio::{gpioa::{PA0, PA1}, Input, Floating},
    gpio::{gpiob::{PB0, PB1, PB2, PB3}, Output, PushPull, Input as GpioInput, PullUp},
    gpio::gpioc::PC13,
    interrupt,
};
    ...
    match mode {
        MeasurementMode::SinglePulse => {
            ...
            if measurement.pulse_width_us > 0.0 {
                let blink_rate = if measurement.pulse_width_us < 1000.0 { 10 } else { 20 };
                valid_led.set_state(((counter / blink_rate) % 2 == 0).into()).ok();
            }
        }
        ...
        MeasurementMode::Frequency => {
            ...
            if measurement.frequency_hz > 0.0 {
                let blink_rate = if measurement.frequency_hz > 100.0 { 5 } else { 15 };
                valid_led.set_state(((counter / blink_rate) % 2 == 0).into()).ok();
            }
        }
