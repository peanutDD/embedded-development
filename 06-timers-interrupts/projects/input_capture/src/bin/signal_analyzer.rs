...
use input_capture::{CaptureEvent, CaptureMode, EdgeType};
use libm::{atan2f, cosf, fabsf, fmaxf, fminf, logf, powf, sinf, sqrtf};
use panic_halt as _;
...
impl SignalQualityAnalyzer {
  ...
  fn calculate_snr(&mut self) -> f32 {
    ...
    // 计算信号功率（RMS）
    let signal_power: f32 = self.amplitude_buffer.iter().map(|&a| a * a).sum::<f32>()
      / self.amplitude_buffer.len() as f32;

    // 计算噪声功率（RMS）
    let noise_power: f32 =
      self.noise_buffer.iter().map(|&n| n * n).sum::<f32>() / self.noise_buffer.len() as f32;
  ...
  fn calculate_jitter(&mut self) -> f32 {
    ...
    // 计算频率标准差作为抖动指标
    let mean_freq: f32 =
      self.frequency_buffer.iter().sum::<f32>() / self.frequency_buffer.len() as f32;
    ...
    let variance: f32 = self
      ...
      .sum::<f32>()
      / (self.frequency_buffer.len() - 1) as f32;
  ...
  fn get_quality_score(&self) -> f32 {
    // 综合质量评分（0-100）
    let snr_score = if self.last_snr > 40.0 {
      100.0
    } else {
      self.last_snr * 2.5
    };
    let thd_score = if self.last_thd < 1.0 {
      100.0
    } else {
      100.0 / self.last_thd
    };
    let jitter_score = if self.last_jitter < 0.1 {
      100.0
    } else {
      10.0 / self.last_jitter
    };
...
impl SignalAnalyzerManager {
  fn new(sample_rate: f32, event_producer: Producer<'static, SignalEvent, 64>) -> Self {
    Self {
...
fn main() -> ! {
  ...
  // 创建事件队列
  let (event_producer, mut event_consumer) = unsafe { SIGNAL_EVENT_QUEUE.split() };
  ...
  loop {
    ...
    let simulated_voltage =
      2.5 + 1.0 * sinf(2.0 * core::f32::consts::PI * 1000.0 * (current_time as f32) / 10000.0);
    ...
    if current_time.wrapping_sub(button3_debounce) > 50 && button3_state && !last_button3_state {
      ...
      analyzer_manager.set_trigger(
        TriggerType::RisingEdge,
        trigger_levels[trigger_level_index as usize],
      );
    }
    ...
    if system_tick % 10000 == 0 {
      ...
      rprintln!(
        "频率检测: {}",
        analyzer_manager.statistics.frequency_detections
      );
      rprintln!(
        "谐波检测: {}",
        analyzer_manager.statistics.harmonic_detections
      );
      rprintln!("触发事件: {}", analyzer_manager.statistics.trigger_events);
      rprintln!(
        "质量测量: {}",
        analyzer_manager.statistics.quality_measurements
      );
      rprintln!(
        "处理时间: {} ms",
        analyzer_manager.statistics.processing_time_ms
      );
