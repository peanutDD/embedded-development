...
use cortex_m::peripheral::{CorePeripherals, DWT, NVIC};
...
use interrupt_manager::{
  ...
  InterruptManager, InterruptProfiler, Priority, ProfileSample,
};
...
fn main() -> ! {
  ...
  if let Some(manager) = get_global_interrupt_manager() {
    // 注册中断
    manager
      .register_interrupt(InterruptId::Timer2, Priority::High)
      .ok();
    manager
  ...
  loop {
    ...
    if PROFILING_ENABLED.load(Ordering::Relaxed) {
      profiling_led
        .set_state(((demo_counter / 20) % 2 == 0).into())
        .ok();
    } else {
