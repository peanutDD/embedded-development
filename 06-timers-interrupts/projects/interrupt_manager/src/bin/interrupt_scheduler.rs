...
use interrupt_manager::{
    InterruptManager, InterruptId, Priority,
...
fn display_scheduler_status(
    ...
    high_prio_led.set_state((high_count > 0 && (counter / 8) % 2 == 0).into()).ok();
    medium_prio_led.set_state((medium_count > 0 && (counter / 12) % 2 == 0).into()).ok();
    low_prio_led.set_state((low_count > 0 && (counter / 16) % 2 == 0).into()).ok();
...
impl InterruptScheduler {
    ...
    fn estimate_cpu_load(&self) -> u32 {
        let total_time = self.scheduler_stats.total_execution_time + 
                        self.scheduler_stats.total_scheduler_time;
...
/// 创建新任务
fn create_task(task_type: TaskType, priority: TaskPriority, interrupt_source: InterruptId) -> ScheduledTask {
...
fn TIM2() {
    interrupt_handler!(InterruptId::Timer2, {
        // 产生关键任务
        let task = create_task(TaskType::Critical, TaskPriority::Critical, InterruptId::Timer2);
...
fn TIM3() {
    interrupt_handler!(InterruptId::Timer3, {
        // 产生高优先级任务
        let task = create_task(TaskType::DataProcessing, TaskPriority::High, InterruptId::Timer3);
...
fn TIM4() {
    interrupt_handler!(InterruptId::Timer4, {
        // 产生中等优先级任务
        let task = create_task(TaskType::Communication, TaskPriority::Medium, InterruptId::Timer4);
