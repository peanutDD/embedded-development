...
fn display_nested_status(
    ...
    level1_led.set_state((count1 > 0 && (counter / 10) % 2 == 0).into()).ok();
    level2_led.set_state((count2 > 0 && (counter / 15) % 2 == 0).into()).ok();
    level3_led.set_state((count3 > 0 && (counter / 20) % 2 == 0).into()).ok();
    level4_led.set_state((count4 > 0 && (counter / 25) % 2 == 0).into()).ok();
...
impl NestedInterruptManager {
    ...
    fn handle_nested_event(&mut self, event: NestedEvent) {
        ...
        // 更新级别计数器
        let level_index = if event.nested_level > 4 { 4 } else { event.nested_level as usize };
        NESTED_COUNTERS[level_index].fetch_add(1, Ordering::Relaxed);
        
        self.nested_stats.avg_execution_time = 
            (self.nested_stats.avg_execution_time * (self.nested_stats.total_events - 1) as f32 + 
             event.execution_time_us as f32) / self.nested_stats.total_events as f32;
    }
