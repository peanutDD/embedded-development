...
use interrupt_manager::{
    InterruptManager, InterruptId, Priority, InterruptError,
...
fn main() -> ! {
    ...
    if let Some(manager) = get_global_interrupt_manager() {
        // 注册中断
        manager.register_interrupt(InterruptId::Timer2, Priority::Highest).ok();
...
fn display_priority_status(
    ...
    priority1_led.set_state(((count1 / 10) % 2 == 0).into()).ok();
    priority2_led.set_state(((count2 / 20) % 2 == 0).into()).ok();
    priority3_led.set_state(((count3 / 40) % 2 == 0).into()).ok();
    priority4_led.set_state(((count4 / 80) % 2 == 0).into()).ok();
    ...
    fn apply_demo_mode(&mut self, mode: u8) {
        ...
        if let Some(manager) = get_global_interrupt_manager() {
            match mode {
                0 => {
                    // 标准优先级模式
                    manager.set_priority(InterruptId::Timer2, Priority::Highest).ok();
                    manager.set_priority(InterruptId::Timer3, Priority::High).ok();
                    manager.set_priority(InterruptId::Timer4, Priority::Medium).ok();
                    manager.set_priority(InterruptId::Timer5, Priority::Low).ok();
                }
                1 => {
                    // 反向优先级模式
                    manager.set_priority(InterruptId::Timer2, Priority::Low).ok();
                    manager.set_priority(InterruptId::Timer3, Priority::Medium).ok();
                    manager.set_priority(InterruptId::Timer4, Priority::High).ok();
                    manager.set_priority(InterruptId::Timer5, Priority::Highest).ok();
                }
                2 => {
                    // 均等优先级模式
                    manager.set_priority(InterruptId::Timer2, Priority::Medium).ok();
                    manager.set_priority(InterruptId::Timer3, Priority::Medium).ok();
                    manager.set_priority(InterruptId::Timer4, Priority::Medium).ok();
                    manager.set_priority(InterruptId::Timer5, Priority::Medium).ok();
                }
                3 => {
                    // 动态优先级模式
                    ...
                    match cycle {
                        0 => {
                            manager.set_priority(InterruptId::Timer2, Priority::Highest).ok();
                            manager.set_priority(InterruptId::Timer3, Priority::Low).ok();
                        }
                        1 => {
                            manager.set_priority(InterruptId::Timer3, Priority::Highest).ok();
                            manager.set_priority(InterruptId::Timer4, Priority::Low).ok();
                        }
                        2 => {
                            manager.set_priority(InterruptId::Timer4, Priority::Highest).ok();
                            manager.set_priority(InterruptId::Timer5, Priority::Low).ok();
                        }
                        _ => {
                            manager.set_priority(InterruptId::Timer5, Priority::Highest).ok();
