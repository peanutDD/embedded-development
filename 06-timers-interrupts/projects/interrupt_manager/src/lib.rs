...
impl InterruptManager {
  ...
  pub fn enable_interrupt(&mut self, id: InterruptId) -> Result<(), InterruptError> {
    if let Some(info) = self.interrupts.get_mut(&id) {
      ...
      // 启用NVIC中断
      unsafe {
        cortex_m::peripheral::NVIC::unmask(cortex_m::peripheral::interrupt::InterruptNumber::from(
          id.irq_number(),
        ));
      }
  ...
  pub fn disable_interrupt(&mut self, id: InterruptId) -> Result<(), InterruptError> {
    if let Some(info) = self.interrupts.get_mut(&id) {
      ...
      // 禁用NVIC中断
      cortex_m::peripheral::NVIC::mask(cortex_m::peripheral::interrupt::InterruptNumber::from(
        id.irq_number(),
      ));
