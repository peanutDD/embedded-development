...
use cortex_m::{
    asm, interrupt,
    peripheral::{NVIC, DWT, SCB},
};
use stm32f4xx_hal::{
    pac::{self, TIM2, TIM3, TIM4, EXTI, USART1},
    prelude::*,
    timer::{Timer, Event},
    gpio::{Input, Output, PushPull, Pin, Edge},
    serial::{Serial, Event as SerialEvent},
    rcc::Clocks,
};
...
/// 全局中断管理器实例
static INTERRUPT_MANAGER: Mutex<RefCell<Option<InterruptManager>>> = 
...
impl InterruptManager {
    ...
    pub fn enter_interrupt(&mut self, interrupt_type: InterruptType) {
        ...
        if nesting_level > max_nesting {
            self.max_nesting_level.store(nesting_level, Ordering::SeqCst);
        }
    ...
    pub fn get_system_statistics(&self) -> SystemStatistics {
        ...
        SystemStatistics {
            ...
            interrupt_rate: self.total_interrupts.load(Ordering::SeqCst) as f32 / (elapsed_time as f32 / 1_000_000.0),
        }
    }
...
impl InterruptHandler for TimerInterruptHandler {
    fn handle(&mut self) -> Duration<u32, 1, 1_000_000> {
        ...
        self.last_execution_time.store(execution_time as u32, Ordering::SeqCst);
...
impl ExternalInterruptHandler {
    pub fn new(interrupt_type: InterruptType, priority: InterruptPriority, debounce_time: u32) -> Self {
        Self {
            interrupt_type,
            priority,
            debounce_time,
...
impl InterruptHandler for ExternalInterruptHandler {
    fn handle(&mut self) -> Duration<u32, 1, 1_000_000> {
        ...
        self.last_trigger_time.store(current_time as u32, Ordering::SeqCst);
...
impl InterruptProfiler {
    ...
    pub fn get_percentile(&mut self, percentile: f32) -> u32 {
        ...
        self.samples.sort();
        let index = ((self.samples.len() as f32 * percentile / 100.0) as usize)
    ...
    loop {
        ...
        critical_section::with(|cs| {
            if let Some(ref manager) = INTERRUPT_MANAGER.borrow(cs).borrow().as_ref() {
                ...
                rtt_target::rprintln!("运行时间: {:.2} 秒", system_stats.elapsed_time_us as f32 / 1_000_000.0);
