...
use stm32f4xx_hal::{
    prelude::*,
    stm32,
    timer::{Timer, Event, Channel, PwmChannel},
    gpio::{gpioa::{PA0, PA1, PA2, PA3}, Alternate, AF1},
...
use precision_timer::{
    FrequencyGenerator, FrequencyConfig, WaveformType,
...
fn main() -> ! {
    ...
    loop {
        ...
        // 生成器活动指示
        generator_led.set_state(GENERATOR_ACTIVE.load(Ordering::Relaxed).into()).ok();
...
fn display_generator_status(
    ...
    frequency_led.set_state(((counter / blink_rate) % 2 == 0).into()).ok();
    ...
    match mode {
        ...
        3 => waveform_led.set_state(((counter / 12) % 2 == 0).into()).ok(), // 自定义
    ...
    if stats.frequency_accuracy > 99.0 {
        ...
        accuracy_led.set_state(((counter / 15) % 2 == 0).into()).ok();
    } else {
    ...
    if stats.cpu_load > 80.0 {
        ...
        overload_led.set_state(((counter / 20) % 2 == 0).into()).ok();
    } else {
...
impl FrequencyGeneratorManager {
    ...
    fn handle_frequency_event(&mut self, event: FrequencyEvent) {
        ...
        if event.generator_id < 4 {
            ...
            if elapsed_time > 1000000 { // 1秒
                let actual_freq = (self.cycle_counters[event.generator_id as usize] as f32 * 1000000.0) / elapsed_time as f32;
    ...
    fn update_frequency_generation(&mut self) {
        ...
        if self.current_mode == 2 {
            ...
            if elapsed_seconds > 0 {
                ...
                let current_freq = base_freq + (elapsed_seconds as f32 * sweep_rate) % (max_freq - base_freq);
    ...
    fn update_generator_statistics(&mut self) {
        ...
        FREQUENCY_STATS[1].store((TARGET_FREQUENCY.load(Ordering::Relaxed) as i32 - ACTUAL_FREQUENCY.load(Ordering::Relaxed) as i32).abs() as u32, Ordering::Relaxed);
        FREQUENCY_STATS[2].store((100.0 - self.generator_stats.frequency_accuracy) as u32, Ordering::Relaxed);
        FREQUENCY_STATS[3].store(self.generator_stats.phase_sync_error as u32, Ordering::Relaxed);
    }
