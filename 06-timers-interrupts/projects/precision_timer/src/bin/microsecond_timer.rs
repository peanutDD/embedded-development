...
use precision_timer::{
    PrecisionTimer, TimerConfig, TimerPrecision, TimerMode,
    Timestamp, update_global_time, delay_us_precise, delay_ns_precise
};
...
fn main() -> ! {
    ...
    loop {
        ...
        // 定时器活动指示
        timer_led.set_state(TIMER_ENABLED.load(Ordering::Relaxed).into()).ok();
...
fn display_timer_status(
    ...
    microsecond_led.set_state((precision_ns >= 1000 && (counter / 10) % 2 == 0).into()).ok();
    nanosecond_led.set_state((precision_ns < 1000 && (counter / 8) % 2 == 0).into()).ok();
    
    // 精度指示
    if precision_ns <= 10 {
        precision_led.set_state(((counter / 5) % 2 == 0).into()).ok();
    } else if precision_ns <= 100 {
        precision_led.set_state(((counter / 8) % 2 == 0).into()).ok();
    } else {
        precision_led.set_state(((counter / 12) % 2 == 0).into()).ok();
    }
...
impl MicrosecondTimerManager {
    ...
    fn test_timing_consistency(&mut self) {
        ...
        let variance: u32 = measurements.iter()
            .map(|&x| {
