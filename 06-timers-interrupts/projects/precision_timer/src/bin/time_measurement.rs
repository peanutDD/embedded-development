...
use precision_timer::{
    TimeMeasurement, MeasurementType, MeasurementResult,
    Timestamp, update_global_time, get_global_time
};
...
fn main() -> ! {
    ...
    loop {
        ...
        // 测量活动指示
        measurement_led.set_state(MEASUREMENT_ACTIVE.load(Ordering::Relaxed).into()).ok();
...
fn display_measurement_status(
    ...
    match mode {
        ...
        1 => {
            ...
            interval_led.set_state(((counter / 10) % 2 == 0).into()).ok();
            frequency_led.set_low().ok();
        }
        2 => {
            // 频率测量模式
            duration_led.set_low().ok();
            interval_led.set_low().ok();
            frequency_led.set_state(((counter / 12) % 2 == 0).into()).ok();
        }
        3 => {
            // 综合测量模式
            duration_led.set_state(((counter / 6) % 2 == 0).into()).ok();
            interval_led.set_state(((counter / 8) % 2 == 0).into()).ok();
            frequency_led.set_state(((counter / 10) % 2 == 0).into()).ok();
        }
        _ => {}
    }
...
impl TimeMeasurementManager {
    ...
    fn start_duration_measurements(&mut self) {
        // 启动持续时间测量
        for i in 0..4 {
            if let Ok(id) = self.time_measurement.start_measurement(MeasurementType::Duration) {
    ...
    fn start_interval_measurements(&mut self) {
        // 启动间隔测量
        for i in 0..4 {
            if let Ok(id) = self.time_measurement.start_measurement(MeasurementType::Interval) {
                self.active_measurements[i] = Some(ActiveMeasurement {
                    id,
    ...
    fn start_frequency_measurements(&mut self) {
        // 启动频率测量
        for i in 0..4 {
            if let Ok(id) = self.time_measurement.start_measurement(MeasurementType::Frequency) {
                self.active_measurements[i] = Some(ActiveMeasurement {
                    id,
        ...
        for measurement in &mut self.active_measurements {
            if let Some(active) = measurement {
                ...
                if elapsed >= active.expected_duration {
                    ...
                    // 重新启动相同类型的测量
                    if let Ok(new_id) = self.time_measurement.start_measurement(active.measurement_type) {
                        active.id = new_id;
