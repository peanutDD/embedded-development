...
use panic_halt as _;
use precision_timer::{
  get_global_time, update_global_time, MeasurementResult, TimingAnalysis, TimingAnalyzer,
};
use stm32f4xx_hal::{
  gpio::{
...
fn main() -> ! {
  ...
  loop {
    ...
    // 分析活动指示
    analysis_led
      .set_state(ANALYSIS_ACTIVE.load(Ordering::Relaxed).into())
      .ok();
...
impl TimingAnalysisManager {
  ...
  fn handle_analysis_event(&mut self, event: AnalysisEvent) {
    ...
    let measurement = MeasurementResult {
      ...
      frequency_hz: if event.period_ns > 0 {
        1_000_000_000.0 / event.period_ns as f32
      } else {
        0.0
      },
  ...
  fn analyze_jitter(&mut self) {
    let sample_count = if self.buffer_full {
      self.measurement_buffer.len()
    } else {
      self.buffer_index
    };
    ...
    if jitter_count > 0 {
      // 计算平均抖动
      let total_jitter: u64 = jitter_values[..jitter_count]
        .iter()
        .map(|&x| x as u64)
        .sum();
      ...
      // 计算最大抖动
      let max_jitter = jitter_values[..jitter_count]
        .iter()
        .max()
        .copied()
        .unwrap_or(0);
  ...
  fn analyze_stability(&mut self) {
    ...
    if duration_count > 10 {
      // 计算平均值
      let mean: f64 = durations[..duration_count]
        .iter()
        .map(|&x| x as f64)
        .sum::<f64>()
        / duration_count as f64;

      // 计算方差
      let variance: f64 = durations[..duration_count]
        .iter()
        .map(|&x| {
  ...
  fn analyze_comprehensive(&mut self) {
    ...
    // 计算综合精度
    let jitter_score = if TIMING_STATS[0].load(Ordering::Relaxed) < 100 {
      100
    } else {
      50
    };
    let drift_score = if TIMING_STATS[2].load(Ordering::Relaxed) < 1000 {
      100
    } else {
      50
    };
  ...
  fn perform_comprehensive_analysis(&mut self) {
    if let Ok(analysis) = self
      .timing_analyzer
      .analyze_measurements(&self.measurement_buffer[..])
    {
      ...
      TIMING_STATS[4].store(
        (analysis.overall_accuracy * 100.0) as u32,
        Ordering::Relaxed,
      );
      TIMING_STATS[5].store(
        (analysis.performance_score * 100.0) as u32,
        Ordering::Relaxed,
      );
    }
  }
