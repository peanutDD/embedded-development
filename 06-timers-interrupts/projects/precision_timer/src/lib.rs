...
impl PrecisionTimer {
  ...
  pub fn start(&self) -> Result<(), PrecisionTimerError> {
    ...
    self
      .base_timestamp
      .store(self.get_system_time_ns(), Ordering::Relaxed);
...
impl TimeMeasurement {
  ...
  pub fn end_measurement(&mut self) -> Result<MeasurementResult, PrecisionTimerError> {
    let (measurement_type, start_time) = self
      .current_measurement
      .take()
...
impl FrequencyGenerator {
  ...
  pub fn get_output_value(&self) -> f32 {
    ...
    match self.config.waveform {
      ...
      WaveformType::Sine => (phase_rad.sin() + 1.0) / 2.0,
      ...
      WaveformType::Sawtooth => (phase % 360.0) / 360.0,
    }
  }
  ...
  pub fn update_phase(&self) {
    ...
    self
      .current_phase
      .store(new_phase as u32, Ordering::Relaxed);
  }
}
