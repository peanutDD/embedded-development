    ...
    TIM: pac::timer::Instance,
{
    ...
    pub fn new(timer: Timer<TIM>, clocks: &Clocks) -> Self {
        ...
        counter.start(Duration::<u32, 1, 1_000_000>::from_ticks(u32::MAX)).unwrap();
...
impl TimingStatistics {
    ...
    pub fn standard_deviation(&self) -> f32 {
        ...
        let variance: f32 = self.samples.iter()
            ...
            .sum::<f32>() / (self.samples.len() - 1) as f32;
...
impl TimerCalibrator {
    pub fn new(
        reference_timer: Timer<TIM5>,
        test_timer: Timer<TIM2>,
        clocks: &Clocks,
    ) -> Self {
        let mut ref_counter = reference_timer.counter_us(clocks);
        let mut test_counter = test_timer.counter_us(clocks);
        
        ref_counter.start(Duration::<u32, 1, 1_000_000>::from_ticks(u32::MAX)).unwrap();
        test_counter.start(Duration::<u32, 1, 1_000_000>::from_ticks(u32::MAX)).unwrap();
    ...
    pub fn print_report(&self) {
        for (name, stats) in &self.results {
            ...
            rtt_target::rprintln!(
                ...
                name, avg, std_dev, jitter, stats.count
            );
        }
    }
}
