  ...
  fn update_pattern(&mut self, dt_ms: u32) -> Result<(), &'static str> {
    ...
    match self.current_pattern {
      ...
      DimmingPattern::Pulsing => {
        // 脉冲效果
        let pulse = if sinf(self.pattern_phase * 4.0) > 0.0 {
          1.0
        } else {
          0.1
        };
      ...
      DimmingPattern::Strobe => {
        ...
        let brightness = if (self.pattern_phase * strobe_freq) % 1.0 < 0.1 {
          1.0
        } else {
          0.0
        };
...
fn main() -> ! {
  ...
  // 创建事件队列
  let (event_producer, mut event_consumer) = unsafe { DIMMING_EVENT_QUEUE.split() };
  ...
  loop {
    ...
    if system_tick % 5000 == 0 {
      ...
      rprintln!(
        "模式切换次数: {}",
        dimming_manager.statistics.pattern_changes
      );
      rprintln!(
        "亮度调整次数: {}",
        dimming_manager.statistics.brightness_changes
      );
      rprintln!(
        "伽马校正次数: {}",
        dimming_manager.statistics.gamma_corrections
      );
      rprintln!("最大亮度: {:.2}", dimming_manager.statistics.max_brightness);
      rprintln!(
        "平均亮度: {:.2}",
        dimming_manager.statistics.average_brightness
      );
      rprintln!(
        "估算功耗: {:.2}W",
        dimming_manager.statistics.power_consumption
      );
