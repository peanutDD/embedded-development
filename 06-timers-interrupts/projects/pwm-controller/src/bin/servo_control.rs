...
use stm32f4xx_hal::{
  ...
  prelude::*,
  rcc::Clocks,
  timer::{Channel1, Channel2, Channel3, Channel4, PwmHz, Timer},
};
...
fn main() -> ! {
  ...
  // 创建事件队列
  let (event_producer, mut event_consumer) = unsafe { SERVO_EVENT_QUEUE.split() };
  ...
  // 创建多舵机管理器
  let mut servo_manager = MultiServoManager::new(servo1, servo2, servo3, servo4, event_producer);
  ...
  loop {
    ...
    // 舵机活动指示
    let any_moving = (servo_manager.servo1.get_current_angle()
      - servo_manager.servo1.get_target_angle())
    .abs()
      > 1.0
      || (servo_manager.servo2.get_current_angle() - servo_manager.servo2.get_target_angle()).abs()
        > 1.0
      || (servo_manager.servo3.get_current_angle() - servo_manager.servo3.get_target_angle()).abs()
        > 1.0
      || (servo_manager.servo4.get_current_angle() - servo_manager.servo4.get_target_angle()).abs()
        > 1.0;
    ...
    if system_tick % 5000 == 0 {
      ...
      rprintln!(
        "总角度移动: {:.1}度",
        servo_manager.statistics.total_angle_moved
      );
