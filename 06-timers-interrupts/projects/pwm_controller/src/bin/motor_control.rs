  ...
  PWM2: PwmChannel,
{
  ...
  fn update_statistics(&mut self, dt_ms: u32) {
    ...
    // 更新平均速度（简化计算）
    self.statistics.average_speed = (self.statistics.average_speed * 0.99) + (current_speed * 0.01);
  }
...
fn main() -> ! {
  ...
  // 创建事件队列
  let (event_producer, mut event_consumer) = unsafe { MOTOR_EVENT_QUEUE.split() };
  ...
  loop {
    ...
    while let Some(event) = event_consumer.dequeue() {
      match event {
        ...
        MotorEvent::DirectionChange => {
          ...
          rprintln!(
            "方向改变，总次数: {}",
            motor_manager.statistics.direction_changes
          );
        }
        MotorEvent::Brake => {
    ...
    match motor_manager.control_mode {
      ...
      MotorControlMode::PID => {
        mode_led.set_high();
        // PID控制演示：阶跃响应
        let target_speed = if (demo_phase / 1000) % 2 == 0 {
          0.6
        } else {
          -0.6
        };
      ...
      MotorControlMode::Position => {
        ...
        let position = if (demo_phase / 1500) % 2 == 0 {
          0.5
        } else {
          -0.5
        };
    ...
    if system_tick % 5000 == 0 {
      ...
      rprintln!(
        "最大速度: {:.2}",
        motor_manager.statistics.max_speed_reached
      );
