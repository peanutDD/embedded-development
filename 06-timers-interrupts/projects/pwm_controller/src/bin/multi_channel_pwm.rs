...
use libm::{cosf, fabsf, fmodf, sinf};
use panic_halt as _;
use pwm_controller::{MultiChannelPwm, PwmChannel};
    ...
    for i in 0..8 {
      if let Some(ref mut channel) = self.channels[i] {
        ...
        let duty = match self.waveform_type {
          WaveformType::Sine => (sinf(phase) + 1.0) / 2.0,
          ...
          WaveformType::Sawtooth => fmodf(phase, 6.283) / 6.283,
          WaveformType::Square => {
            if sinf(phase) > 0.0 {
              1.0
            } else {
...
fn main() -> ! {
  ...
  // 创建事件队列
  let (event_producer, mut event_consumer) = unsafe { PWM_EVENT_QUEUE.split() };

  // 创建多通道PWM管理器
  let mut pwm_manager =
    MultiChannelPwmManager::new(ch1, ch2, ch3, ch4, ch5, ch6, ch7, ch8, event_producer);
