...
fn main() -> ! {
    ...
    loop {
        ...
        match demo_mode {
            ServoDemo::Sweep => {
                ...
                if tick_count % 2000 == 0 {
                    rprintln!("Sweep angles: {:.1}°, {:.1}°", sweep_angle, 180.0 - sweep_angle);
                }
            },
            ...
            ServoDemo::Position => {
                ...
                if position_timer >= 3000 { // 每3秒切换位置
            ...
            ServoDemo::Speed => {
                ...
                if mode_timer % 5000 == 0 && mode_timer > 0 {
                    ...
                    servo1.move_to_angle(speed_test_angle, speed_test_speed).unwrap();
                    servo2.move_to_angle(180.0 - speed_test_angle, speed_test_speed * 0.8).unwrap();
                    
                    rprintln!("Speed test: {:.0}°/s to {:.0}°", speed_test_speed, speed_test_angle);
                }
            },
        ...
        if tick_count % 10000 == 0 {
            rprintln!("System status - Tick: {}, Mode: {}", 
                tick_count, demo_mode.name());
            rprintln!("Servo1: current={:.1}°, target={:.1}°", 
                servo1.get_current_angle(), servo1.get_target_angle());
            rprintln!("Servo2: current={:.1}°, target={:.1}°", 
                servo2.get_current_angle(), servo2.get_target_angle());
        }
