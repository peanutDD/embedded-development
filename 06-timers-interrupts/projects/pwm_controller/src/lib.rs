    ...
    PWM: PwmChannel,
{
    ...
    /// 添加PWM通道
    pub fn add_channel(&mut self, index: usize, pwm: PWM, config: ChannelConfig) -> Result<(), &'static str> {
        if index >= N {
    ...
    pub fn set_duty_cycle(&mut self, channel: usize, duty: f32) -> Result<(), &'static str> {
        ...
        if let Some(ref mut pwm) = self.channels[channel] {
            ...
            pwm.set_duty_cycle(adjusted_duty).map_err(|_| "Failed to set duty cycle")?;
    ...
    PWM: PwmChannel,
{
    ...
    fn update_pwm(&mut self) -> Result<(), &'static str> {
        ...
        self.pwm.set_duty_cycle(duty_cycle).map_err(|_| "Failed to set PWM duty cycle")?;
        Ok(())
    }
    ...
    PWM2: PwmChannel,
{
    ...
    pub fn enable_brake(&mut self) -> Result<(), &'static str> {
        ...
        self.pwm_forward.set_duty_cycle(1.0).map_err(|_| "Failed to set forward PWM")?;
        self.pwm_backward.set_duty_cycle(1.0).map_err(|_| "Failed to set backward PWM")?;
        Ok(())
    }
    ...
    fn update_pwm(&mut self) -> Result<(), &'static str> {
        ...
        if self.current_speed >= 0.0 {
            // 正向旋转
            self.pwm_forward.set_duty_cycle(speed_abs).map_err(|_| "Failed to set forward PWM")?;
            self.pwm_backward.set_duty_cycle(0.0).map_err(|_| "Failed to set backward PWM")?;
        } else {
            // 反向旋转
            self.pwm_forward.set_duty_cycle(0.0).map_err(|_| "Failed to set forward PWM")?;
            self.pwm_backward.set_duty_cycle(speed_abs).map_err(|_| "Failed to set backward PWM")?;
        }
    ...
    PWM: PwmChannel,
{
    ...
    /// 设置波形参数
    pub fn set_waveform(&mut self, waveform_type: WaveformType, amplitude: f32, frequency: f32, offset: f32) {
    ...
    pub fn generate_sample(&mut self, time: f32) -> Result<f32, &'static str> {
        ...
        let sample = match self.waveform_type {
            ...
            WaveformType::Square => {
                if (normalized_time % 1.0) < 0.5 { 1.0 } else { -1.0 }
            },
        ...
        self.pwm.set_duty_cycle(duty_cycle.clamp(0.0, 1.0)).map_err(|_| "Failed to set PWM")?;
