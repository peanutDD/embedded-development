...
use panic_halt as _;
use servo_control::{ServoCalibrator, ServoConfig, ServoController};
use stm32f4xx_hal::{
  gpio::{
    gpioa::PA8,
    gpiob::{PB0, PB1, PB2},
    gpioc::{PC13, PC14, PC15},
    Alternate, Input, Output, PullUp, PushPull, AF1,
  },
  prelude::*,
  stm32,
  timer::{Channel, Timer},
    ...
    match calibration_mode {
      CalibrationMode::WaitingForStart => {
        ...
        if calibration_clicked {
          ...
          // 移动到第一个校准点
          calibrator
            .set_calibrated_angle(calibration_angles[calibration_index])
            .ok();
