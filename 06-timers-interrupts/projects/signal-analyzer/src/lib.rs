...
use libm::{atan2, cos, log10, sin, sqrt};
...
impl FrequencyCounter {
  ...
  fn detect_edge(&self, current: f32, previous: f32) -> Option<EdgeType> {
    match self.edge_type {
      EdgeType::Rising => {
        if previous < (self.threshold - self.hysteresis)
          && current > (self.threshold + self.hysteresis)
        {
      ...
      EdgeType::Both => {
        ...
        {
          Some(EdgeType::Rising)
        } else if previous > (self.threshold + self.hysteresis)
          && current < (self.threshold - self.hysteresis)
        {
          Some(EdgeType::Falling)
        } else {
        ...
        if sample < self.threshold_low {
          ...
          if let Some(measurement) = self.analyze_pulse(timestamp_us) {
            self
              .pulse_measurements
              .push(measurement)
              .map_err(|_| "Measurement buffer full")?;
          }
          PulseState::Low
  ...
  fn apply_window(&self, samples: &mut [f32]) {
    ...
    for (i, sample) in samples.iter_mut().enumerate() {
      let window_value = match self.window_type {
        ...
        WindowType::Hanning => {
          0.5 * (1.0 - cos(2.0 * core::f32::consts::PI * i as f32 / (n - 1) as f32))
        }
        WindowType::Hamming => {
          0.54 - 0.46 * cos(2.0 * core::f32::consts::PI * i as f32 / (n - 1) as f32)
        }
        WindowType::Blackman => {
          let a0 = 0.42;
          let a1 = 0.5;
...
impl SignalAnalyzer for WaveformAnalyzer {
  ...
  fn add_sample(&mut self, sample: f32, timestamp_us: u64) -> Result<(), Self::Error> {
    ...
    self
      .timestamps
      .push(timestamp_us)
      .map_err(|_| "Timestamp buffer full")?;
  ...
  fn analyze(&mut self) -> Result<SignalAnalysisResult, Self::Error> {
    ...
    // RMS值计算
    let rms_value =
      sqrt(self.samples.iter().map(|&x| x * x).sum::<f32>() / self.samples.len() as f32);
...
impl RealTimeMonitor {
  ...
  pub fn update(
    &mut self,
    current_time_us: u64,
  ) -> Result<Option<SignalAnalysisResult>, &'static str> {
