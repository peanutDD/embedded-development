...
use signal_analyzer::{
  EdgeType, FrequencyCounter, PulseWidthAnalyzer, RealTimeMonitor, SignalAnalysisResult,
  SignalAnalyzer, WaveformAnalyzer, WindowType,
};
...
impl SignalGenerator {
  ...
  fn generate_sample(&mut self) -> f32 {
    let sample = match self.waveform_type {
      WaveformType::Sine => libm::sinf(2.0 * core::f32::consts::PI * self.phase),
      WaveformType::Square => {
        if libm::sinf(2.0 * core::f32::consts::PI * self.phase) >= 0.0 {
          1.0
        } else {
          -1.0
        }
      ...
      WaveformType::Sawtooth => 2.0 * (self.phase - libm::floorf(self.phase)) - 1.0,
      WaveformType::Noise => {
        // 简单的伪随机噪声
...
fn main() -> ! {
  ...
  loop {
    ...
    // 检查按钮状态
    if timestamp_counter > button_debounce_time + 50000 {
      // 50ms防抖
    ...
    match system_state {
      ...
      SystemState::FrequencyMeasurement => {
        ...
        if let Ok(_) = frequency_counter.add_sample(calibrated_sample, timestamp_counter) {
          if timestamp_counter > last_analysis_time + 100000 {
            // 每100ms分析一次
      ...
      SystemState::PulseAnalysis => {
        ...
        if let Ok(_) = pulse_analyzer.add_sample(calibrated_sample, timestamp_counter) {
          if timestamp_counter > last_analysis_time + 200000 {
            // 每200ms分析一次
      ...
      SystemState::WaveformAnalysis => {
        ...
        if let Ok(_) = waveform_analyzer.add_sample(calibrated_sample, timestamp_counter) {
          if timestamp_counter > last_analysis_time + 500000 {
            // 每500ms分析一次
