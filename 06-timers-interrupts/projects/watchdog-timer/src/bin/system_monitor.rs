...
use nb::block;
use watchdog_timer::{
  IndependentWatchdog, RecoveryStrategy, ResetReason, SystemHealth, SystemMonitor, WatchdogConfig,
  WatchdogTimer,
};
...
impl SystemMonitorController {
  ...
  fn collect_system_metrics(&mut self, adc_values: &[u16; 4]) {
    ...
    let load_factor = if self.system_time_ms % 5000 < 1000 {
      30.0
    } else {
      0.0
    };
  ...
  fn check_alerts(&mut self) {
    ...
    if self.metrics.voltage < thresholds.voltage_low_critical {
      self.add_alert(
        ...
        thresholds.voltage_low_critical,
      );
      ...
      self.add_alert(
        ...
        "Voltage",
        self.metrics.voltage,
        thresholds.voltage_low_warning,
      );
    }
    ...
    if self.metrics.voltage > thresholds.voltage_high_critical {
      self.add_alert(
        ...
        "Voltage",
        self.metrics.voltage,
        thresholds.voltage_high_critical,
      );
      ...
      self.add_alert(
        ...
        "Voltage",
        self.metrics.voltage,
        thresholds.voltage_high_warning,
      );
    }
  }
...
fn main() -> ! {
  ...
  loop {
    ...
    monitor_controller
      .monitor
      .update_task(1, monitor_controller.system_time_ms)
      .ok();
    if monitor_controller.system_time_ms % 500 == 0 {
      monitor_controller
        .monitor
        .update_task(2, monitor_controller.system_time_ms)
        .ok();
    }
    if monitor_controller.system_time_ms % 2000 == 0 {
      monitor_controller
        .monitor
        .update_task(3, monitor_controller.system_time_ms)
        .ok();
    }
    if monitor_controller.system_time_ms % 3000 == 0 {
      monitor_controller
        .monitor
        .update_task(4, monitor_controller.system_time_ms)
        .ok();
    }

    // 检查任务超时
    let timed_out_tasks = monitor_controller
      .monitor
      .check_system(monitor_controller.system_time_ms);
...
fn update_monitoring_leds(
  ...
  // 严重告警LED：有严重告警时亮起
  let has_critical = controller
    .alerts
    .iter()
    .any(|a| a.level == AlertLevel::Critical || a.level == AlertLevel::Emergency);
  if has_critical {
