...
impl SystemMonitor {
    ...
    /// 添加任务监控
    pub fn add_task(&mut self, task_id: u8, name: &str, timeout_ms: u32, is_critical: bool) -> Result<(), ()> {
    ...
    /// 更新系统健康状态
    pub fn update_health(&mut self, cpu_usage: f32, memory_usage: f32, temperature: f32, voltage: f32) {
...
impl FaultRecoveryManager {
    ...
    /// 设置任务的恢复策略
    pub fn set_recovery_strategy(&mut self, task_id: u8, strategy: RecoveryStrategy) -> Result<(), ()> {
        // 检查是否已存在该任务的策略
        ...
        // 添加新的策略
        self.recovery_strategies.push((task_id, strategy)).map_err(|_| ())
    }
    ...
    pub fn mark_recovery_success(&mut self, task_id: u8) {
        if let Some(record) = self.fault_history.iter_mut().rev().find(|r| r.task_id == task_id) {
    ...
    pub fn get_fault_statistics(&self) -> (u32, u32, u32) {
        ...
        let successful_recoveries = self.fault_history.iter()
...
mod tests {
    ...
    fn test_fault_recovery_manager() {
        ...
        // 设置恢复策略
        manager.set_recovery_strategy(1, RecoveryStrategy::Restart).unwrap();
        manager.set_recovery_strategy(2, RecoveryStrategy::SafeMode).unwrap();
