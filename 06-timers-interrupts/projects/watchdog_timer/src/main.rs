...
use nb::block;
use watchdog_timer::{
  FaultRecoveryManager, IndependentWatchdog, RecoveryStrategy, ResetReason, SystemMonitor,
  WatchdogConfig, WatchdogTimer, WindowWatchdog,
};
...
impl SystemController {
  fn new() -> Self {
    ...
    monitor
      .add_task(COMM_TASK_ID, "comm_task", 2000, false)
      .ok();
    monitor
      .add_task(SENSOR_TASK_ID, "sensor_task", 500, true)
      .ok();
    monitor
      .add_task(ACTUATOR_TASK_ID, "actuator_task", 1500, false)
      .ok();

    // 设置恢复策略
    recovery_manager
      .set_recovery_strategy(MAIN_TASK_ID, RecoveryStrategy::Restart)
      .ok();
    recovery_manager
      .set_recovery_strategy(COMM_TASK_ID, RecoveryStrategy::SafeMode)
      .ok();
    recovery_manager
      .set_recovery_strategy(SENSOR_TASK_ID, RecoveryStrategy::Restart)
      .ok();
    recovery_manager
      .set_recovery_strategy(ACTUATOR_TASK_ID, RecoveryStrategy::Ignore)
      .ok();
  ...
  fn execute_sensor_task(&mut self) -> Result<(), ()> {
    ...
    if self.system_time_ms % 20 == 13 {
      ...
      self
        .monitor
        .update_health(cpu_usage, memory_usage, temperature, voltage);
      Ok(())
    }
  }
  ...
  fn check_system_health(&mut self) {
    ...
    for task in timed_out_tasks {
      ...
      let recovery_strategy = self
        .recovery_manager
        .handle_fault(task.task_id, self.system_time_ms);
  ...
  fn feed_watchdog(&mut self) -> Result<(), ()> {
    if self.system_time_ms > self.last_watchdog_feed + 1000 {
      // 每秒喂一次狗
      self.watchdog.feed()?;
