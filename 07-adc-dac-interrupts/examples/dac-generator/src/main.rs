...
use stm32f4xx_hal::{
    dac::{C1, C2, DacExt, DacOut, DacPin},
    gpio::{Analog, Pin},
    pac,
    prelude::*,
    timer::{TimerExt},
...
impl WaveformGenerator {
    ...
    fn next_sample(&mut self) -> f32 {
        ...
        let raw_value = match self.waveform_type {
            ...
            WaveformType::Square => {
                if sinf(phase_radians) >= 0.0 { 1.0 } else { -1.0 }
            },
    ...
    loop {
        ...
        if sample_count % 1000 == 0 {
            ...
            rprintln!(
                ...
                sample_count / 1000,
                dac1_value, v1_int, v1_frac,
                dac2_value, v2_int, v2_frac,
                delta1, delta2
            );
        ...
        if sample_count % 50000 == 0 {
            ...
            rprintln!("\n=== 波形切换 #{} ===", switch_count);
            rprintln!("  DAC1: {} ({:.1}Hz)", 
                waveform_name(gen1.waveform_type), gen1.frequency);
            rprintln!("  DAC2: {} ({:.1}Hz)", 
                waveform_name(gen2.waveform_type), gen2.frequency);
        ...
        if sample_count % 300000 == 0 {
            ...
            rprintln!("  运行时间: {} 分钟", sample_count / UPDATE_FREQUENCY_HZ / 60);
