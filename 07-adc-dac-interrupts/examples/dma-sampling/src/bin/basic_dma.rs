...
use stm32f4xx_hal::{
  ...
  dma::{DmaFlag, PeripheralToMemory, Stream0, StreamsTuple, Transfer},
  gpio::Analog,
...
use dma_sampling::{DmaPriority, DmaSamplingConfig, DmaSamplingManager, SamplingStats};

const BUFFER_SIZE: usize = 1024;

// 全局变量用于DMA中断处理
static DMA_MANAGER: Mutex<RefCell<Option<DmaSamplingManager>>> = Mutex::new(RefCell::new(None));
...
fn process_dma_buffer(
  ...
  if transfer_count % 10 == 0 {
    writeln!(
      serial,
      "传输#{:04}: 样本数={}, 均值={:.3}V, 最大={:.3}V, 最小={:.3}V",
      transfer_count,
      buffer.len(),
      stats.mean_voltage,
      stats.max_voltage,
...
/// 计算缓冲区统计信息
fn calculate_buffer_statistics(
  buffer: &Vec<u16, dma_sampling::MAX_BUFFER_SIZE>,
  vref: f32,
) -> BufferStatistics {
  if buffer.is_empty() {
  ...
  // 转换为电压值
  let voltages: Vec<f32, dma_sampling::MAX_BUFFER_SIZE> = buffer
    .iter()
...
fn detect_signal_anomalies(
  ...
  if stats.max_voltage > 3.2 {
    writeln!(
      serial,
      "警告: 电压过高 {:.3}V (传输#{})",
      stats.max_voltage, transfer_count
    )
    .ok();
  }
  ...
  if stats.min_voltage < 0.1 {
    writeln!(
      serial,
      "警告: 电压过低 {:.3}V (传输#{})",
      stats.min_voltage, transfer_count
    )
    .ok();
  }
  ...
  if stats.std_deviation > 0.5 {
    writeln!(
      serial,
      "警告: 信号不稳定，标准差={:.3}V (传输#{})",
      stats.std_deviation, transfer_count
    )
    .ok();
  }
  ...
  if stats.mean_voltage < 0.2 || stats.mean_voltage > 3.0 {
    writeln!(
      serial,
      "警告: 直流偏移异常 {:.3}V (传输#{})",
      stats.mean_voltage, transfer_count
    )
    .ok();
  }
}
...
fn perform_realtime_processing(buffer: &Vec<u16, dma_sampling::MAX_BUFFER_SIZE>) {
  ...
  if buffer.len() >= 8 {
    ...
    for i in 4..buffer.len() - 4 {
      let sum: u32 = (i - 4..=i + 4).map(|idx| buffer[idx] as u32).sum();
