...
fn main() -> ! {
    ...
    loop {
        ...
        if process_timer.wait().is_ok() {
            ...
            if samples_read > 0 {
                process_data_chunk(&mut serial, &processing_buffer[..samples_read], 
                                 process_count, vref);
            }
...
fn process_data_chunk(
    ...
    if process_count % 20 == 0 {
        writeln!(serial, "处理#{:04}: 样本数={}, 均值={:.3}V, 峰峰值={:.3}V, RMS={:.3}V",
                process_count, data.len(), stats.mean, stats.peak_to_peak, stats.rms).ok();
    }
...
fn calculate_chunk_statistics(data: &[u16], vref: f32) -> ChunkStatistics {
    ...
    // 转换为电压
    let voltages: Vec<f32, PROCESSING_CHUNK_SIZE> = data.iter()
...
fn perform_signal_analysis(
    ...
    if signal_quality < 0.5 {
        writeln!(serial, "警告: 信号质量低 {:.1}% (处理#{})", 
                signal_quality * 100.0, process_count).ok();
    }
    
    if estimated_freq > 1000.0 {
        writeln!(serial, "检测到高频信号: {:.1} Hz (处理#{})", 
                estimated_freq, process_count).ok();
    }
}
...
fn detect_signal_features(
    ...
    if process_count % 100 == 0 && (!peaks.is_empty() || !edges.is_empty()) {
        writeln!(serial, "特征检测 (处理#{}): 峰值={}, 边沿={}", 
                process_count, peaks.len(), edges.len()).ok();
    }
}
...
fn output_buffer_status(
    ...
    free(|cs| {
        ...
        writeln!(serial, "缓冲区状态 (处理#{}): 已用={}/{}, 利用率={:.1}%",
                process_count,
                buffer.used_space(),
                buffer.capacity(),
                (buffer.used_space() as f32 / buffer.capacity() as f32) * 100.0).ok();
