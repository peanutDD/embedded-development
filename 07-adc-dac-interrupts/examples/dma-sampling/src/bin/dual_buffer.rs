...
// 全局双缓冲区
static DUAL_BUFFER: Mutex<RefCell<DualBuffer>> = 
    Mutex::new(RefCell::new(DualBuffer::new()));
...
fn main() -> ! {
    ...
    loop {
        ...
        if status_timer.wait().is_ok() {
            output_system_status(&mut serial, sample_count, total_processed_samples, 
                               &performance_monitor);
        }
    }
}
...
fn process_buffer_data(
    ...
    if process_count % 10 == 0 {
        ...
        writeln!(serial, "RMS: {:.3}V, 标准差: {:.3}V", stats.rms, stats.std_dev).ok();
        writeln!(serial, "主频: {:.1}Hz, 功率: {:.2}dB", 
                spectrum_info.dominant_frequency, spectrum_info.power_db).ok();
        writeln!(serial, "").ok();
    }
    ...
    if quality_score < 0.6 {
        writeln!(serial, "警告: 信号质量低 {:.1}% (处理#{})", 
                quality_score * 100.0, process_count).ok();
    }
}
...
fn calculate_buffer_statistics(data: &Vec<u16, HALF_BUFFER_SIZE>, vref: f32) -> BufferStatistics {
    if data.is_empty() {
        return BufferStatistics {
            ...
            rms: 0.0, std_dev: 0.0, dc_component: 0.0, ac_component: 0.0,
        };
    }
    
    // 转换为电压
    let voltages: Vec<f32, HALF_BUFFER_SIZE> = data.iter()
    ...
    BufferStatistics {
        mean, max, min, peak_to_peak, rms, std_dev,
        dc_component, ac_component,
    }
}
...
/// 移动平均滤波器
fn apply_moving_average_filter(data: &Vec<u16, HALF_BUFFER_SIZE>, window: usize) -> Vec<u16, HALF_BUFFER_SIZE> {
...
/// 中值滤波器
fn apply_median_filter(data: &Vec<u16, HALF_BUFFER_SIZE>, window: usize) -> Vec<u16, HALF_BUFFER_SIZE> {
    let mut filtered = Vec::new();
...
fn perform_spectrum_analysis(data: &Vec<u16, HALF_BUFFER_SIZE>) -> SpectrumInfo {
    ...
    // 分析几个关键频率
    let test_frequencies = [100.0, 200.0, 500.0, 1000.0, 2000.0, 5000.0, 10000.0, 20000.0];
    ...
    // 找到主频
    let max_power_idx = power_spectrum.iter()
        .enumerate()
...
/// 计算特定频率的功率
fn calculate_power_at_frequency(data: &Vec<u16, HALF_BUFFER_SIZE>, frequency: f32, sample_rate: f32) -> f32 {
    if data.is_empty() {
        return 0.0;
    }
...
fn detect_signal_anomalies(
    ...
    if stats.peak_to_peak > 3.0 {
        writeln!(serial, "异常: 信号幅度过大 {:.3}V (处理#{})", 
                stats.peak_to_peak, process_count).ok();
    }
    
    if stats.peak_to_peak < 0.1 {
        writeln!(serial, "异常: 信号幅度过小 {:.3}V (处理#{})", 
                stats.peak_to_peak, process_count).ok();
    }
    
    // 频率异常检测
    if spectrum.dominant_frequency > 15000.0 {
        writeln!(serial, "异常: 检测到高频信号 {:.1}Hz (处理#{})", 
                spectrum.dominant_frequency, process_count).ok();
    }
    
    // DC偏移异常
    if stats.dc_component > 2.8 || stats.dc_component < 0.5 {
        writeln!(serial, "异常: DC偏移异常 {:.3}V (处理#{})", 
                stats.dc_component, process_count).ok();
    }
    
    // 噪声水平异常
    if stats.std_dev > 0.5 {
        writeln!(serial, "异常: 噪声水平过高 {:.3}V (处理#{})", 
                stats.std_dev, process_count).ok();
    }
}
...
fn output_system_status(
    ...
    writeln!(serial, "处理效率: {:.1}%", monitor.get_processing_efficiency() * 100.0).ok();
    writeln!(serial, "溢出次数: {}", monitor.get_overrun_count()).ok();
    writeln!(serial, "处理延迟: {:.2}ms", monitor.get_average_processing_delay()).ok();
    writeln!(serial, "").ok();
}
...
fn simulate_adc_reading(sample_count: u32) -> u16 {
    ...
    let harmonic_signal = harmonic_amplitude * (2.0 * core::f32::consts::PI * harmonic_freq * t).sin();
