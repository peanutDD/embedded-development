        ...
        if process_timer.wait().is_ok() {
            ...
            for channel in 0..CHANNEL_COUNT {
                ...
                if let Some(data) = channel_data {
                    process_channel_data(&mut serial, channel, &data, process_count, vref,
                                       &mut channel_monitors[channel]);
                }
            }
            
            // 执行跨通道分析
            perform_cross_channel_analysis(&mut serial, &channel_monitors, 
                                         &mut cross_channel_analyzer, process_count);
        }
        
        // 输出系统状态
        if status_timer.wait().is_ok() {
            output_multi_channel_status(&mut serial, sample_count, process_count,
                                      &channel_monitors, &cross_channel_analyzer);
        }
    }
}
...
fn process_channel_data(
    ...
    if process_count % 50 == 0 {
        writeln!(serial, "CH{}: 均值={:.3}V, RMS={:.3}V, 峰峰值={:.3}V",
                channel, stats.mean, stats.rms, stats.peak_to_peak).ok();
    }
}
...
fn calculate_channel_statistics(data: &[u16; SAMPLES_PER_CHANNEL], vref: f32) -> ChannelStatistics {
    ...
    ChannelStatistics {
        mean, max, min, rms, std_dev, peak_to_peak,
        dc_offset, ac_component, snr,
    }
}

/// 应用通道特定处理
fn apply_channel_processing(data: &[u16; SAMPLES_PER_CHANNEL], channel: usize) -> [u16; SAMPLES_PER_CHANNEL] {
...
fn detect_channel_anomalies(
    ...
    if stats.peak_to_peak > 3.0 {
        writeln!(serial, "CH{} 异常: 幅度过大 {:.3}V (处理#{})", 
                channel, stats.peak_to_peak, process_count).ok();
    }
    
    if stats.peak_to_peak < 0.05 {
        writeln!(serial, "CH{} 异常: 幅度过小 {:.3}V (处理#{})", 
                channel, stats.peak_to_peak, process_count).ok();
    }
    
    // DC偏移异常
    if stats.dc_offset > 2.8 || stats.dc_offset < 0.5 {
        writeln!(serial, "CH{} 异常: DC偏移 {:.3}V (处理#{})", 
                channel, stats.dc_offset, process_count).ok();
    }
    
    // 信噪比异常
    if stats.snr < 20.0 {
        writeln!(serial, "CH{} 异常: SNR过低 {:.1}dB (处理#{})", 
                channel, stats.snr, process_count).ok();
    }
}
    ...
    fn update_analysis(&mut self, monitors: &[ChannelPerformanceMonitor; CHANNEL_COUNT]) {
        ...
        for i in 0..CHANNEL_COUNT {
            for j in 0..CHANNEL_COUNT {
                if i != j {
                    self.correlation_matrix[i][j] = calculate_channel_correlation(
        ...
        for i in 1..CHANNEL_COUNT {
            self.phase_differences[i] = calculate_phase_difference(
                &monitors[reference_channel], &monitors[i]
            );
            self.amplitude_ratios[i] = calculate_amplitude_ratio(
                &monitors[reference_channel], &monitors[i]
            );
            self.coherence_values[i] = calculate_coherence(
                &monitors[reference_channel], &monitors[i]
            );
        }
    }
...
fn perform_cross_channel_analysis(
    ...
    if process_count % 100 == 0 {
        ...
        for i in 1..CHANNEL_COUNT {
            writeln!(serial, "CH0-CH{}: 相位差={:.1}°, 幅度比={:.2}, 相干性={:.3}",
                    i, analyzer.phase_differences[i], 
                    analyzer.amplitude_ratios[i], analyzer.coherence_values[i]).ok();
        }
        writeln!(serial, "").ok();
    }
...
fn detect_cross_channel_anomalies(
    ...
    if max_correlation > 0.9 {
        writeln!(serial, "警告: 检测到通道间串扰 {:.3} (处理#{})", 
                max_correlation, process_count).ok();
    }
    
    // 检测异常的幅度比
    for i in 1..CHANNEL_COUNT {
        if analyzer.amplitude_ratios[i] > 5.0 || analyzer.amplitude_ratios[i] < 0.2 {
            writeln!(serial, "警告: CH0-CH{} 幅度比异常 {:.2} (处理#{})", 
                    i, analyzer.amplitude_ratios[i], process_count).ok();
        }
    }
    ...
    for i in 1..CHANNEL_COUNT {
        if analyzer.coherence_values[i] < 0.3 {
            writeln!(serial, "警告: CH0-CH{} 相干性低 {:.3} (处理#{})", 
                    i, analyzer.coherence_values[i], process_count).ok();
        }
    }
}
...
fn output_multi_channel_status(
    ...
    for (i, monitor) in monitors.iter().enumerate() {
        writeln!(serial, "CH{}: 成功率={:.1}%, 平均SNR={:.1}dB, 溢出={}",
                i, monitor.get_success_rate() * 100.0, 
                monitor.avg_snr, monitor.overflow_count).ok();
    }
    
    writeln!(serial, "最大通道相关性: {:.3}", analyzer.get_max_correlation()).ok();
    writeln!(serial, "").ok();
}
...
fn simulate_multi_channel_adc(channel: usize, sample_count: u32) -> u16 {
    ...
    let signal = match channel {
        ...
        2 => {
            ...
            if phase.sin() > 0.0 { amplitude } else { -amplitude }
        }
        3 => {
            // 通道3：1.5kHz三角波
            let amplitude = 300.0;
