...
use stm32f4xx_hal::{
    ...
    dma::{Stream, StreamsTuple, Transfer, PeripheralToMemory, MemoryToPeripheral, DmaFlag},
    gpio::{Analog, Pin},
    pac::{ADC1, DMA2},
    prelude::*,
};
...
impl DmaSamplingManager {
    ...
    pub fn get_current_buffer(&self) -> &Vec<u16, MAX_BUFFER_SIZE> {
        match self.current_buffer {
            ...
            BufferIndex::Secondary => {
                self.secondary_buffer.as_ref().unwrap_or(&self.primary_buffer)
            }
        }
    }
    ...
    pub fn get_current_buffer_mut(&mut self) -> &mut Vec<u16, MAX_BUFFER_SIZE> {
        match self.current_buffer {
            ...
            BufferIndex::Secondary => {
                self.secondary_buffer.as_mut().unwrap_or(&mut self.primary_buffer)
            }
        }
    }
...
impl DmaTransferMonitor {
    ...
    fn update_performance_metrics(&mut self) {
        ...
        self.performance_metrics.average_transfer_time = 
            alpha * transfer_time as f32 + 
            (1.0 - alpha) * self.performance_metrics.average_transfer_time;
    ...
    pub fn calculate_throughput(&mut self, bytes_transferred: u32) {
        if self.performance_metrics.average_transfer_time > 0.0 {
            let throughput_bps = bytes_transferred as f32 / 
                (self.performance_metrics.average_transfer_time / 1000000.0); // 转换为秒
