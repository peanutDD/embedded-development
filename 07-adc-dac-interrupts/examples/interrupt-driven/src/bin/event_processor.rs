...
use interrupt_driven::{
    EventProcessor, SystemEvent, Priority, EdgeType,
    ProcessorConfig,
};
    ...
    fn generate_random_event(&self) -> Option<(SystemEvent, Priority)> {
        ...
        match event_type {
            0 => Some((
                SystemEvent::GpioInterrupt { 
                    pin: (rand % 16) as u8, 
                    edge: if rand % 2 == 0 { EdgeType::Rising } else { EdgeType::Falling }
                },
                if rand % 3 == 0 { Priority::Critical } else { Priority::High }
            )),
            1 => Some((
                SystemEvent::TimerInterrupt { 
...
// 全局静态变量
static EVENT_CONTROLLER: Mutex<RefCell<Option<EventProcessorController<
    ...
    LED: embedded_hal::digital::OutputPin,
{
    ...
    fn emergency_processing(&mut self) {
        defmt::warn!("进入紧急处理模式，队列长度: {}", self.processor.queue_length());
    ...
    fn monitor_system(&mut self) {
        ...
        // 定期输出统计信息
        if self.stats.processing_cycles % (self.config.stats_interval / self.config.processing_interval) == 0 
    ...
    fn print_status(&self) {
        defmt::info!("=== 系统状态 ===");
        defmt::info!("处理模式: {:?}", self.processing_mode);
        defmt::info!("队列长度: {}/{}", self.processor.queue_length(), self.config.queue_size);
        defmt::info!("批处理大小: {}", self.config.batch_size);
...
fn main() -> ! {
    ...
    // 创建事件处理器控制器
    let mut controller = EventProcessorController::new(
        status_led,
        GeneratorMode::Mixed,
