...
impl<const N: usize> EventProcessor<N> {
  ...
  pub fn queue_length(&self) -> usize {
    critical_section::with(|cs| self.event_queue.borrow_ref(cs).len())
  }
  ...
  P: OutputPin,
{
  ...
  /// 获取队列长度
  pub fn queue_length(&self) -> usize {
    critical_section::with(|cs| self.task_queue.borrow_ref(cs).len())
  }
}
...
mod tests {
  ...
  fn test_event_processor() {
    ...
    // 测试事件添加
    let event = SystemEvent::GpioInterrupt {
      pin: 1,
  ...
  fn test_priority_ordering() {
    ...
    let event2 = SystemEvent::GpioInterrupt {
      pin: 1,
      edge: EdgeType::Rising,
    };
