...
use interrupt_driven::{
  AsyncInterruptHandler, AsyncTask, BufferConfig, EdgeType, EventProcessor, InterruptStateMachine,
  Priority, ProcessorConfig, RingBufferManager, SystemEvent, TaskType,
};
...
// 全局静态变量
static SYSTEM_CONTROLLER: Mutex<
  RefCell<
    Option<
      InterruptDrivenController<hal::gpio::Pin<Output<PushPull>>, hal::gpio::Pin<Input<PullUp>>>,
    >,
  >,
...
{
  /// 创建新的控制器
  fn new(status_led: LED, user_button: BTN) -> Self {
  ...
  fn update_performance_metrics(&mut self) {
    ...
    if self.performance_metrics.total_interrupts > 0 {
      self.performance_metrics.avg_response_time = self.performance_metrics.total_interrupts / 100;
      // 模拟值
    }
  }
  ...
  fn print_statistics(&self) {
    ...
    defmt::info!(
      "缓冲区读写: {}/{}",
      buffer_stats.total_reads,
      buffer_stats.total_writes
    );
