...
use stm32f4xx_hal::{
    ...
    dma::{Stream0, StreamsTuple, Transfer, PeripheralToMemory},
};
use nb::block;
...
// 全局变量用于中断处理
static SYNC_RESULTS: Mutex<RefCell<Option<SyncSampleResult>>> = 
...
fn monitor_critical_parameters(
    ...
    for (i, &voltage) in voltages.iter().enumerate() {
        if voltage > 3.1 || voltage < 0.2 {
            writeln!(serial, "警告: 通道{}电压异常 {:.3}V (样本#{})", 
                    i, voltage, count).ok();
        }
    }
    ...
    if correlation < 0.8 && count > 100 {
        writeln!(serial, "警告: ADC1_CH1与ADC3_CH1相关性低 {:.3} (样本#{})", 
                correlation, count).ok();
    }
}
...
fn perform_sync_analysis(
    ...
    for i in 0..SYNC_CHANNELS {
        let values: Vec<u16, BUFFER_SIZE> = buffer.iter()
        ...
        writeln!(serial, "通道{}: 均值={:.1}, 标准差={:.1}, 范围=[{}-{}]",
                i, stats.mean, stats.std_dev, stats.min, stats.max).ok();
    }
...
fn calculate_channel_correlations(
    ...
    writeln!(serial, "ADC1_CH1 vs ADC3_CH1 相关性: {:.3}", correlation_1_3).ok();
    ...
    writeln!(serial, "ADC1_CH2 vs ADC3_CH2 相关性: {:.3}", correlation_2_3).ok();
}
    ...
    I2: Iterator<Item = f32>,
{
    ...
    let sum_xy: f32 = x_values.iter().zip(y_values.iter()).map(|(&x, &y)| x * y).sum();
