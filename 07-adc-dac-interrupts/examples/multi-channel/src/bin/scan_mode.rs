...
use stm32f4xx_hal::{
    ...
    dma::{Stream0, StreamsTuple, Transfer, PeripheralToMemory, MemoryToPeripheral},
};
use nb::block;
...
fn detect_anomalies(
    ...
    for (i, &adc_value) in results.iter().enumerate() {
        ...
        if voltage > VOLTAGE_THRESHOLD_HIGH {
            writeln!(serial, "异常检测: CH{} 电压过高 {:.3}V (扫描#{})", 
                    i, voltage, scan_count).ok();
        } else if voltage < VOLTAGE_THRESHOLD_LOW {
            writeln!(serial, "异常检测: CH{} 电压过低 {:.3}V (扫描#{})", 
                    i, voltage, scan_count).ok();
        }
    }
}
