...
impl MultiChannelADC {
    ...
    /// 添加ADC通道
    pub fn add_channel<const P: char, const N: u8>(&mut self, pin: Pin<P, N, Analog>) -> Result<(), &'static str> {
...
impl ChannelCalibration {
    ...
    pub fn add_calibration_point(&mut self, point: CalibrationPoint) -> Result<(), &'static str> {
        self.calibration_data.push(point).map_err(|_| "Calibration buffer full")
    }
    
    /// 计算校准系数
    pub fn calculate_calibration(&mut self, channel: u8) -> Result<(f32, f32), &'static str> {
        let points: Vec<&CalibrationPoint, MAX_CHANNELS> = self.calibration_data
            .iter()
        ...
        let sum_xy: f32 = points.iter().map(|p| p.adc_value as f32 * p.actual_voltage).sum();
    ...
    pub fn filter(&mut self, input: f32) -> f32 {
        match self.filter_type {
            FilterType::MovingAverage(_) => {
                ...
                let output: f32 = self.delay_line.iter()
