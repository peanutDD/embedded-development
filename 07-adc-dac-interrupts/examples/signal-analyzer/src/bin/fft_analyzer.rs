...
use signal_analyzer::{FFTProcessor, Complex, adc_to_voltage};
...
// 全局FFT缓冲区
static FFT_BUFFER: Mutex<RefCell<[f32; FFT_SIZE]>> = 
    Mutex::new(RefCell::new([0.0; FFT_SIZE]));
...
fn main() -> ! {
    ...
    writeln!(serial, "频率分辨率: {:.2} Hz", SAMPLE_RATE / FFT_SIZE as f32).ok();
    ...
    loop {
        ...
        if analysis_timer.wait().is_ok() {
            ...
            if analysis_ready {
                ...
                // 执行FFT分析
                perform_fft_analysis(&mut serial, &fft_processor, &mut analysis_buffer, 
                                   analysis_count, &mut fft_stats);
...
fn perform_fft_analysis(
    ...
    // 输出分析结果
    if analysis_count % 5 == 0 { // 每5次分析输出一次详细结果
        output_detailed_analysis(serial, analysis_count, &original_stats, 
                                &spectrum_features, &magnitude_spectrum);
    }
...
fn calculate_signal_statistics(signal: &[f32; FFT_SIZE]) -> SignalStatistics {
    ...
    SignalStatistics {
        mean, rms, peak, peak_to_peak, std_dev, crest_factor,
    }
}
...
impl FFTStatistics {
    ...
    fn update(&mut self, _signal_stats: &SignalStatistics, spectrum_features: &SpectrumFeatures) {
        ...
        self.avg_dominant_freq = self.avg_dominant_freq * (1.0 - alpha) + 
                                spectrum_features.dominant_frequency * alpha;
        self.avg_total_power = self.avg_total_power * (1.0 - alpha) + 
                              spectrum_features.total_power * alpha;
        self.avg_snr = self.avg_snr * (1.0 - alpha) + 
                      spectrum_features.snr_estimate * alpha;
        self.avg_spectral_centroid = self.avg_spectral_centroid * (1.0 - alpha) + 
                                    spectrum_features.spectral_centroid * alpha;
        self.avg_spectral_flatness = self.avg_spectral_flatness * (1.0 - alpha) + 
                                    spectrum_features.spectral_flatness * alpha;
...
fn output_detailed_analysis(
    ...
    writeln!(serial, "  主导频率: {:.1} Hz", spectrum_features.dominant_frequency).ok();
    writeln!(serial, "  主导幅度: {:.4}", spectrum_features.dominant_magnitude).ok();
    writeln!(serial, "  总功率: {:.6}", spectrum_features.total_power).ok();
    writeln!(serial, "  频谱质心: {:.1} Hz", spectrum_features.spectral_centroid).ok();
    writeln!(serial, "  频谱扩散: {:.1} Hz", spectrum_features.spectral_spread).ok();
    writeln!(serial, "  频谱滚降: {:.1} Hz", spectrum_features.spectral_rolloff).ok();
    writeln!(serial, "  频谱平坦度: {:.3}", spectrum_features.spectral_flatness).ok();
    writeln!(serial, "  峰值数量: {}", spectrum_features.peak_count).ok();
    writeln!(serial, "  SNR估算: {:.1} dB", spectrum_features.snr_estimate).ok();
...
fn detect_spectrum_anomalies(
    ...
    if features.peak_count > 20 {
        writeln!(serial, "警告: 检测到过多频谱峰值 {} (分析#{})", 
                features.peak_count, analysis_count).ok();
    }
    
    // 检测异常低的SNR
    if features.snr_estimate < 10.0 {
        writeln!(serial, "警告: SNR过低 {:.1}dB (分析#{})", 
                features.snr_estimate, analysis_count).ok();
    }
    
    // 检测异常的频谱平坦度
    if features.spectral_flatness > 0.8 {
        writeln!(serial, "警告: 频谱过于平坦 {:.3} (可能为噪声) (分析#{})", 
                features.spectral_flatness, analysis_count).ok();
    }
    
    // 检测异常的主导频率
    if features.dominant_frequency > SAMPLE_RATE * 0.4 {
        writeln!(serial, "警告: 主导频率过高 {:.1}Hz (分析#{})", 
                features.dominant_frequency, analysis_count).ok();
    }
}
...
fn output_fft_status(
    ...
    writeln!(serial, "缓冲区填充: {} ({:.1}%)", stats.buffer_fills, buffer_fill_rate).ok();
    ...
    writeln!(serial, "平均频谱质心: {:.1} Hz", stats.avg_spectral_centroid).ok();
