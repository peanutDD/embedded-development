...
use nb::block;
use panic_halt as _;
use signal_analyzer::{adc_to_voltage, DigitalFilter, FilterType, SpectrumAnalyzer};
use stm32f4xx_hal::{
...
impl FilterAnalysisStatistics {
  ...
  fn update_filter_performance(
    ...
    performance.avg_snr_improvement =
      performance.avg_snr_improvement * (1.0 - alpha) + snr_improvement * alpha;

    // 计算THD减少
    let thd_reduction = if result.thd < 50.0 {
      // 合理的THD值
      result.thd
    } else {
      0.0
    };
    performance.avg_thd_reduction =
      performance.avg_thd_reduction * (1.0 - alpha) + thd_reduction * alpha;
    ...
    performance.noise_reduction_factor =
      performance.noise_reduction_factor * (1.0 - alpha) + noise_reduction * alpha;
  }
}
...
fn perform_filter_analysis(
  ...
  if analysis_count % 4 == 0 {
    // 每4次分析输出一次详细结果
    output_detailed_filter_analysis(
      serial,
      filter_index,
      analysis_count,
      &result,
...
fn analyze_filter_effectiveness(
  ...
  match filter_index {
    0 => {
      // 低通滤波器
      // 检查高频衰减
      let high_freq_power = calculate_band_power(
      ...
      if high_freq_ratio < 10.0 {
        writeln!(
          serial,
          "低通滤波器效果良好: 高频能量占比 {:.1}% (分析#{})",
          high_freq_ratio, analysis_count
        )
        .ok();
      } else {
        writeln!(
          serial,
          "低通滤波器效果不佳: 高频能量占比 {:.1}% (分析#{})",
          high_freq_ratio, analysis_count
        )
        .ok();
      }
    ...
    1 => {
      ...
      // 检查低频衰减
      let low_freq_power =
        calculate_band_power(&result.power_spectrum, &result.frequency_bins, 0.0, 2000.0);
      let total_power = result.total_power;
      ...
      if low_freq_ratio < 20.0 {
        writeln!(
          serial,
          "高通滤波器效果良好: 低频能量占比 {:.1}% (分析#{})",
          low_freq_ratio, analysis_count
        )
        .ok();
      } else {
        writeln!(
          serial,
          "高通滤波器效果不佳: 低频能量占比 {:.1}% (分析#{})",
          low_freq_ratio, analysis_count
        )
        .ok();
      }
    ...
    2 => {
      ...
      // 检查50Hz附近的衰减
      let notch_power =
        calculate_band_power(&result.power_spectrum, &result.frequency_bins, 45.0, 55.0);
      let total_power = result.total_power;
      ...
      if notch_ratio < 5.0 {
        writeln!(
          serial,
          "50Hz陷波滤波器效果良好: 50Hz能量占比 {:.1}% (分析#{})",
          notch_ratio, analysis_count
        )
        .ok();
      }
    }
    3 => {
      // 1kHz陷波滤波器
      // 检查1kHz附近的衰减
      let notch_power = calculate_band_power(
        &result.power_spectrum,
        &result.frequency_bins,
        950.0,
        1050.0,
...
fn analyze_frequency_response(
  ...
  for (band_name, low_freq, high_freq) in bands.iter() {
    let band_power = calculate_band_power(
      &result.power_spectrum,
      &result.frequency_bins,
      *low_freq,
      *high_freq,
...
fn output_filter_status(
  ...
  for i in 0..5 {
    ...
    writeln!(
      serial,
      "  {}: {} 次",
      filter_name, stats.filter_usage_count[i]
    )
    .ok();
  }
...
fn simulate_noisy_signal(sample_count: u32) -> u16 {
  ...
  // 高频噪声
  let high_freq_noise =
    100.0 * (2.0 * core::f32::consts::PI * 5000.0 * t).sin() * ((sample_count * 3) % 7) as f32
      / 7.0;
  ...
  let result =
    dc_offset + signal + power_line_noise + high_freq_noise + scaled_white_noise + impulse_noise;
