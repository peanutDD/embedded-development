...
use nb::block;
use panic_halt as _;
use signal_analyzer::{adc_to_voltage, HarmonicAnalyzer, SpectrumAnalyzer};
use stm32f4xx_hal::{
...
impl HarmonicAnalysisStatistics {
  ...
  fn update_harmonic_distribution(
    ...
    // 更新基频功率
    distribution.avg_fundamental_power =
      distribution.avg_fundamental_power * (1.0 - alpha) + result.fundamental_power * alpha;
    ...
    for (i, harmonic) in result.harmonics.iter().take(10).enumerate() {
      distribution.harmonic_pattern[i] =
        distribution.harmonic_pattern[i] * (1.0 - alpha) + harmonic.power * alpha;
    }

    // 更新最大谐波阶数
    if !result.harmonics.is_empty() {
      let max_order = result.harmonics.iter().map(|h| h.order).max().unwrap_or(0);
      distribution.max_harmonic_order = distribution.max_harmonic_order.max(max_order);
    }

    // 更新特定谐波
    if result.harmonics.len() > 0 {
      distribution.avg_second_harmonic = result
        .harmonics
        .iter()
        .find(|h| h.order == 2)
        .map(|h| h.power)
      ...
      distribution.avg_third_harmonic = result
        .harmonics
        .iter()
        .find(|h| h.order == 3)
        .map(|h| h.power)
    ...
    // 计算总谐波功率
    let total_harmonic_power: f32 = result
      .harmonics
      .iter()
      .filter(|h| h.order > 1)
      .map(|h| h.power)
      .sum();
    distribution.avg_total_harmonics =
      distribution.avg_total_harmonics * (1.0 - alpha) + total_harmonic_power * alpha;
  }
...
fn perform_harmonic_analysis(
  ...
  if analysis_count % 2 == 0 {
    // 每2次分析输出一次详细结果
    output_detailed_harmonic_analysis(
      serial,
      signal_type,
      analysis_count,
      &harmonic_result,
...
fn output_detailed_harmonic_analysis(
  ...
  writeln!(
    serial,
    "  基频功率: {:.6}",
    harmonic_result.fundamental_power
  )
  .ok();
  ...
  for (i, harmonic) in harmonic_result.harmonics.iter().take(10).enumerate() {
    ...
    writeln!(
      serial,
      "  H{}: {:.1} Hz, 功率: {:.6} ({:.1}%)",
      harmonic.order, harmonic.frequency, harmonic.power, percentage
    )
    .ok();
  }
  ...
  match signal_type {
    ...
    1 => {
      ...
      writeln!(
        serial,
        "方波特征: {} 个奇次谐波, THD {:.1}% (分析#{})",
        expected_harmonics, result.thd, analysis_count
      )
      .ok();
    ...
    2 => {
      ...
      writeln!(
        serial,
        "锯齿波特征: {} 个谐波, THD {:.1}% (分析#{})",
        total_harmonics, result.thd, analysis_count
      )
      .ok();
    ...
    3 => {
      ...
      writeln!(
        serial,
        "三角波特征: {} 个奇次谐波, THD {:.1}% (分析#{})",
        odd_harmonics, result.thd, analysis_count
      )
      .ok();
...
fn check_square_wave_harmonics(
  ...
  let odd_harmonics: heapless::Vec<_, 10> = harmonics
    .iter()
    .filter(|h| h.order % 2 == 1 && h.order > 1)
    .take(5)
    .collect();
  ...
  if odd_harmonics.len() >= 2 {
    ...
    for harmonic in odd_harmonics.iter() {
      ...
      writeln!(
        serial,
        "  H{}: 理论比例 1/{} = {:.3}",
        harmonic.order, harmonic.order, theoretical_ratio
      )
      .ok();
    }
  }
    ...
    for (i, harmonic) in harmonics.iter().take(5).enumerate() {
      if harmonic.order > 1 {
        ...
        writeln!(
          serial,
          "  H{}: 期望衰减 1/{} = {:.3}",
          harmonic.order, harmonic.order, expected_decay
        )
        .ok();
      }
    }
...
fn assess_harmonic_quality(
  ...
  writeln!(
    ...
    "  谐波丰富度: {} ({} 个谐波)",
    harmonic_richness,
    harmonic_result.harmonics.len()
  )
  .ok();

  // 基频稳定性
  let freq_stability =
    if (harmonic_result.fundamental_frequency - get_expected_frequency(signal_type)).abs() < 5.0 {
      "稳定"
    } else {
      "不稳定"
    };
  ...
  writeln!(
    serial,
    "  基频稳定性: {} ({:.1} Hz)",
    freq_stability, harmonic_result.fundamental_frequency
  )
  .ok();
...
fn detect_harmonic_anomalies(
  ...
  // 检查高次谐波异常强
  let high_order_harmonics = result.harmonics.iter().filter(|h| h.order > 10).count();
...
fn output_harmonic_status(
  ...
  for i in 0..5 {
    ...
    writeln!(
      serial,
      "  {}: {} 次",
      signal_name, stats.signal_usage_count[i]
    )
    .ok();
  }
