...
use signal_analyzer::{RealTimeAnalyzer, SpectrumAnalyzer, HarmonicAnalyzer, DigitalFilter, FilterType, adc_to_voltage};
    ...
    loop {
        ...
        if realtime_timer.wait().is_ok() {
            ...
            if realtime_analyzer.is_ready() {
                ...
                // 根据当前模式执行不同的分析
                match current_mode {
                    0 => perform_basic_realtime_analysis(&mut serial, &realtime_result, 
                                                       analysis_count, &mut realtime_stats),
                    1 => perform_spectrum_realtime_analysis(&mut serial, &spectrum_analyzer, 
                                                          analysis_count, &mut realtime_stats),
                    2 => perform_harmonic_realtime_analysis(&mut serial, &harmonic_analyzer, 
                                                          analysis_count, &mut realtime_stats),
                    3 => perform_comprehensive_realtime_analysis(&mut serial, &realtime_result,
                                                               &spectrum_analyzer, &harmonic_analyzer,
                                                               analysis_count, &mut realtime_stats),
                    _ => {}
                }
        ...
        if status_timer.wait().is_ok() {
            output_realtime_status(&mut serial, sample_count, analysis_count, 
                                 current_mode, &realtime_stats, &performance_monitor);
        }
    }
}
...
impl RealTimeAnalysisStatistics {
    ...
    fn update_signal_characteristics(&mut self, result: &signal_analyzer::RealTimeResult<REALTIME_SIZE>) {
...
fn perform_basic_realtime_analysis(
    ...
    // 输出基础分析结果
    if analysis_count % 5 == 0 { // 每5次分析输出一次
...
fn perform_spectrum_realtime_analysis(
    ...
    // 输出频谱分析结果
    if analysis_count % 3 == 0 { // 每3次分析输出一次
        writeln!(serial, "\n=== 频谱实时分析 #{} ===", analysis_count).ok();
        writeln!(serial, "总功率: {:.6}", spectrum_result.total_power).ok();
        writeln!(serial, "基频: {:.1} Hz", spectrum_result.fundamental_frequency).ok();
...
fn perform_harmonic_realtime_analysis(
    ...
    // 输出谐波分析结果
    if analysis_count % 4 == 0 { // 每4次分析输出一次
        ...
        for (i, harmonic) in harmonic_result.harmonics.iter().take(5).enumerate() {
            ...
            writeln!(serial, "  H{}: {:.1} Hz ({:.1}%)", 
                    harmonic.order, harmonic.frequency, percentage).ok();
        }
...
fn perform_comprehensive_realtime_analysis(
    ...
    // 输出综合分析结果
    if analysis_count % 6 == 0 { // 每6次分析输出一次
        ...
        writeln!(serial, "  主频: {:.1} Hz", spectrum_result.fundamental_frequency).ok();
        ...
        // 信号质量评估
        let overall_quality = assess_overall_signal_quality(realtime_result, &spectrum_result, &harmonic_result);
        writeln!(serial, "整体质量: {:.1}%", overall_quality * 100.0).ok();
        
        // 综合异常检测
        detect_comprehensive_anomalies(serial, realtime_result, &spectrum_result, &harmonic_result, stats);
...
fn check_realtime_alerts(
    ...
    if result.peak_amplitude > 2.5 {
        writeln!(serial, "🚨 峰值警报: {:.3} V (分析#{})", result.peak_amplitude, analysis_count).ok();
    }
    ...
    unsafe {
        if LAST_FREQUENCY > 0.0 {
            ...
            if freq_change > 100.0 {
                writeln!(serial, "🚨 频率跳变: {:.1} Hz -> {:.1} Hz (分析#{})", 
                        LAST_FREQUENCY, result.dominant_frequency, analysis_count).ok();
            }
        }
...
fn detect_spectrum_changes(
    ...
    unsafe {
        ...
        if LAST_PEAK_COUNT > 0 {
            ...
            if peak_change > 2 {
                writeln!(serial, "频谱变化: 峰值数 {} -> {} (分析#{})", 
                        LAST_PEAK_COUNT, result.peaks.len(), analysis_count).ok();
            }
        }
        ...
        if LAST_TOTAL_POWER > 0.0 {
            ...
            if power_ratio > 2.0 || power_ratio < 0.5 {
                writeln!(serial, "功率变化: {:.6} -> {:.6} (分析#{})", 
                        LAST_TOTAL_POWER, result.total_power, analysis_count).ok();
            }
        }
...
fn detect_comprehensive_anomalies(
    ...
    // 频域时域不一致
    let freq_diff = (realtime_result.dominant_frequency - spectrum_result.fundamental_frequency).abs();
    if freq_diff > 50.0 {
...
fn output_realtime_status(
    ...
    writeln!(serial, "  平均采样时间: {:.1} μs", performance.avg_sampling_time).ok();
    writeln!(serial, "  平均分析时间: {:.1} μs", performance.avg_analysis_time).ok();
    writeln!(serial, "  CPU利用率: {:.1}%", performance.get_cpu_utilization()).ok();
    ...
    writeln!(serial, "  平均幅度: {:.4} V", stats.signal_characteristics.avg_amplitude).ok();
    writeln!(serial, "  平均频率: {:.1} Hz", stats.signal_characteristics.avg_frequency).ok();
    writeln!(serial, "  信号稳定性: {:.3}", stats.signal_characteristics.signal_stability).ok();
    writeln!(serial, "  噪声水平: {:.4} V", stats.signal_characteristics.noise_level).ok();
...
fn simulate_realtime_signal(sample_count: u32) -> u16 {
    ...
    let main_signal = 600.0 * amplitude_modulation * 
