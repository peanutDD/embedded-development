...
use signal_analyzer::{SpectrumAnalyzer, adc_to_voltage};
...
fn main() -> ! {
    ...
    writeln!(serial, "频率分辨率: {:.2} Hz", SAMPLE_RATE / SPECTRUM_SIZE as f32).ok();
    ...
    loop {
        ...
        if analysis_timer.wait().is_ok() {
            ...
            if analysis_ready {
                ...
                // 执行频谱分析
                perform_spectrum_analysis(&mut serial, &spectrum_analyzer, &mut analysis_buffer, 
                                        analysis_count, &mut spectrum_stats, &mut frequency_tracker);
        ...
        if status_timer.wait().is_ok() {
            output_spectrum_status(&mut serial, sample_count, analysis_count, 
                                 &spectrum_stats, &frequency_tracker);
        }
    }
}
...
fn perform_spectrum_analysis(
    ...
    // 输出分析结果
    if analysis_count % 3 == 0 { // 每3次分析输出一次详细结果
...
impl SpectrumStatistics {
    ...
    fn update(&mut self, result: &signal_analyzer::SpectrumResult<SPECTRUM_SIZE>) {
        ...
        self.avg_fundamental_freq = self.avg_fundamental_freq * (1.0 - alpha) + 
                                   result.fundamental_frequency * alpha;
...
impl FrequencyTracker {
        ...
        for tracked in self.tracked_frequencies.iter_mut() {
            ...
            for peak in peaks.iter() {
                if (peak.frequency - tracked.frequency).abs() < self.frequency_tolerance {
                    ...
                    // 更新平均幅度
                    let alpha = 0.1f32;
                    tracked.avg_magnitude = tracked.avg_magnitude * (1.0 - alpha) + 
                                          peak.magnitude * alpha;
                    
                    // 更新幅度方差
                    let diff = peak.magnitude - tracked.avg_magnitude;
                    tracked.magnitude_variance = tracked.magnitude_variance * (1.0 - alpha) + 
                                               diff * diff * alpha;
    ...
    fn get_stable_frequencies(&self) -> heapless::Vec<TrackedFrequency, 16> {
        ...
        for freq in self.tracked_frequencies.iter() {
            if freq.stability_count >= 5 { // 至少稳定出现5次
...
fn detect_spectrum_changes(
    ...
    for peak in result.peaks.iter() {
        ...
        if !is_stable && peak.magnitude > 0.1 {
            writeln!(serial, "新频率检测: {:.1} Hz ({:.4}) (分析#{})", 
                    peak.frequency, peak.magnitude, analysis_count).ok();
        }
    }
    ...
    for stable in stable_frequencies.iter() {
        if stable.last_seen > 3 {
            writeln!(serial, "频率消失: {:.1} Hz (分析#{})", 
                    stable.frequency, analysis_count).ok();
        }
    }
    ...
    for stable in stable_frequencies.iter() {
        if stable.magnitude_variance > 0.01 {
            writeln!(serial, "频率幅度不稳定: {:.1} Hz (方差={:.4}) (分析#{})", 
                    stable.frequency, stable.magnitude_variance, analysis_count).ok();
        }
    }
}
...
fn analyze_frequency_bands(
    ...
    if analysis_count % 10 == 0 {
        ...
        for (name, low_freq, high_freq) in bands.iter() {
            ...
            writeln!(serial, "  {}: {:.2}% ({} 峰值)", 
                    name, band_percentage, band_peak_count).ok();
        }
        writeln!(serial, "").ok();
    }
}
...
fn simulate_multi_tone_signal(sample_count: u32) -> u16 {
    ...
    let noise = 30.0 * (2.0 * core::f32::consts::PI * noise_freq * t).sin() * 
                ((sample_count * 7 + 13) % 31) as f32 / 31.0;
