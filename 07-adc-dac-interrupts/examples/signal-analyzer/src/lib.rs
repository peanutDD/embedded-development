...
impl<const N: usize> SpectrumAnalyzer<N> {
  ...
  pub fn analyze(&self, signal: &mut [f32; N]) -> SpectrumResult<N> {
    ...
    self
      .fft_processor
      .power_spectrum(&fft_output, &mut power_spectrum);

    // 计算幅度谱
    let mut magnitude_spectrum = [0.0f32; N];
    self
      .fft_processor
      .magnitude_spectrum(&fft_output, &mut magnitude_spectrum);
  ...
  fn calculate_snr(&self, magnitude: &[f32; N]) -> f32 {
    ...
    // 找到最大信号
    let max_signal = magnitude[1..half_n]
      .iter()
...
impl<const N: usize> RealTimeAnalyzer<N> {
  ...
  pub fn analyze(&mut self) -> Option<RealTimeResult<N>> {
    ...
    let harmonic_result = self
      .harmonic_analyzer
      .analyze_harmonics(&mut harmonic_buffer);
