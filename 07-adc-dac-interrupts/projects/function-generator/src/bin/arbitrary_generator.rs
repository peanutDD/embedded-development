...
use function_generator::{
  dac_to_voltage, voltage_to_dac, ArbitraryWaveform, GeneratorConfig, GeneratorStatistics,
  MultiChannelGenerator, WaveformGenerator, WaveformType,
};
use nb::block;
use panic_halt as _;
use stm32f4xx_hal::{
...
impl WaveformAnalyzer {
  ...
  fn calculate_timing_parameters(&mut self, buffer: &[u16]) {
    ...
    let threshold_low =
      (self.min_value as f32 + (self.peak_value - self.min_value) as f32 * 0.1) as u16;
...
impl ArbitraryPerformance {
  ...
  fn calculate_fidelity(&self, analyzer: &WaveformAnalyzer) -> f32 {
    // 基于波形参数计算保真度
    let peak_score = if analyzer.peak_value > 100 && analyzer.peak_value < 4000 {
      100.0
    } else {
      80.0
    };
    let rms_score = if analyzer.rms_value > 100.0 && analyzer.rms_value < 3000.0 {
      100.0
    } else {
      80.0
    };
...
fn main() -> ! {
  ...
  writeln!(
    tx,
    "波形库: {}种预定义波形",
    waveform_library.waveform_count
  )
  .unwrap();
  ...
  loop {
    ...
    // 定期输出状态信息和分析
    if sample_count.wrapping_sub(last_status_time) >= 800000 {
      // 每2秒
      ...
      writeln!(
        tx,
        "波形切换次数: {}",
        arbitrary_performance.waveform_switches
      )
      .unwrap();
      ...
      writeln!(
        tx,
        "波形编号: {}/{}",
        waveform_library.current_waveform + 1,
        waveform_library.waveform_count
      )
      .unwrap();
      writeln!(tx, "波形大小: {}点", WAVEFORM_SIZE).unwrap();
      ...
      writeln!(
        tx,
        "峰值: {} ({}mV)",
        waveform_analyzer.peak_value,
        dac_to_voltage(waveform_analyzer.peak_value, VREF_MV)
      )
      .unwrap();
      writeln!(
        tx,
        "最小值: {} ({}mV)",
        waveform_analyzer.min_value,
        dac_to_voltage(waveform_analyzer.min_value, VREF_MV)
      )
      .unwrap();
      writeln!(
        tx,
        "峰峰值: {} ({}mV)",
        waveform_analyzer.peak_to_peak,
        dac_to_voltage(waveform_analyzer.peak_to_peak, VREF_MV).abs()
      )
      .unwrap();
      ...
      writeln!(
        tx,
        "上升时间: {}us",
        waveform_analyzer.rise_time * 1000000 / SAMPLE_RATE
      )
      .unwrap();
      writeln!(
        tx,
        "脉冲宽度: {}us",
        waveform_analyzer.pulse_width * 1000000 / SAMPLE_RATE
      )
      .unwrap();
      ...
      writeln!(
        tx,
        "波形分类: {}",
        waveform_analyzer.get_waveform_classification()
      )
      .unwrap();
      ...
      writeln!(
        tx,
        "波形保真度: {:.1}%",
        arbitrary_performance.waveform_fidelity
      )
      .unwrap();
      ...
      writeln!(
        tx,
        "切换时间: {}ms",
        arbitrary_performance.switching_time / (SAMPLE_RATE / 1000)
      )
      .unwrap();
      ...
      writeln!(
        tx,
        "性能评级: {}",
        arbitrary_performance.get_performance_rating()
      )
      .unwrap();
      ...
      let remaining_time =
        (waveform_switch_interval - (sample_count - last_waveform_switch)) / SAMPLE_RATE;
