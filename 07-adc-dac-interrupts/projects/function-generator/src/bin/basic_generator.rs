...
impl GeneratorState {
  ...
  fn update_parameters(&mut self, time: u32) {
    // 每10秒切换波形
    if time.wrapping_sub(self.last_switch_time) >= 1000000 {
      // 10秒 @ 100kHz
  ...
  loop {
    ...
    if sample_count.wrapping_sub(last_status_time) >= 500000 {
      ...
      writeln!(
        tx,
        "平均生成时间: {}us",
        performance.get_average_generation_time() * 10
      )
      .unwrap();
      writeln!(
        tx,
        "平均输出时间: {}us",
        performance.get_average_output_time() * 10
      )
      .unwrap();
      writeln!(
        tx,
        "最大生成时间: {}us",
        performance.max_generation_time * 10
      )
      .unwrap();
      ...
      writeln!(
        ...
        "峰峰值: {} ({}mV)",
        peak_to_peak,
        dac_to_voltage(peak_to_peak, VREF_MV).abs()
      )
      .unwrap();
      writeln!(
        tx,
        "平均值: {} ({}mV)",
        avg_sample,
        dac_to_voltage(avg_sample as u16, VREF_MV)
      )
      .unwrap();
      writeln!(
        tx,
        "最大值: {} ({}mV)",
        max_sample,
        dac_to_voltage(max_sample, VREF_MV)
      )
      .unwrap();
      writeln!(
        tx,
        "最小值: {} ({}mV)",
        min_sample,
        dac_to_voltage(min_sample, VREF_MV)
      )
      .unwrap();
...
fn generate_complex_waveform(time: u32, waveform_type: WaveformType) -> u16 {
  ...
  match waveform_type {
    ...
    WaveformType::Pulse => {
      let pulse = if ((t * 1000.0) % 1.0) < 0.25 {
        1.0
      } else {
        -1.0
      };
