...
use function_generator::{
    WaveformGenerator, GeneratorConfig, WaveformType, ModulationConfig, ModulationType,
...
impl ModulationController {
    ...
    fn update(&mut self, sample_count: u32) -> bool {
        ...
        // 每8秒切换调制类型和参数
        if sample_count.wrapping_sub(self.last_switch_time) >= 2400000 { // 8秒 @ 300kHz
...
impl ModulationAnalyzer {
    ...
    fn analyze(&mut self, buffer: &[u16], carrier_freq: u32, mod_freq: u32, mod_type: ModulationType) {
    ...
    fn get_modulation_quality(&self) -> f32 {
        // 调制质量评分 (0-100)
        let depth_score = if self.modulation_depth_measured > 0.1 && self.modulation_depth_measured < 0.9 {
            100.0
        } else {
            50.0
        };
...
impl ModulationPerformance {
    ...
    fn update(&mut self, analyzer: &ModulationAnalyzer, controller: &ModulationController, sample_count: u32) {
        // 计算调制精度
        ...
        // 计算频率稳定性
        let freq_error = analyzer.carrier_frequency_measured.abs_diff(controller.carrier_freq) as f32;
...
fn main() -> ! {
    ...
    loop {
        ...
        // 定期输出状态信息和分析
        if sample_count.wrapping_sub(last_status_time) >= 600000 { // 每2秒
            ...
            // 更新基础统计信息
            statistics.update(&sample_buffer, modulation_controller.carrier_freq, SAMPLE_RATE);
            ...
            writeln!(tx, "调制深度: {:.1}%", modulation_controller.modulation_depth * 100.0).unwrap();
            writeln!(tx, "调制指数: {:.2}", modulation_controller.modulation_index).unwrap();
            
            // 显示调制分析结果
            writeln!(tx, "\n--- 调制分析 ---").unwrap();
            writeln!(tx, "载波功率: {:.1}", modulation_analyzer.carrier_power).unwrap();
            writeln!(tx, "边带功率: {:.1}", modulation_analyzer.sideband_power).unwrap();
            writeln!(tx, "测量调制深度: {:.1}%", modulation_analyzer.modulation_depth_measured * 100.0).unwrap();
            writeln!(tx, "测量调制指数: {:.2}", modulation_analyzer.modulation_index_measured).unwrap();
            writeln!(tx, "载波频率测量: {}Hz", modulation_analyzer.carrier_frequency_measured).unwrap();
            writeln!(tx, "调制频率测量: {}Hz", modulation_analyzer.modulation_frequency_measured).unwrap();
            writeln!(tx, "调制失真: {:.2}%", modulation_analyzer.distortion).unwrap();
            writeln!(tx, "杂散信号: {:.2}%", modulation_analyzer.spurious_signals).unwrap();
            writeln!(tx, "调制质量: {:.1}/100", modulation_analyzer.get_modulation_quality()).unwrap();
            ...
            writeln!(tx, "峰峰值: {}mV", 
                     ...
                     dac_to_voltage(*sample_buffer.iter().min().unwrap_or(&0), VREF_MV)).unwrap();
            
            // 显示调制性能
            writeln!(tx, "\n--- 调制性能 ---").unwrap();
            writeln!(tx, "调制精度: {:.1}%", modulation_performance.modulation_accuracy).unwrap();
            writeln!(tx, "频率稳定性: {:.1}%", modulation_performance.frequency_stability).unwrap();
            writeln!(tx, "相位噪声: {:.2}%", modulation_performance.phase_noise).unwrap();
            writeln!(tx, "杂散抑制: {:.1}dB", modulation_performance.spurious_suppression).unwrap();
            writeln!(tx, "切换时间: {}ms", modulation_performance.switching_time / (SAMPLE_RATE / 1000)).unwrap();
            writeln!(tx, "综合性能: {:.1}/100", modulation_performance.get_overall_performance()).unwrap();
            ...
            let remaining_time = (2400000 - (sample_count - modulation_controller.last_switch_time)) / SAMPLE_RATE;
            writeln!(tx, "\n--- 下一个调制 ---").unwrap();
