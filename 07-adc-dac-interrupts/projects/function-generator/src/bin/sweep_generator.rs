...
use function_generator::{
  dac_to_voltage, voltage_to_dac, GeneratorConfig, GeneratorStatistics, SweepConfig, SweepType,
  WaveformGenerator, WaveformType,
};
use nb::block;
use panic_halt as _;
use stm32f4xx_hal::{
...
impl SweepController {
  ...
  fn update(&mut self) -> (u32, u16) {
    ...
    self.current_amplitude =
      (self.start_amplitude as i32 + (amp_range as f32 * progress) as i32) as u16;
  ...
  fn update(
    ...
    if target_freq > 0 {
      self.frequency_accuracy =
        100.0 - (current_freq.abs_diff(target_freq) as f32 / target_freq as f32 * 100.0);
    }

    // 计算幅度精度
    if target_amp > 0 {
      self.amplitude_accuracy =
        100.0 - (current_amp.abs_diff(target_amp) as f32 / target_amp as f32 * 100.0);
    }
...
fn main() -> ! {
  ...
  writeln!(
    tx,
    "扫频范围: {}Hz - {}Hz",
    sweep_controller.start_freq, sweep_controller.stop_freq
  )
  .unwrap();
  writeln!(
    tx,
    "幅度范围: {}mV - {}mV",
    sweep_controller.start_amplitude, sweep_controller.stop_amplitude
  )
  .unwrap();
  writeln!(
    tx,
    "扫描时间: {}秒",
    sweep_controller.sweep_time / SAMPLE_RATE
  )
  .unwrap();
  ...
  loop {
    ...
    if sample_count.wrapping_sub(last_status_time) >= 500000 {
      ...
      writeln!(
        tx,
        "频率跨度: {}Hz",
        sweep_controller.stop_freq - sweep_controller.start_freq
      )
      .unwrap();
      writeln!(
        tx,
        "幅度跨度: {}mV",
        sweep_controller.stop_amplitude - sweep_controller.start_amplitude
      )
      .unwrap();
      ...
      writeln!(
        tx,
        "幅度误差: {:.1}%",
        (output_voltage.abs_diff(sweep_controller.current_amplitude as i16) as f32
          / sweep_controller.current_amplitude as f32
          * 100.0)
      )
      .unwrap();
      ...
      writeln!(
        tx,
        "频率分辨率: {:.1}Hz",
        SAMPLE_RATE as f32 / sweep_controller.total_steps as f32
      )
      .unwrap();
