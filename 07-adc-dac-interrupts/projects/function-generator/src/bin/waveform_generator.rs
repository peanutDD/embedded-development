...
impl WaveformQuality {
    ...
    fn analyze(&mut self, buffer: &[u16], waveform_type: WaveformType) {
        ...
        // 计算线性度误差（针对三角波和锯齿波）
        if matches!(waveform_type, WaveformType::Triangle | WaveformType::Sawtooth) {
...
fn main() -> ! {
    ...
    writeln!(tx, "每个波形持续: {}秒", sequence.time_per_waveform / SAMPLE_RATE).unwrap();
    ...
    loop {
        ...
        // 定期输出状态信息和分析
        if sample_count.wrapping_sub(last_status_time) >= 400000 { // 每2秒
            ...
            if matches!(current_waveform, WaveformType::Triangle | WaveformType::Sawtooth) {
