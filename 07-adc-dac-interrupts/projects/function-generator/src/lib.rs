...
impl WaveformGenerator {
  ...
  pub fn generate_sample(&mut self) -> u16 {
    ...
    // 计算相位增量
    let phase_increment =
      2.0 * core::f32::consts::PI * self.current_frequency as f32 / self.config.sample_rate as f32;

    // 生成基础波形
    let mut sample =
      self.generate_waveform(self.phase_accumulator + self.config.phase.to_radians());
    ...
    // 更新调制相位
    let mod_phase_increment =
      2.0 * core::f32::consts::PI * self.modulation.modulation_frequency as f32
  ...
  fn generate_waveform(&self, phase: f32) -> f32 {
    match self.config.waveform {
      ...
      WaveformType::Pulse => {
        let normalized_phase = phase / (2.0 * core::f32::consts::PI);
        let frac = normalized_phase - normalized_phase.floor();
        if frac < (self.config.duty_cycle as f32 / 100.0) {
          1.0
        } else {
          -1.0
        }
  ...
  fn apply_modulation(&self, sample: f32) -> f32 {
    match self.modulation.modulation_type {
      ...
      ModulationType::AM => {
        let mod_signal =
          (self.modulation_phase + self.modulation.modulation_phase.to_radians()).sin();
  ...
  fn update_sweep(&mut self) {
    match self.sweep.sweep_type {
      SweepType::None => {}
      SweepType::Linear => {
        let sweep_progress = (self.sample_count as f32 * 1000.0)
          / (self.config.sample_rate as f32 * self.sweep.sweep_time as f32);
        if sweep_progress <= 1.0 {
          ...
          if self.sweep.sweep_direction {
            self.current_frequency =
              self.sweep.start_frequency + (freq_range * sweep_progress) as u32;
          } else {
            self.current_frequency =
              self.sweep.stop_frequency - (freq_range * sweep_progress) as u32;
          }
        }
      }
  ...
  pub fn update(&mut self, samples: &[u16], target_frequency: u32, sample_rate: u32) {
    ...
    if let Some(measured_freq) = self.measure_frequency(samples, sample_rate) {
      self.frequency_accuracy =
        ((measured_freq as f32 - target_frequency as f32) / target_frequency as f32).abs() * 100.0;
    }
