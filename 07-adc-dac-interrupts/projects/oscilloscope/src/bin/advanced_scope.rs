...
impl AdvancedMeasurementEngine {
    ...
    fn measure_single(&self, data: &[u16], measurement_type: MeasurementType, sample_rate: u32) -> f32 {
        match measurement_type {
            ...
            MeasurementType::Period => {
                self.calculate_period(data, sample_rate)
            },
            MeasurementType::Frequency => {
            ...
            MeasurementType::RiseTime => {
                self.calculate_rise_time(data, sample_rate)
            },
            MeasurementType::FallTime => {
                self.calculate_fall_time(data, sample_rate)
            },
            MeasurementType::PulseWidth => {
                self.calculate_pulse_width(data, sample_rate)
            },
            MeasurementType::DutyCycle => {
                self.calculate_duty_cycle(data)
            },
            MeasurementType::PulseCount => {
                self.count_pulses(data) as f32
            },
            MeasurementType::Mean => {
                let sum: u32 = data.iter().map(|&x| x as u32).sum();
            ...
            MeasurementType::StdDev => {
                self.calculate_std_dev(data)
            },
            MeasurementType::OvershootPos => {
                self.calculate_overshoot(data, true)
            },
            _ => 0.0, // 其他测量类型的占位实现
        }
    }
    ...
    fn get_measurement_unit(&self, measurement_type: MeasurementType) -> &'static str {
        match measurement_type {
            MeasurementType::VoltageMax | MeasurementType::VoltageMin | 
            MeasurementType::VoltagePP | MeasurementType::VoltageRMS | 
            MeasurementType::VoltageAvg | MeasurementType::VoltageAmp => "mV",
            MeasurementType::Period => "s",
            MeasurementType::Frequency => "Hz",
            MeasurementType::RiseTime | MeasurementType::FallTime | 
            MeasurementType::PulseWidth => "μs",
...
fn main() -> ! {
    ...
    loop {
        ...
        if sample_count.wrapping_sub(last_status_time) >= 1000000 { // 每2秒
            ...
            writeln!(tx, "触发电平: {}mV", 
                     (trigger_system.config.level1 as u32 * VREF_MV as u32) / 4096).unwrap();
            ...
            // 显示测量统计
            writeln!(tx, "\n--- 测量统计 ---").unwrap();
            writeln!(tx, "测量速率: {:.1}次/秒", measurement_engine.statistics.measurement_rate).unwrap();
            writeln!(tx, "测量精度: {:.1}%", measurement_engine.statistics.accuracy).unwrap();
            writeln!(tx, "测量精密度: {:.1}%", measurement_engine.statistics.precision).unwrap();
            writeln!(tx, "测量稳定性: {:.1}%", measurement_engine.statistics.stability).unwrap();
            writeln!(tx, "更新时间: {}us", measurement_engine.statistics.update_time).unwrap();
            ...
            let remaining_time = (trigger_switch_interval - (sample_count - last_trigger_switch_time)) / SAMPLE_RATE;
