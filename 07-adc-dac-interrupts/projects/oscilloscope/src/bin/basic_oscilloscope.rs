...
use nb::block;
use oscilloscope::{
  adc_to_voltage, ChannelConfig, DisplayBuffer, OscilloscopeBuffer, OscilloscopeMeasurement,
  TimebaseConfig, TriggerConfig, TriggerDetector, TriggerType,
};
use panic_halt as _;
use stm32f4xx_hal::{
  ...
  loop {
    ...
    if sample_count.wrapping_sub(last_status_time) >= 500000 {
      ...
      writeln!(
        tx,
        "幅度: {}mV (ADC: {})",
        amplitude_mv, statistics.last_amplitude
      )
      .unwrap();
      writeln!(
        tx,
        "直流偏移: {}mV (ADC: {})",
        dc_offset_mv, statistics.last_dc_offset
      )
      .unwrap();
      ...
      writeln!(
        tx,
        "平均采集时间: {}us",
        performance.get_average_acquisition_time() * 10
      )
      .unwrap();
      writeln!(
        tx,
        "平均处理时间: {}us",
        performance.get_average_processing_time() * 10
      )
      .unwrap();
      writeln!(
        tx,
        "最大采集时间: {}us",
        performance.max_acquisition_time * 10
      )
      .unwrap();
      writeln!(
        tx,
        "最大处理时间: {}us",
        performance.max_processing_time * 10
      )
      .unwrap();
      ...
      writeln!(
        tx,
        "触发阈值: {}mV",
        adc_to_voltage(trigger_config.threshold, VREF_MV)
      )
      .unwrap();
      ...
      writeln!(
        tx,
        "光标位置: ({}, {})",
        display_buffer.cursor_x, display_buffer.cursor_y
      )
      .unwrap();
    }
  }
...
fn generate_test_signal(time: u32, signal_type: u8) -> u16 {
  ...
  match signal_type {
    ...
    1 => {
      // 方波
      let square = if (t * 1000.0) as u32 % 2 == 0 {
        1.0
      } else {
        -1.0
      };
