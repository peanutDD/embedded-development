...
use nb::block;
use oscilloscope::{
  adc_to_voltage, ChannelConfig, DisplayBuffer, MultiChannelBuffer, OscilloscopeMeasurement,
  ...
  loop {
    ...
    if sample_count.wrapping_sub(last_status_time) >= 1000000 {
      ...
      writeln!(
        tx,
        "通道切换时间: {}us",
        performance.channel_switch_time * 5
      )
      .unwrap();
      writeln!(
        tx,
        "最大采集时间: {}us",
        performance.max_acquisition_time * 5
      )
      .unwrap();
