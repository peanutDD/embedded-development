...
impl StorageManager {
  ...
  fn store_envelope(&mut self, ch1_data: &[u16], ch2_data: &[u16]) -> [u16; SEGMENT_SIZE * 2] {
    let mut result = [0u16; SEGMENT_SIZE * 2];
    let len = ch1_data.len().min(ch2_data.len()).min(SEGMENT_SIZE);
    let decimation = if len > SEGMENT_SIZE {
      len / SEGMENT_SIZE
    } else {
      1
    };
  ...
  fn analyze_harmonics(&self, data: &[u16]) -> f32 {
    ...
    for i in 1..data.len() {
      ...
      if i > 2 {
        let second_diff =
          (data[i] as i32 - 2 * data[i - 1] as i32 + data[i - 2] as i32).abs() as f32;
  ...
  fn update_storage_statistics(&mut self) {
    ...
    self.storage_statistics.storage_usage =
      (self.waveform_count as f32 / MAX_WAVEFORMS as f32) * 100.0;
    ...
    // 计算存储效率
    self.storage_statistics.storage_efficiency = (self.storage_statistics.average_quality / 100.0)
      * (100.0 - self.storage_statistics.storage_usage)
      / 100.0
      * 100.0;
  }
...
fn main() -> ! {
  ...
  loop {
    ...
    // 定期输出状态信息
    if sample_count.wrapping_sub(last_status_time) >= 500000 {
      // 每2秒
      ...
      writeln!(
        tx,
        "已存储波形: {}/{}",
        storage_manager.waveform_count, MAX_WAVEFORMS
      )
      .unwrap();
      writeln!(
        tx,
        "存储使用率: {:.1}%",
        storage_manager.storage_statistics.storage_usage
      )
      .unwrap();
      writeln!(
        tx,
        "自动保存: {}",
        if storage_manager.auto_save {
          "开启"
        } else {
          "关闭"
        }
      ...
      writeln!(
        tx,
        "压缩模式: {}",
        if storage_manager.compression_enabled {
          "开启"
        } else {
          "关闭"
        }
      ...
      writeln!(
        tx,
        "当前段: {}/{}",
        storage_manager.current_segment, STORAGE_SEGMENTS
      )
      .unwrap();
      ...
      writeln!(
        tx,
        "总存储数: {}",
        storage_manager.storage_statistics.total_stored
      )
      .unwrap();
      writeln!(
        tx,
        "平均质量: {:.1}%",
        storage_manager.storage_statistics.average_quality
      )
      .unwrap();
      writeln!(
        tx,
        "压缩比: {:.1}:1",
        storage_manager.storage_statistics.compression_ratio
      )
      .unwrap();
      writeln!(
        tx,
        "存储效率: {:.1}%",
        storage_manager.storage_statistics.storage_efficiency
      )
      .unwrap();
      writeln!(
        tx,
        "数据完整性: {:.1}%",
        storage_manager.storage_statistics.data_integrity
      )
      .unwrap();
      writeln!(
        tx,
        "检索时间: {}us",
        storage_manager.storage_statistics.retrieval_time
      )
      .unwrap();
      ...
      if storage_manager.waveform_count > 0 {
        ...
        writeln!(
          tx,
          "峰峰值: {} ({}mV)",
          latest_waveform.metadata.peak_to_peak,
          (latest_waveform.metadata.peak_to_peak as u32 * VREF_MV as u32) / 4096
        )
        .unwrap();
        ...
        writeln!(
          tx,
          "质量评分: {:.1}%",
          latest_waveform.metadata.quality_score
        )
        .unwrap();
      }
