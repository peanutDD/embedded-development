  ...
  fn process_sample(&mut self, sample: u16, channel: u8, timestamp: u32) -> bool {
    ...
    match self.state {
      ...
      TriggerState::Holdoff => {
        // 保持期结束后重新武装
        if timestamp.wrapping_sub(self.last_trigger_time) > 10000 {
          // 10ms holdoff
          self.state = TriggerState::Armed;
        }
      }
      ...
      TriggerSlope::Rising => {
        prev_sample
          < self
            .condition
            .level
            .saturating_sub(self.condition.hysteresis)
          && sample
            > self
              .condition
              .level
              .saturating_add(self.condition.hysteresis)
      }
  ...
  fn update_trigger_statistics(&mut self, interval: u32) {
    ...
    if total_intervals > 0 {
      self.trigger_statistics.average_trigger_interval =
        (self.trigger_statistics.average_trigger_interval * (total_intervals - 1) + interval)
          / total_intervals;
    }

    // 计算触发效率
    self.trigger_statistics.trigger_efficiency = (self.trigger_statistics.total_triggers as f32)
      / ((self.trigger_statistics.total_triggers + self.false_trigger_count) as f32)
      * 100.0;
  }
...
impl TriggerScopePerformance {
  ...
  fn update(&mut self, trigger_system: &AdvancedTriggerSystem, sample_count: u32) {
    ...
    if sample_count > 0 {
      self.trigger_rate =
        (trigger_system.trigger_count as f32) / (sample_count as f32 / SAMPLE_RATE as f32);
    }
    ...
    if trigger_system.trigger_count > 0 {
      self.false_trigger_rate =
        (trigger_system.false_trigger_count as f32) / (trigger_system.trigger_count as f32) * 100.0;
    }
  ...
  fn get_performance_rating(&self) -> &'static str {
    let score = (self.capture_efficiency
      + (100.0 - self.false_trigger_rate)
      + (100.0 - self.processing_load.min(100.0)))
      / 3.0;
  ...
  loop {
    ...
    if sample_count.wrapping_sub(last_status_time) >= 1000000 {
      ...
      writeln!(
        ...
        (trigger_condition.level as u32 * VREF_MV as u32) / 4096
      )
      .unwrap();
      ...
      writeln!(
        tx,
        "滞回: {} ({}mV)",
        trigger_condition.hysteresis,
        (trigger_condition.hysteresis as u32 * VREF_MV as u32) / 4096
      )
      .unwrap();
      ...
      match trigger_condition.trigger_type {
        AdvancedTriggerType::Pulse => {
          writeln!(
            tx,
            "脉冲宽度范围: {}-{}us",
            trigger_condition.pulse_width_min, trigger_condition.pulse_width_max
          )
          .unwrap();
        }
        AdvancedTriggerType::Window => {
          writeln!(
            ...
            (trigger_condition.window_upper as u32 * VREF_MV as u32) / 4096
          )
          .unwrap();
        }
        AdvancedTriggerType::Pattern => {
      ...
      writeln!(
        tx,
        "预触发缓冲: {}/{}",
        trigger_system.pre_trigger_index, PRE_TRIGGER_SIZE
      )
      .unwrap();
      ...
      writeln!(
        tx,
        "总触发数: {}",
        trigger_system.trigger_statistics.total_triggers
      )
      .unwrap();
      writeln!(
        tx,
        "误触发数: {}",
        trigger_system.trigger_statistics.false_triggers
      )
      .unwrap();
      writeln!(
        tx,
        "触发效率: {:.1}%",
        trigger_system.trigger_statistics.trigger_efficiency
      )
      .unwrap();
      writeln!(
        tx,
        "平均间隔: {}ms",
        trigger_system.trigger_statistics.average_trigger_interval / (SAMPLE_RATE / 1000)
      )
      .unwrap();
      writeln!(
        tx,
        "最小间隔: {}ms",
        trigger_system.trigger_statistics.min_trigger_interval / (SAMPLE_RATE / 1000)
      )
      .unwrap();
      writeln!(
        tx,
        "最大间隔: {}ms",
        trigger_system.trigger_statistics.max_trigger_interval / (SAMPLE_RATE / 1000)
      )
      .unwrap();
      ...
      writeln!(
        tx,
        "采集速率: {:.0}Hz",
        trigger_performance.acquisition_rate
      )
      .unwrap();
      ...
      writeln!(
        tx,
        "误触发率: {:.2}%",
        trigger_performance.false_trigger_rate
      )
      .unwrap();
      writeln!(
        tx,
        "捕获效率: {:.1}%",
        trigger_performance.capture_efficiency
      )
      .unwrap();
      writeln!(
        tx,
        "性能评级: {}",
        trigger_performance.get_performance_rating()
      )
      .unwrap();
      ...
      let remaining_time =
        (trigger_switch_interval - (sample_count - trigger_type_switch_time)) / SAMPLE_RATE;
