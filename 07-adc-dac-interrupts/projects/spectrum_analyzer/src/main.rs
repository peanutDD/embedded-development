...
use stm32f4xx_hal::{
  adc::{
    config::{AdcConfig, Eoc, Resolution, SampleTime, Scan, Sequence},
    Adc,
  },
  dma::{config::DmaConfig, Channel0, MemoryToPeripheral, Stream0, StreamsTuple, Transfer},
  gpio::{Analog, Pin},
...
impl SpectrumAnalyzer {
    ...
    match window_type {
      ...
      WindowFunction::Blackman => {
        for i in 0..FFT_SIZE {
          ...
          let a1 = 0.5;
          let a2 = 0.08;
          self.window_function[i] = a0 - a1 * cos(2.0 * core::f32::consts::PI * n / (N - 1.0))
  ...
  fn compute_bandwidth(&mut self) {
    let peak_power =
      self.spectrum_statistics.peak_magnitude * self.spectrum_statistics.peak_magnitude;
    ...
    if spectrum_analyzer.process_samples() {
      ...
      if elapsed_seconds >= 1.0 {
        ...
        rtt_target::rprintln!(
          "帧数: {} | 帧率: {:.1} FPS",
          frame_counter,
          frame_counter as f32 / elapsed_seconds
        );
        rtt_target::rprintln!(
          "峰值频率: {:.1} Hz | 峰值幅度: {:.3}",
