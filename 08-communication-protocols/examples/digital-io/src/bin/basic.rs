...
use stm32f4xx_hal::{
    gpio::{
        ...
        Input, Output, PullUp, PushPull, Edge,
    },
...
impl ButtonState {
    ...
    pub fn any_pressed(&self) -> bool {
        self.button1_pressed || self.button2_pressed || 
        self.button3_pressed || self.button4_pressed
    }
    
    pub fn get_pressed_buttons(&self) -> Vec<u8, 4> {
        let mut pressed = Vec::new();
        
        if self.button1_pressed { pressed.push(1).ok(); }
        if self.button2_pressed { pressed.push(2).ok(); }
        if self.button3_pressed { pressed.push(3).ok(); }
        if self.button4_pressed { pressed.push(4).ok(); }
...
impl LedController {
        ...
        match self.pattern {
            ...
            LedPattern::Random => {
                if self.blink_counter % 300 == 0 {
                    ...
                    if random & 0x01 != 0 { self.led1.set_low(); } else { self.led1.set_high(); }
                    if random & 0x02 != 0 { self.led2.set_low(); } else { self.led2.set_high(); }
                    if random & 0x04 != 0 { self.led3.set_low(); } else { self.led3.set_high(); }
                }
            },
        }
    ...
    pub fn set_individual_led(&mut self, led: u8, state: bool) {
        match led {
            1 => if state { self.led1.set_low(); } else { self.led1.set_high(); },
            2 => if state { self.led2.set_low(); } else { self.led2.set_high(); },
            3 => if state { self.led3.set_low(); } else { self.led3.set_high(); },
            _ => {}
        }
    }
...
impl GpioManager {
    ...
    pub fn scan_inputs(&mut self) -> Vec<ButtonEvent, 8> {
        ...
        for (i, &state) in states.iter().enumerate() {
            ...
            // 检查长按
            if self.input_processor.is_long_press(i, 2000) { // 2秒长按
        ...
        for event in events {
            match event {
                ButtonEvent::Pressed(button) => {
                    // 更新全局按键状态
                    critical_section::with(|cs| {
                        BUTTON_STATE.borrow(cs).borrow_mut().update_button(button, true);
                    });
                    ...
                    match button {
                        ...
                        4 => {
                            ...
                            let count = critical_section::with(|cs| {
                                BUTTON_STATE.borrow(cs).borrow().press_count
                            });
                ...
                ButtonEvent::Released(button, duration) => {
                    critical_section::with(|cs| {
                        BUTTON_STATE.borrow(cs).borrow_mut().update_button(button, false);
                    });
