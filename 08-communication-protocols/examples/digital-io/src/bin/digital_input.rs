...
use digital_io::{
  DebounceConfig, DigitalInputController, EdgeDetection, EdgeHistory, InputConfig, PinMode,
  PinState,
};
...
impl PatternRecognizer {
  ...
  pub fn handle_timeout(&mut self, timestamp: u32) {
    ...
    self
      .key_sequence
      .retain(|key| timestamp.saturating_sub(key.release_time) < self.config.sequence_max_interval);
...
impl InputProcessor {
  ...
  pub fn init(&mut self) -> Result<(), ()> {
    ...
    for pin in 0..4 {
      ...
      self
        .timeout_monitor
        .configure_pin_timeout(pin, timeout_config)?;
    }
    ...
    if self.config.enable_pattern_recognition && edge_type == EdgeType::Falling {
      ...
      if let Ok(history) = self.digital_input.get_edge_history(pin) {
        ...
        if let Some(pattern) = self
          .pattern_recognizer
          .add_key_event(pin, press_time, timestamp)
        {
...
/// 全局输入处理器
static INPUT_PROCESSOR: Mutex<RefCell<Option<InputProcessor>>> = Mutex::new(RefCell::new(None));
