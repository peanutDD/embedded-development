...
impl Default for KeyMapping {
    fn default() -> Self {
        // 标准4x4键盘布局
        let position_map = [
            [KeyCode::Digit1, KeyCode::Digit2, KeyCode::Digit3, KeyCode::Up],
            [KeyCode::Digit4, KeyCode::Digit5, KeyCode::Digit6, KeyCode::Down],
            [KeyCode::Digit7, KeyCode::Digit8, KeyCode::Digit9, KeyCode::Left],
            [KeyCode::Star, KeyCode::Digit0, KeyCode::Hash, KeyCode::Right],
        ];
        ...
        // 导航键映射
        let _ = function_map.insert(KeyCode::Up, KeyboardFunction::Navigation(NavigationDirection::Up));
        let _ = function_map.insert(KeyCode::Down, KeyboardFunction::Navigation(NavigationDirection::Down));
        let _ = function_map.insert(KeyCode::Left, KeyboardFunction::Navigation(NavigationDirection::Left));
        let _ = function_map.insert(KeyCode::Right, KeyboardFunction::Navigation(NavigationDirection::Right));
        
        // 功能键映射
        let _ = function_map.insert(KeyCode::Star, KeyboardFunction::SystemControl(SystemCommand::Menu));
        let _ = function_map.insert(KeyCode::Hash, KeyboardFunction::SystemControl(SystemCommand::Enter));
...
impl ComboKeyHandler {
    ...
    /// 处理按键按下
    pub fn handle_key_press(&mut self, row: usize, col: usize, key_code: KeyCode) -> Option<KeyboardFunction> {
        // 添加到按下的键列表
...
impl FunctionHandler {
    ...
    fn handle_system_command(&mut self, command: SystemCommand) -> Result<(), ()> {
        match command {
            ...
            SystemCommand::VolumeUp | SystemCommand::VolumeDown |
            SystemCommand::BrightnessUp | SystemCommand::BrightnessDown => {
                // 系统控制命令
...
impl KeyboardController {
    ...
    fn handle_key_press(&mut self, row: usize, col: usize, timestamp: u32) -> Result<(), ()> {
        ...
        if let Some(combo_function) = self.combo_handler.handle_key_press(row, col, key_code) {
            self.function_handler.handle_function(combo_function, timestamp)?;
    ...
    /// 处理按键长按
    fn handle_key_hold(&mut self, row: usize, col: usize, duration: u32, timestamp: u32) -> Result<(), ()> {
        if row >= 4 || col >= 4 {
            return Err(());
        }
