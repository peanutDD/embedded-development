...
impl ApplicationController {
    ...
    fn demo_seven_segment(&mut self) {
        ...
        // 切换前导零显示
        self.seven_segment.set_leading_zeros((self.demo_counter / 1000) % 2 == 0);
    }
...
fn main() -> ! {
    ...
    // 创建移位寄存器控制器
    let mut shift_controller = ShiftRegisterController::new(
        data_pin, clock_pin, latch_pin, enable_pin, clear_pin, 4
    );
...
fn handle_button_press(button: usize, app: &mut ApplicationController) {
    match button {
        ...
        2 => {
            // 按键3：切换数码管前导零显示
            app.seven_segment.set_leading_zeros(!app.seven_segment.leading_zeros);
        },
        _ => {}
    }
}
