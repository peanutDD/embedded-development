...
use digital_io::{
  DebounceConfig, DigitalInputController, DigitalOutputController, DriveStrength, EdgeDetection,
  InputConfig, KeyEvent, MatrixKeypadController, OutputConfig, OutputMode, PinMode, PinState,
  ScanConfig, ShiftRegisterConfig, ShiftRegisterController, SlewRate,
};
...
impl SystemController {
  ...
  pub fn init(&mut self) -> Result<(), ()> {
    ...
    self
      .output_controller
      .set_output_mode(OutputMode::Individual);
  ...
  fn process_inputs(&mut self, timestamp: u32) -> Result<(), ()> {
    ...
    for (pin, &state) in hardware_inputs.iter().enumerate() {
      ...
      if let Ok(edge_detected) = self
        .input_controller
        .update_pin_state(pin, pin_state, timestamp)
      {
        if edge_detected {
  ...
  fn scan_keypad_rows(&self) -> [bool; 4] {
    ...
    // 模拟一些按键活动
    let time = self.system_state.system_time;
    [(time / 3000) % 2 == 0, false, false, false]
  }
  ...
  fn interactive_output_control(&mut self) -> Result<(), ()> {
    ...
    self
      .output_controller
      .set_pins(0xFFFF, output_pattern as u32)?;
  ...
  fn process_led_pattern(&mut self, timestamp: u32) -> Result<(), ()> {
    match self.system_state.led_pattern {
      ...
      LedPattern::Blink => {
        let pattern = if (timestamp / 500) % 2 == 0 {
          0xFF00
        } else {
          0x0000
        };
...
/// 全局系统控制器
static SYSTEM_CONTROLLER: Mutex<RefCell<Option<SystemController>>> = Mutex::new(RefCell::new(None));
