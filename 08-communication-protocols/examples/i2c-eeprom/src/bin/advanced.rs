...
impl EepromManager {
    pub fn new() -> Self {
        ...
        // 添加支持的设备
        devices.insert(EEPROM_24C02_ADDR, EepromDevice {
        ...
        devices.insert(EEPROM_24C08_ADDR, EepromDevice {
        ...
        devices.insert(EEPROM_24C32_ADDR, EepromDevice {
    ...
    pub fn write_record(&mut self, 
                       i2c: &mut I2cType, 
                       address: u16, 
                       record: &DataRecord) -> Result<(), EepromError> {
        let device = self.devices.get(&self.current_device)
    ...
    pub fn read_record(&mut self, 
                      i2c: &mut I2cType, 
                      address: u16) -> Result<DataRecord, EepromError> {
        let device = self.devices.get(&self.current_device)
            .ok_or(EepromError::DeviceNotFound)?;
        ...
        buffer.resize(total_length, 0).map_err(|_| EepromError::BufferTooSmall)?;
    ...
    fn serialize_record(&self, record: &DataRecord) -> Result<Vec<u8, 64>, EepromError> {
        ...
        // 写入记录头
        buffer.extend_from_slice(&record.id.to_le_bytes()).map_err(|_| EepromError::BufferTooSmall)?;
        buffer.extend_from_slice(&record.timestamp.to_le_bytes()).map_err(|_| EepromError::BufferTooSmall)?;
        buffer.push(record.data.len() as u8).map_err(|_| EepromError::BufferTooSmall)?;
        buffer.push(DATA_VERSION).map_err(|_| EepromError::BufferTooSmall)?;
        
        // 写入数据
        buffer.extend_from_slice(&record.data).map_err(|_| EepromError::BufferTooSmall)?;
        
        // 计算并写入校验和
        let checksum = CRC16.checksum(&buffer);
        buffer.extend_from_slice(&checksum.to_le_bytes()).map_err(|_| EepromError::BufferTooSmall)?;
    ...
    fn deserialize_record(&self, buffer: &[u8]) -> Result<DataRecord, EepromError> {
        ...
        data.extend_from_slice(&buffer[8..data_end]).map_err(|_| EepromError::BufferTooSmall)?;
    ...
    /// 带重试的数据写入
    fn write_data_with_retry(&mut self, 
                           i2c: &mut I2cType, 
                           device: &EepromDevice, 
                           address: u16, 
                           data: &[u8]) -> Result<(), EepromError> {
    ...
    fn write_data_paged(&self, 
        ...
        while offset < data.len() {
            ...
            write_buffer.extend_from_slice(&current_address.to_be_bytes()).map_err(|_| EepromError::BufferTooSmall)?;
            write_buffer.extend_from_slice(&data[offset..offset + bytes_to_write]).map_err(|_| EepromError::BufferTooSmall)?;
            
            i2c.write(device.address, &write_buffer).map_err(|_| EepromError::I2cError)?;
    ...
    /// 带重试的数据读取
    fn read_data_with_retry(&mut self, 
                          i2c: &mut I2cType, 
                          device: &EepromDevice, 
                          address: u16, 
                          buffer: &mut [u8]) -> Result<(), EepromError> {
        for attempt in 0..self.retry_count {
...
fn demonstrate_advanced_operations(
    ...
    for i in 0..5 {
        let mut data = Vec::new();
        data.extend_from_slice(&format!("Test data {}", i).as_bytes()[..10]).ok();
