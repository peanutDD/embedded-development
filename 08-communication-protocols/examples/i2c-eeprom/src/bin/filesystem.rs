...
impl FileAttributes {
    ...
    pub fn to_byte(&self) -> u8 {
        let mut byte = 0u8;
        if self.read_only { byte |= 0x01; }
        if self.hidden { byte |= 0x02; }
        if self.system { byte |= 0x04; }
        if self.directory { byte |= 0x08; }
        byte
    }
...
impl EepromFileSystem {
    ...
    pub fn create_file(&mut self, 
        ...
        filename_string.push_str(filename).map_err(|_| FsError::FilenameTooLong)?;
        ...
        // 添加到目录
        self.directory.push(dir_entry).map_err(|_| FsError::DirectoryFull)?;
    ...
    pub fn read_file(&mut self, 
        ...
        // 查找文件
        let dir_entry = self.find_file(filename)
    ...
    pub fn delete_file(&mut self, i2c: &mut I2cType, filename: &str) -> Result<(), FsError> {
        // 查找文件
        let file_index = self.directory.iter()
    ...
    pub fn get_fs_info(&self) -> FileSystemInfo {
        let free_sectors = self.fat.iter()
    ...
    fn read_fat(&mut self, i2c: &mut I2cType) -> Result<(), FsError> {
        ...
        for sector in 0..FAT_SECTORS {
            ...
            for i in 0..entries_per_sector {
                ...
                self.fat.push(FatEntry::from(value)).map_err(|_| FsError::InternalError)?;
            }
        }
    ...
    fn read_root_directory(&mut self, i2c: &mut I2cType) -> Result<(), FsError> {
        ...
        while offset < SECTOR_SIZE {
            ...
            match self.deserialize_directory_entry(&buffer[offset..]) {
                Ok((entry, size)) => {
                    self.directory.push(entry).map_err(|_| FsError::DirectoryFull)?;
                    offset += size;
                },
                Err(_) => break,
            }
    ...
    fn allocate_sectors(&mut self, count: usize) -> Result<Vec<u16, 32>, FsError> {
        ...
        // 查找空闲扇区
        for (i, &entry) in self.fat.iter().enumerate().skip(self.super_block.data_start_sector as usize) {
            if entry == FatEntry::Free {
                allocated.push(i as u16).map_err(|_| FsError::InternalError)?;
                found += 1;
    ...
    fn write_file_data(&self, i2c: &mut I2cType, sectors: &[u16], data: &[u8]) -> Result<(), FsError> {
        let mut offset = 0;
    ...
    fn read_file_data(&self, 
                     i2c: &mut I2cType, 
                     sectors: &[u16], 
                     buffer: &mut [u8], 
                     file_size: usize) -> Result<usize, FsError> {
        let mut offset = 0;
    ...
    fn write_sector(&self, i2c: &mut I2cType, sector: usize, data: &[u8]) -> Result<(), FsError> {
        ...
        while offset < data.len() {
            ...
            i2c.write(self.eeprom_addr, &write_buffer[..2 + page_size])
    ...
    fn read_sector(&self, i2c: &mut I2cType, sector: usize, buffer: &mut [u8]) -> Result<(), FsError> {
        let address = (sector * SECTOR_SIZE) as u16;
        let addr_bytes = address.to_be_bytes();
        
        i2c.write_read(self.eeprom_addr, &addr_bytes, buffer)
            .map_err(|_| FsError::I2cError)
    }
    
    fn find_file(&self, filename: &str) -> Option<&DirectoryEntry> {
        self.directory.iter().find(|entry| entry.filename == filename)
    }
    ...
    fn serialize_directory_entry(&self, entry: &DirectoryEntry) -> Vec<u8, 64> {
        ...
        // 起始扇区
        buffer.extend_from_slice(&entry.start_sector.to_le_bytes()).ok();
        
        // 属性
        buffer.push(entry.attributes.to_byte()).ok();
        
        // 时间戳
        buffer.extend_from_slice(&entry.created_time.to_le_bytes()).ok();
        buffer.extend_from_slice(&entry.modified_time.to_le_bytes()).ok();
    ...
    fn deserialize_directory_entry(&self, buffer: &[u8]) -> Result<(DirectoryEntry, usize), FsError> {
        ...
        if buffer.len() < 1 + filename_len + 15 { // 最小目录项大小
            return Err(FsError::CorruptedData);
        }
        ...
        filename.push_str(core::str::from_utf8(&buffer[offset..offset + filename_len])
