...
use stm32f4xx_hal::{
    ...
    pac,
    i2c::I2c,
    gpio::{Pin, Input, PullUp, Output, PushPull, Alternate},
    timer::{Timer, Event},
    interrupt,
};
use defmt_rtt as _;
...
use io_expander::{
    Pcf8574Manager, Pin as Pcf8574Pin, PinState,
    pin_mask, pin_groups,
};
...
fn main() -> ! {
    ...
    defmt::info!("发现 {} 个设备: {:?}", found_devices.len(), found_devices.as_slice());
    ...
    for config in DEVICE_CONFIGS {
        if found_devices.contains(&config.address) {
            match pcf_manager.init_device(config.address, config.input_pins, config.output_pins) {
                Ok(_) => {
                    defmt::info!("设备 0x{:02X} ({}) 初始化成功", config.address, config.description);
                }
                Err(e) => {
            ...
            defmt::warn!("设备 0x{:02X} ({}) 未找到", config.address, config.description);
        }
    }
                ...
                for config in DEVICE_CONFIGS {
                    match state.pcf_manager.check_interrupts(config.address) {
                        Ok(Some(changed_pins)) => {
                            defmt::info!("设备 0x{:02X} 引脚变化: 0b{:08b}", 
                                config.address, changed_pins);
                        }
                        Ok(None) => {}
                        Err(e) => {
                            defmt::error!("检查设备 0x{:02X} 中断失败: {:?}", 
                                config.address, e);
                        }
                    }
                }
...
fn process_buttons() {
    unsafe {
        if let Some(ref mut state) = SYSTEM_STATE {
            // 读取主控制器的按键状态
            if let Ok(button_data) = state.pcf_manager.pcf_mut().read_pins(0x20, pin_groups::BUTTONS) {
                // 处理4个按键
                for i in 0..4 {
                    ...
                    if state.button_states[i].update(raw_state) {
                        defmt::info!("按键 {} 被按下 (总计: {}次)", 
                            i, state.button_states[i].press_count);
...
fn TIM2() {
    unsafe {
        if let Some(ref mut state) = SYSTEM_STATE {
            ...
            // 切换状态LED
            if state.tick_counter % 50 == 0 { // 每5秒切换一次
            ...
            match state.led_pattern {
                ...
                LedPattern::Chase => {
                    ...
                    // 主控制器
                    if let Ok(_) = state.pcf_manager.pcf_mut().write_device(0x20, 
                        0xF0 | (!led_mask & 0x0F)) {
                    }
                ...
                LedPattern::Random => {
                    ...
                    if state.tick_counter % 5 == 0 {
                        ...
                        for config in DEVICE_CONFIGS {
                            ...
                            let _ = state.pcf_manager.pcf_mut().write_device(
                                config.address, 
                                pattern
                            );
                        }
                ...
                LedPattern::Binary => {
                    ...
                    // LED扩展板显示高4位
                    if let Ok(_) = state.pcf_manager.pcf_mut().write_device(0x21, 
                        !(count >> 4)) {
                    }
                }
                _ => {}
