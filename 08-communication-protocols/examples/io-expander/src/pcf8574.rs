    ...
    I2C: I2c<Error = E>,
{
    ...
    /// 设置多个引脚状态
    pub fn set_pins(&mut self, address: u8, mask: u8, state: PinState) -> Result<(), Pcf8574Error<E>> {
        let device = self.get_device_mut(address)?;
    ...
    I2C: I2c<Error = E>,
{
    ...
    /// 初始化设备
    pub fn init_device(&mut self, address: u8, input_pins: u8, output_pins: u8) -> Result<(), Pcf8574Error<E>> {
    ...
    pub fn check_interrupts(&mut self, address: u8) -> Result<Option<u8>, Pcf8574Error<E>> {
        ...
        // 查找对应的中断状态
        if let Some(device_index) = self.pcf.devices.iter().position(|dev| dev.address == address) {
