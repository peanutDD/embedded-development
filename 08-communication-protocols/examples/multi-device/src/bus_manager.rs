use crate::{
  i2c_addresses, BusType, CommunicationError, DeviceInfo, DeviceStatus, DeviceType,
  PerformanceStats, Priority,
};
  ...
  SPI: SpiDevice<Error = E2>,
{
    ...
    let result = if let Some(ref mut i2c) = self.i2c {
      match request.operation_type {
        OperationType::Read => {
          ...
          i2c
            .read(address, &mut buffer[..len])
            .map_err(|_| CommunicationError::BusError)
        }
        OperationType::Write => i2c
          .write(address, &request.data)
          .map_err(|_| CommunicationError::BusError),
        OperationType::WriteRead => {
          ...
          i2c
            .write_read(
              address,
              &request.data[..write_len],
              &mut read_buffer[..read_len],
            )
            .map_err(|_| CommunicationError::BusError)
        }
        OperationType::Scan => {
    ...
    let result = if let Some(ref mut spi) = self.spi {
      match request.operation_type {
        ...
        OperationType::WriteRead => {
          ...
          spi
            .transfer(&mut read_buffer, &request.data)
            .map_err(|_| CommunicationError::BusError)
        }
        OperationType::Scan => {
          // SPI设备扫描需要具体协议支持
  ...
  pub fn cleanup_offline_devices(&mut self) {
    self
      .devices
      .retain(|dev| dev.status != DeviceStatus::Offline);
  }

  /// 获取总线使用率
  pub fn calculate_bus_utilization(&mut self) -> (u8, u8) {
    // 简化的总线使用率计算
    let i2c_util = if self.i2c_locked {
      100
    } else {
      (self
        .request_queue
    ...
    let spi_util = if self.spi_locked {
      100
    } else {
      (self
        .request_queue
