...
use crate::protocol::{
    ProtocolMessage, ProtocolError, ProtocolStats, 
    I2cToSpiConverter, SpiToI2cConverter, OperationType
};
...
impl ProtocolBridge {
    ...
    pub fn add_request(
        ...
        if self.config.priority_scheduling {
            ...
            self.request_queue.push_back(request).map_err(|_| ProtocolError::BufferOverflow)?;
        }
    ...
    fn insert_request_by_priority(&mut self, request: BridgeRequest) -> Result<(), ProtocolError> {
        ...
        if request.priority > 5 {
            self.request_queue.push_front(request).map_err(|_| ProtocolError::BufferOverflow)
        } else {
            self.request_queue.push_back(request).map_err(|_| ProtocolError::BufferOverflow)
        }
    }
    ...
    pub fn process_requests(&mut self, current_time_ms: u32) -> Vec<BridgeResponse, 8> {
        ...
        for message in i2c_messages {
            if let Ok(request_id) = self.add_request(
        ...
        while let Some(request) = self.request_queue.pop_front() {
            ...
            // 更新统计
            self.stats.update_processing(processing_time_us, result.is_ok());
            if result.is_ok() {
                self.stats.protocol_stats.update_conversion(processing_time_us, true);
            } else {
                self.stats.protocol_stats.update_conversion(processing_time_us, false);
            }
    ...
    fn process_single_request(
        ...
        self.state = if result.is_ok() { BridgeState::Idle } else { BridgeState::Error };
        result
    }
    ...
    fn process_i2c_to_spi(
        ...
        // 转换SPI响应为I2C格式
        let i2c_response_data = self.i2c_to_spi_converter.convert_spi_to_i2c(&spi_response)?;
    ...
    pub fn get_response(&mut self, request_id: u32) -> Option<BridgeResponse> {
        // 查找并移除匹配的响应
        if let Some(pos) = self.response_queue.iter().position(|r| r.request_id == request_id) {
    ...
    pub fn cleanup_expired_responses(&mut self, current_time_ms: u32) {
        ...
        while let Some(response) = self.response_queue.pop_front() {
            ...
            if elapsed < self.config.timeout_ms * 2 { // 保留响应2倍超时时间
                let _ = temp_queue.push_back(response);
            }
        }
    ...
    pub fn set_enabled(&mut self, enabled: bool) {
        self.state = if enabled { BridgeState::Idle } else { BridgeState::Disabled };
    }
