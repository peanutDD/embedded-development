use stm32f4xx_hal::{
    gpio::{Pin, Alternate, OpenDrain},
    i2c::{I2c, Event, Error as I2cError},
    pac::I2C1,
    prelude::*,
};
use heapless::{Vec, pool::{Pool, Node}};
use crate::{BUFFER_SIZE, COMMAND_BUFFER_SIZE, I2C_TIMEOUT_MS, status};
...
impl I2cSlave {
    ...
    pub fn init(&mut self) -> Result<(), ProtocolError> {
        ...
        defmt::info!("I2C slave initialized at address 0x{:02X}", self.config.address);
    ...
    /// 处理I2C事件
    pub fn handle_event(&mut self, event: SlaveEvent, current_time_ms: u32) -> Result<Option<ProtocolMessage>, ProtocolError> {
        self.last_activity_ms = current_time_ms;
        
        match event {
            SlaveEvent::AddressMatched(is_read) => {
                self.handle_address_matched(is_read)
            },
            SlaveEvent::DataReceived(data) => {
                self.handle_data_received(data)
            },
            SlaveEvent::DataRequested => {
                self.handle_data_requested()
            },
            SlaveEvent::Stop => {
                self.handle_stop()
            },
            SlaveEvent::Error(error) => {
    ...
    /// 处理地址匹配事件
    fn handle_address_matched(&mut self, is_read: bool) -> Result<Option<ProtocolMessage>, ProtocolError> {
        match self.state {
            SlaveState::Idle => {
                if is_read {
...
impl I2cSlaveManager {
    ...
    pub fn add_slave(&mut self, slave: I2cSlave) -> Result<(), ProtocolError> {
        self.slaves.push(slave).map_err(|_| ProtocolError::BufferOverflow)
    }
    ...
    pub fn get_slave_mut(&mut self, address: u8) -> Option<&mut I2cSlave> {
        self.slaves.iter_mut().find(|slave| slave.config.address == address)
    }
