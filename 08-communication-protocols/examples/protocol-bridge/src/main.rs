...
use stm32f4xx_hal::{
    gpio::{Pin, Output, PushPull, Input, PullUp, Alternate, OpenDrain},
...
// 导入库模块
use protocol_bridge::{
    bridge::{ProtocolBridge, BridgeConfig, BridgeOperation, BridgeState},
    i2c_slave::{I2cSlaveManager, I2cSlave, SlaveConfig},
    spi_master::{SpiDeviceManager, SpiMaster, SpiDeviceConfig, SpiMode, BitOrder},
    protocol::{ProtocolMessage, OperationType},
};
...
impl LedController {
    ...
    fn update(&mut self, system_state: &SystemState, bridge_state: BridgeState, current_time_ms: u32) {
        // 状态LED闪烁
...
impl ApplicationController {
    ...
    fn run_device_scan(&mut self, current_time_ms: u32) {
        ...
        defmt::info!("Bridge stats - Total: {}, Success: {}, Failed: {}", 
    ...
    fn run_performance_test(&mut self, current_time_ms: u32) {
        ...
        for i in 0..3 {
            let mut test_message = ProtocolMessage::new(
                (i % 3) + 1, 
                if i % 2 == 0 { OperationType::Read } else { OperationType::Write }, 
                0x20 + i as u16
            );
            let _ = test_message.add_data(&[i as u8, (i + 1) as u8]);
            test_message.calculate_crc();

            let _ = self.bridge.add_request(
                BridgeOperation::I2cToSpi,
                test_message,
                current_time_ms,
            );
    ...
    fn run_error_recovery_test(&mut self, current_time_ms: u32) {
        ...
        if stats.failed_conversions > 0 || stats.timeout_errors > 0 {
            defmt::info!("Error recovery - Errors: {}, Timeouts: {}", 
                        stats.failed_conversions, stats.timeout_errors);
...
fn main() -> ! {
    ...
    loop {
        ...
        if stats_elapsed >= 10000 { // 每10秒
            let stats = app_controller.get_stats();
            let bridge_stats = app_controller.bridge.get_stats();
            
            defmt::info!("=== System Stats ===");
            defmt::info!("Uptime: {}s, Errors: {}", 
                        stats.uptime_ms / 1000, stats.error_count);
            defmt::info!("Bridge - Requests: {}, Success: {}, Failed: {}", 
