use heapless::Vec;
use crate::{BUFFER_SIZE, CRC_POLYNOMIAL, commands, status};
...
impl ProtocolMessage {
    ...
    pub fn add_data(&mut self, data: &[u8]) -> Result<(), ProtocolError> {
        for &byte in data {
            self.data.push(byte).map_err(|_| ProtocolError::BufferOverflow)?;
        }
        Ok(())
    }
    ...
    pub fn serialize(&self) -> Result<Vec<u8, BUFFER_SIZE>, ProtocolError> {
        ...
        // 添加头部信息
        buffer.push(self.device_id).map_err(|_| ProtocolError::BufferOverflow)?;
        buffer.push(self.operation as u8).map_err(|_| ProtocolError::BufferOverflow)?;
        buffer.push((self.address >> 8) as u8).map_err(|_| ProtocolError::BufferOverflow)?;
        buffer.push((self.address & 0xFF) as u8).map_err(|_| ProtocolError::BufferOverflow)?;
        buffer.push(self.data.len() as u8).map_err(|_| ProtocolError::BufferOverflow)?;
        
        // 添加数据
        for &byte in &self.data {
            buffer.push(byte).map_err(|_| ProtocolError::BufferOverflow)?;
        }
        
        // 添加CRC
        if let Some(crc) = self.crc {
            buffer.push((crc >> 8) as u8).map_err(|_| ProtocolError::BufferOverflow)?;
            buffer.push((crc & 0xFF) as u8).map_err(|_| ProtocolError::BufferOverflow)?;
        }
    ...
    pub fn deserialize(buffer: &[u8]) -> Result<Self, ProtocolError> {
        if buffer.len() < 7 {  // 最小长度：设备ID + 操作 + 地址(2) + 长度 + CRC(2)
        ...
        for i in 0..data_len {
            data.push(buffer[5 + i]).map_err(|_| ProtocolError::BufferOverflow)?;
        }
...
impl I2cToSpiConverter {
    ...
    pub fn convert_i2c_to_spi(&self, i2c_data: &[u8]) -> Result<ProtocolMessage, ProtocolError> {
        ...
        // 查找设备映射
        let spi_device_id = self.device_mappings
            .iter()
    ...
    /// 转换SPI响应到I2C格式
    pub fn convert_spi_to_i2c(&self, spi_message: &ProtocolMessage) -> Result<Vec<u8, BUFFER_SIZE>, ProtocolError> {
        let mut i2c_response = Vec::new();

        // 添加状态字节
        i2c_response.push(status::SUCCESS).map_err(|_| ProtocolError::BufferOverflow)?;

        // 添加数据
        for &byte in &spi_message.data {
            i2c_response.push(byte).map_err(|_| ProtocolError::BufferOverflow)?;
        }
...
impl SpiToI2cConverter {
    ...
    pub fn convert_spi_to_i2c(&self, spi_message: &ProtocolMessage) -> Result<Vec<u8, BUFFER_SIZE>, ProtocolError> {
        // 查找设备映射
        let i2c_addr = self.device_mappings
            .iter()
        ...
        // 添加I2C地址（包含读写位）
        let addr_with_rw = (i2c_addr << 1) | match spi_message.operation {
            OperationType::Read => 0x01,
            _ => 0x00,
        };
        i2c_data.push(addr_with_rw).map_err(|_| ProtocolError::BufferOverflow)?;

        // 添加寄存器地址
        i2c_data.push((spi_message.address & 0xFF) as u8).map_err(|_| ProtocolError::BufferOverflow)?;

        // 添加数据
        for &byte in &spi_message.data {
            i2c_data.push(byte).map_err(|_| ProtocolError::BufferOverflow)?;
        }
