use stm32f4xx_hal::{
    gpio::{Pin, Output, PushPull, Alternate},
    spi::{Spi, Mode, Phase, Polarity, Error as SpiError},
    pac::SPI1,
    prelude::*,
};
use heapless::Vec;
use crate::{BUFFER_SIZE, MAX_SPI_DEVICES, SPI_MAX_FREQUENCY, SPI_TIMEOUT_MS};
use crate::protocol::{ProtocolMessage, ProtocolError, OperationType};
...
impl From<SpiMode> for Mode {
    fn from(mode: SpiMode) -> Self {
        match mode {
            ...
            SpiMode::Mode1 => Mode { polarity: Polarity::IdleLow, phase: Phase::CaptureOnSecondTransition },
            SpiMode::Mode2 => Mode { polarity: Polarity::IdleHigh, phase: Phase::CaptureOnFirstTransition },
...
impl SpiMaster {
    ...
    pub fn add_device(
        ...
        self.devices.push(device).map_err(|_| ProtocolError::BufferOverflow)?;
        self.cs_pins.push(cs_pin).map_err(|_| ProtocolError::BufferOverflow)?;

        defmt::info!("Added SPI device {} with CS pin {}", config.device_id, config.cs_pin);
        Ok(())
    }

    /// 选择设备
    fn select_device(&mut self, device_id: u8) -> Result<usize, ProtocolError> {
        let device_index = self.devices
            .iter()
        ...
        if let Some(current_id) = self.current_device {
            if let Some(current_index) = self.devices.iter().position(|dev| dev.config.device_id == current_id) {
    ...
    fn configure_spi_for_device(&mut self, config: &SpiDeviceConfig) -> Result<(), ProtocolError> {
        ...
        // 由于HAL限制，这里只是示例代码
        defmt::debug!("Configuring SPI for device {}: freq={}, mode={:?}", 
                     config.device_id, config.frequency, config.mode);
        Ok(())
    }

    /// 执行SPI传输
    pub fn transfer(&mut self, message: &ProtocolMessage, current_time_ms: u32) -> Result<ProtocolMessage, ProtocolError> {
    ...
    /// 执行实际的SPI传输
    fn perform_transfer(&mut self, message: &ProtocolMessage, current_time_ms: u32) -> Result<ProtocolMessage, ProtocolError> {
        // 准备发送缓冲区
        ...
        match message.operation {
            OperationType::Read => {
                ...
                self.tx_buffer.push(0x80 | (message.address & 0x7F) as u8)
                    .map_err(|_| ProtocolError::BufferOverflow)?;
                
                // 添加虚拟字节用于读取数据
                let read_length = if message.data.is_empty() { 1 } else { message.data.len() };
                for _ in 0..read_length {
                    self.tx_buffer.push(0x00).map_err(|_| ProtocolError::BufferOverflow)?;
                }
            },
            OperationType::Write => {
                // 写命令格式: [WRITE_CMD | address] [data...]
                self.tx_buffer.push((message.address & 0x7F) as u8)
                    .map_err(|_| ProtocolError::BufferOverflow)?;
                
                for &byte in &message.data {
                    self.tx_buffer.push(byte).map_err(|_| ProtocolError::BufferOverflow)?;
                }
            },
            OperationType::Status => {
                // 状态查询命令
                self.tx_buffer.push(0xF0).map_err(|_| ProtocolError::BufferOverflow)?;
                self.tx_buffer.push(0x00).map_err(|_| ProtocolError::BufferOverflow)?;
            },
            OperationType::Reset => {
                // 复位命令
                self.tx_buffer.push(0xFF).map_err(|_| ProtocolError::BufferOverflow)?;
            }
        }

        // 准备接收缓冲区
        for _ in 0..self.tx_buffer.len() {
            self.rx_buffer.push(0x00).map_err(|_| ProtocolError::BufferOverflow)?;
        }

        // 执行SPI传输
        let start_time = current_time_ms;
        let transfer_result = self.spi.transfer(&mut self.rx_buffer[..], &self.tx_buffer[..]);
    ...
    pub fn scan_devices(&mut self, current_time_ms: u32) -> Vec<u8, MAX_SPI_DEVICES> {
        ...
        for device in &mut self.devices {
            // 尝试读取设备状态
            let status_message = ProtocolMessage::new(
    ...
    pub fn get_device(&self, device_id: u8) -> Option<&SpiDevice> {
        self.devices.iter().find(|dev| dev.config.device_id == device_id)
    }

    /// 获取设备信息（可变引用）
    pub fn get_device_mut(&mut self, device_id: u8) -> Option<&mut SpiDevice> {
        self.devices.iter_mut().find(|dev| dev.config.device_id == device_id)
    }
...
impl SpiDeviceManager {
    ...
    pub fn add_master(&mut self, master: SpiMaster) -> Result<(), ProtocolError> {
        self.masters.push(master).map_err(|_| ProtocolError::BufferOverflow)
    }
