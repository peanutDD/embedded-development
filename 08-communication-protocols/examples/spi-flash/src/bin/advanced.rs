...
impl FlashManager {
    ...
    pub fn init_device(&mut self, 
                      spi: &mut SpiType, 
                      cs: &mut CsPin, 
                      device_id: u8) -> Result<(), FlashError> {
        // 读取JEDEC ID
        ...
        self.devices.push(device).map_err(|_| FlashError::TooManyDevices)?;
    ...
    /// 高级读取操作（带重试和错误恢复）
    pub fn advanced_read(&mut self, 
                        spi: &mut SpiType, 
                        cs: &mut CsPin, 
                        address: u32, 
                        buffer: &mut [u8]) -> Result<usize, FlashError> {
    ...
    /// 高级写入操作（带磨损均衡）
    pub fn advanced_write(&mut self, 
                         spi: &mut SpiType, 
                         cs: &mut CsPin, 
                         address: u32, 
                         data: &[u8]) -> Result<(), FlashError> {
        let start_time = get_timestamp();
    ...
    /// 智能擦除操作
    pub fn smart_erase(&mut self, 
                      spi: &mut SpiType, 
                      cs: &mut CsPin, 
                      address: u32) -> Result<(), FlashError> {
    ...
    /// 数据完整性检查
    pub fn verify_data_integrity(&mut self, 
                                spi: &mut SpiType, 
                                cs: &mut CsPin, 
                                address: u32, 
                                expected_data: &[u8]) -> Result<bool, FlashError> {
    ...
    pub fn garbage_collect(&mut self, 
                          spi: &mut SpiType, 
                          cs: &mut CsPin) -> Result<u32, FlashError> {
    ...
    fn read_jedec_id(&self, spi: &mut SpiType, cs: &mut CsPin) -> Result<JedecId, FlashError> {
        ...
        cs.set_low();
        spi.write(&[CMD_JEDEC_ID]).map_err(|_| FlashError::SpiError)?;
        spi.transfer(&mut buffer).map_err(|_| FlashError::SpiError)?;
        cs.set_high();
    ...
    fn read_unique_id(&self, spi: &mut SpiType, cs: &mut CsPin) -> Result<[u8; 8], FlashError> {
        ...
        cs.set_low();
        spi.write(&[CMD_UNIQUE_ID, 0x00, 0x00, 0x00, 0x00]).map_err(|_| FlashError::SpiError)?;
        spi.transfer(&mut buffer).map_err(|_| FlashError::SpiError)?;
        cs.set_high();
    ...
    fn read_with_verification(&self, 
                             spi: &mut SpiType, 
                             cs: &mut CsPin, 
                             address: u32, 
                             buffer: &mut [u8]) -> Result<usize, FlashError> {
    ...
    fn write_with_verification(&self, 
                              spi: &mut SpiType, 
                              cs: &mut CsPin, 
                              address: u32, 
                              data: &[u8]) -> Result<(), FlashError> {
    ...
    fn write_enable(&self, spi: &mut SpiType, cs: &mut CsPin) -> Result<(), FlashError> {
        cs.set_low();
        spi.write(&[CMD_WRITE_ENABLE]).map_err(|_| FlashError::SpiError)?;
        cs.set_high();
    ...
    fn read_status(&self, spi: &mut SpiType, cs: &mut CsPin) -> Result<StatusRegister, FlashError> {
        ...
        cs.set_low();
        spi.write(&[CMD_READ_STATUS]).map_err(|_| FlashError::SpiError)?;
        spi.transfer(&mut buffer).map_err(|_| FlashError::SpiError)?;
        cs.set_high();
    ...
    fn erase_sector(&self, spi: &mut SpiType, cs: &mut CsPin, address: u32) -> Result<(), FlashError> {
        self.write_enable(spi, cs)?;
    ...
    fn erase_block_32k(&self, spi: &mut SpiType, cs: &mut CsPin, address: u32) -> Result<(), FlashError> {
        self.write_enable(spi, cs)?;
        
        let addr_bytes = address.to_be_bytes();
        
        cs.set_low();
        spi.write(&[CMD_BLOCK_ERASE_32K, addr_bytes[1], addr_bytes[2], addr_bytes[3]])
            .map_err(|_| FlashError::SpiError)?;
        cs.set_high();
    ...
    fn erase_block_64k(&self, spi: &mut SpiType, cs: &mut CsPin, address: u32) -> Result<(), FlashError> {
        self.write_enable(spi, cs)?;
        
        let addr_bytes = address.to_be_bytes();
        
        cs.set_low();
        spi.write(&[CMD_BLOCK_ERASE_64K, addr_bytes[1], addr_bytes[2], addr_bytes[3]])
            .map_err(|_| FlashError::SpiError)?;
        cs.set_high();
    ...
    fn init_wear_leveling(&mut self, device_id: u8) -> Result<(), FlashError> {
        ...
        for sector in 0..total_sectors as u16 {
            ...
            self.wear_leveling.insert(sector, info).map_err(|_| FlashError::InternalError)?;
        }
    ...
    fn verify_erase(&self, 
                   spi: &mut SpiType, 
                   cs: &mut CsPin, 
                   address: u32, 
                   size: usize) -> Result<(), FlashError> {
    ...
    fn recover_from_error(&mut self, 
                         spi: &mut SpiType, 
                         cs: &mut CsPin, 
                         error: &FlashError) -> Result<(), FlashError> {
        match error {
    ...
    fn calculate_average_erase_count(&self) -> u32 {
        ...
        let total: u32 = self.wear_leveling.values()
    ...
    fn get_max_erase_count(&self) -> u32 {
        self.wear_leveling.values()
            .map(|info| info.erase_count)
            .max()
            .unwrap_or(0)
    }
    
    fn calculate_health_score(&self, 
                             bad_block_percentage: usize, 
                             average_erase_count: u32, 
                             max_erase_count: u32) -> u8 {
        let mut score = 100u8;
    ...
    fn update_average_read_time(&mut self, elapsed: u32) {
        let total_time = self.performance_stats.average_read_time_us * 
                        (self.performance_stats.read_operations - 1);
    ...
    fn update_average_write_time(&mut self, elapsed: u32) {
        let total_time = self.performance_stats.average_write_time_us * 
                        (self.performance_stats.write_operations - 1);
    ...
    fn extract_valid_data(&self, 
                         spi: &mut SpiType, 
                         cs: &mut CsPin, 
                         sector: u16) -> Result<Vec<u8, 4096>, FlashError> {
