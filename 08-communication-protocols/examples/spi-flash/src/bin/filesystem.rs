...
impl LogStructuredFS {
  ...
  pub fn create_file(
    ...
    // 检查文件是否已存在
    if self
      .find_file_in_directory(spi, cs, parent_inode, filename)?
      .is_some()
    {
  ...
  pub fn list_directory(
    ...
    cs: &mut CsPin,
    inode_num: u32,
  ) -> Result<Vec<DirectoryEntry, 32>, FsError> {
    let inode = self.get_inode(spi, cs, inode_num)?;
  ...
  pub fn garbage_collect(&mut self, spi: &mut SpiType, cs: &mut CsPin) -> Result<u32, FsError> {
    ...
    // 检查是否需要垃圾回收
    let usage_ratio =
      1.0 - (self.super_block.free_blocks as f32 / self.super_block.total_blocks as f32);
  ...
  pub fn get_fs_info(&self) -> FileSystemInfo {
    FileSystemInfo {
      ...
      used_size: (self.super_block.total_blocks - self.super_block.free_blocks) as usize
        * SECTOR_SIZE,
  ...
  fn read_block_cached(
    ...
    cs: &mut CsPin,
    block_num: u32,
  ) -> Result<Vec<u8, SECTOR_SIZE>, FsError> {
    // 检查缓存
  ...
  fn flush_write_buffer(
    ...
    while let Some(entry) = self.write_buffer.pop_front() {
      ...
      inode
        .blocks
        .push(block_num)
        .map_err(|_| FsError::TooManyBlocks)?;
    }
  ...
  fn find_blocks_for_gc(
    &self,
    _spi: &mut SpiType,
    _cs: &mut CsPin,
  ) -> Result<Vec<u32, 16>, FsError> {
    // 简化实现
    Ok(Vec::new())
  }
