  ...
  pub fn update(&mut self, data: &SensorData) {
    ...
    let accel_mag =
      (data.accel_x * data.accel_x + data.accel_y * data.accel_y + data.accel_z * data.accel_z)
        .sqrt();
    let gyro_mag =
      (data.gyro_x * data.gyro_x + data.gyro_y * data.gyro_y + data.gyro_z * data.gyro_z).sqrt();
...
impl DataLogger {
  ...
  pub fn log_with_data(&mut self, level: LogLevel, message: &str, data: u32) {
    ...
    if self.log_entries.push(entry).is_err() {
      ...
      let _ = self
        .log_entries
        .push(LogEntry::with_data(level, message, data));
    }
  }
