  ...
  CS: OutputPin,
{
  ...
  pub fn write_sensor_data(&mut self, data: &[SensorData]) -> Result<(), FlashError> {
    ...
    for sensor_data in data {
      ...
      while offset < data_bytes.len() {
        ...
        self.write_page(page_start, &data_bytes[offset..offset + bytes_to_write])?;
  ...
  pub fn read_sensor_data(
    ...
    start_record: u32,
    count: u32,
  ) -> Result<Vec<SensorData, 32>, FlashError> {
    ...
    for i in 0..actual_count {
      ...
      if sensor_data.verify_checksum() {
        result
          .push(sensor_data)
          .map_err(|_| FlashError::BufferFull)?;
      }
    }
