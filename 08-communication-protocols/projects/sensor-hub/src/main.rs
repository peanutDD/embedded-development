...
use stm32f4xx_hal::{
  ...
  prelude::*,
  rcc::Clocks,
  spi::{Mode as SpiMode, NoMiso, Phase, Polarity, Spi},
};
...
// 系统启动序列
fn startup_sequence<LED, DELAY>(led: &mut LED, delay: &mut DELAY)
where
...
// 显示系统状态
fn display_system_status<LED>(status_led: &mut LED, sample_count: u32, error_count: u32)
where
  LED: embedded_hal::digital::v2::OutputPin,
{
  if error_count > 0 {
...
fn system_recovery<DELAY>(
  _sensor_manager: &mut SensorManager<
    I2c<
      pac::I2C1,
      (
        stm32f4xx_hal::gpio::gpiob::PB8<
          stm32f4xx_hal::gpio::Alternate<4, stm32f4xx_hal::gpio::OpenDrain>,
        >,
...
// LED闪烁控制
fn led_blink<LED, DELAY>(led: &mut LED, delay: &mut DELAY, duration_ms: u32)
where
  LED: embedded_hal::digital::v2::OutputPin,
  DELAY: embedded_hal::blocking::delay::DelayMs<u32>,
{
