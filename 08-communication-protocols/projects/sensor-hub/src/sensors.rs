    ...
    I2C: WriteRead<Error = E> + Write<Error = E>,
{
    ...
    fn init_bmp280(&mut self) -> Result<(), E> {
        ...
        self.i2c.write_read(BMP280_ADDR, &[BMP280_REG_ID], &mut id_buffer)?;
    ...
    fn read_bmp280(&mut self) -> Result<(f32, f32), E> {
        // 读取原始数据
        let mut data = [0u8; 6];
        self.i2c.write_read(BMP280_ADDR, &[BMP280_REG_PRESS_MSB], &mut data)?;
    ...
    fn init_mpu6050(&mut self) -> Result<(), E> {
        // 读取设备ID
        let mut id_buffer = [0u8; 1];
        self.i2c.write_read(MPU6050_ADDR, &[MPU6050_REG_WHO_AM_I], &mut id_buffer)?;
        ...
        // 唤醒设备
        self.i2c.write(MPU6050_ADDR, &[MPU6050_REG_PWR_MGMT_1, 0x00])?;
        
        // 配置陀螺仪 (±250°/s)
        self.i2c.write(MPU6050_ADDR, &[MPU6050_REG_GYRO_CONFIG, 0x00])?;
        
        // 配置加速度计 (±2g)
        self.i2c.write(MPU6050_ADDR, &[MPU6050_REG_ACCEL_CONFIG, 0x00])?;
    ...
    fn read_mpu6050(&mut self) -> Result<([f32; 3], [f32; 3]), E> {
        // 读取加速度计和陀螺仪数据 (14字节)
        let mut data = [0u8; 14];
        self.i2c.write_read(MPU6050_ADDR, &[MPU6050_REG_ACCEL_XOUT_H], &mut data)?;
