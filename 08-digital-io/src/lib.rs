...
impl<const N: usize> DigitalInputController<N> {
    ...
    /// 更新引脚状态（由硬件层调用）
    pub fn update_pin_state(&mut self, pin: usize, new_state: PinState, timestamp: u32) -> Result<bool, ()> {
        if pin >= N {
            return Err(());
        }
...
impl<const ROWS: usize, const COLS: usize> MatrixKeypadController<ROWS, COLS> {
                ...
                for col in 0..COLS {
                    ...
                    if key_pressed != old_state {
                        ...
                        if hold_time >= self.scan_config.hold_threshold_ms as u32 {
                            let _ = events.push(KeyEvent::Hold { row, col, duration: hold_time });
                        }
                    }
                }
            }
    ...
    /// 处理按键防抖
    fn process_key_debounce(&mut self, row: usize, col: usize, new_state: bool, timestamp: u32) -> bool {
...
pub enum KeyEvent {
    ...
    Release { row: usize, col: usize },
    Hold { row: usize, col: usize, duration: u32 },
    Ghost,
}
