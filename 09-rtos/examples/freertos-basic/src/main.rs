...
use freertos_rust::{
    Task, Queue, Mutex, BinarySemaphore, CountingSemaphore,
    Duration, TaskPriority, CurrentTask,
    FreeRtosError, TaskHandle,
};
...
impl LedController {
    fn new(
        ...
        Self { green, orange, red, blue }
    }
    
    fn set_led(&mut self, led_id: u8, state: bool) {
        match led_id {
            0 => if state { self.green.set_high() } else { self.green.set_low() },
            1 => if state { self.orange.set_high() } else { self.orange.set_low() },
            2 => if state { self.red.set_high() } else { self.red.set_low() },
            3 => if state { self.blue.set_high() } else { self.blue.set_low() },
            _ => {},
        }
    }
                    ...
                    if let Ok(_guard) = led_mutex.lock(Duration::ms(10)) {
                        unsafe {
                            if let Some(ref mut controller) = LED_CONTROLLER {
                                ...
                                rprintln!("[LED] LED {} 设置为 {}", led_id, if state { "开" } else { "关" });
                            }
                        }
                    }
                },
...
fn button_monitor_task(message_queue: Queue<TaskMessage>, button_semaphore: BinarySemaphore) {
    ...
    loop {
        let current_state = unsafe {
            BUTTON_PIN.as_ref().map_or(false, |pin| pin.is_low())
        };
        
        if current_state != last_state {
            debounce_counter += 1;
            if debounce_counter >= 5 { // 50ms消抖（假设10ms周期）
                ...
                if message_queue.send(message, Duration::ms(10)).is_ok() {
                    rprintln!("[BTN] 按键 {} - 时间戳: {}", 
                        if current_state { "按下" } else { "释放" }, timestamp);
...
fn system_monitor_task(message_queue: Queue<TaskMessage>, system_semaphore: CountingSemaphore) {
    ...
    loop {
        ...
        // 每秒更新系统状态
        if tick_counter % 100 == 0 { // 假设10ms周期
            unsafe {
        ...
        if let Ok(message) = message_queue.receive(Duration::ms(50)) {
            ...
            match message {
                TaskMessage::ButtonEvent { pressed, timestamp } => {
                    rprintln!("[MSG] 处理按键事件: {} @ {}", 
                        if pressed { "按下" } else { "释放" }, timestamp);
                ...
                TaskMessage::SystemStatus { uptime, free_heap } => {
                    if uptime % 10 == 0 { // 每10秒报告一次
                        rprintln!("[MSG] 系统状态: 运行时间 {}s, 可用堆 {} 字节", 
                            uptime, free_heap);
                    }
                },
...
fn idle_monitor_task() {
    ...
    loop {
        ...
        // 每分钟报告一次空闲状态
        if idle_counter % 6000 == 0 { // 假设10ms周期
...
// FreeRTOS钩子函数
#[no_mangle]
extern "C" fn vApplicationStackOverflowHook(task_handle: *mut freertos_sys::TaskHandle_t, task_name: *const i8) {
