...
use stm32f4xx_hal::{
  ...
  delay::Delay,
  gpio::{Alternate, Input, Output, Pin, PullUp, PushPull, AF4},
  i2c::I2c,
  prelude::*,
  ...
  I2C: embedded_hal::blocking::i2c::Write + embedded_hal::blocking::i2c::WriteRead,
{
  ...
  fn write_register(&mut self, reg: u8, value: u8) -> Result<(), u8> {
    self.i2c.write(self.address, &[reg, value]).map_err(|_| 2)
  }
  ...
  I2C: embedded_hal::blocking::i2c::Write + embedded_hal::blocking::i2c::WriteRead,
{
  ...
  fn read(&mut self) -> Result<Vec<SensorReading, 8>, Self::Error> {
    ...
    self
      .i2c
      .write_read(self.address, &[0xFA], &mut temp_buffer)
      .map_err(|_| 5)?;
    ...
    self
      .i2c
      .write_read(self.address, &[0xF7], &mut press_buffer)
      .map_err(|_| 6)?;
...
impl SensorFusion {
  ...
  pub fn add_reading(&mut self, mut reading: SensorReading) -> Result<(), &'static str> {
    ...
    if let Some(filter) = self.filters.get_mut(&filter_key) {
      ...
      self
        .filters
        .insert(filter_key, filter)
        .map_err(|_| "Filter map full")?;
    }
...
impl MultiSensorSystem {
  ...
  pub fn initialize_all_sensors(&mut self) -> Result<(), &'static str> {
    for sensor in &mut self.sensors {
      sensor
        .initialize()
        .map_err(|_| "Sensor initialization failed")?;
    }
    Ok(())
  }
...
impl DataLogger {
  ...
  pub fn export_recent_data(&self, count: usize) -> Result<Vec<u8, 1024>, &'static str> {
    ...
    for i in 0..actual_count {
      ...
      export_data
        .push(&self.log_buffer[index])
        .map_err(|_| "Export buffer full")?;
    }
    ...
    for entry in export_data {
      ...
      result
        .push(entry.sensor_id)
        .map_err(|_| "Result buffer full")?;
      result
        .push(entry.sensor_type as u8)
        .map_err(|_| "Result buffer full")?;
...
impl AlertManager {
  ...
  pub fn set_threshold(
    ...
    self
      .thresholds
      .insert(sensor_type, threshold)
      .map_err(|_| "Threshold map full")?;
    Ok(())
  }
...
fn format_statistics(
  ...
  write!(
    output,
    "STATS,{},MEAN={:.2},STD={:.2},MIN={:.2},MAX={:.2},COUNT={},TIME={}\r\n",
