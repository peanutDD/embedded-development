    ...
    PIN: embedded_hal::digital::v2::OutputPin + embedded_hal::digital::v2::InputPin,
{
    ...
    fn reset(&mut self) -> Result<bool, DS18B20Error> {
        // 发送复位脉冲
        self.pin.set_low().map_err(|_| DS18B20Error::CommunicationError)?;
        self.delay.delay_us(480u32);
        
        // 释放总线并等待存在脉冲
        self.pin.set_high().map_err(|_| DS18B20Error::CommunicationError)?;
        self.delay.delay_us(70u32);
        
        // 检查存在脉冲
        let presence = self.pin.is_low().map_err(|_| DS18B20Error::CommunicationError)?;
    ...
    fn read_bit(&mut self) -> Result<bool, DS18B20Error> {
        ...
        let bit = self.pin.is_high().map_err(|_| DS18B20Error::CommunicationError)?;
    ...
    I2C: embedded_hal::blocking::i2c::Write + embedded_hal::blocking::i2c::WriteRead,
{
    ...
    fn read_raw_temperature(&mut self) -> Result<i32, BMP280Error> {
        ...
        self.i2c.write_read(self.address, &[0xFA], &mut buffer)
            .map_err(|_| BMP280Error::I2cError)?;
    ...
    I2C: embedded_hal::blocking::i2c::Write + embedded_hal::blocking::i2c::WriteRead,
{
    ...
    fn is_available(&mut self) -> bool {
        self.read_register(0xD0).map(|id| id == Self::CHIP_ID).unwrap_or(false)
    }
}
...
impl TemperatureFilter {
    ...
    fn get_variance(&self) -> f32 {
        ...
        let sum_sq_diff: f32 = self.history.iter()
            .map(|&x| (x - avg) * (x - avg))
            .sum();
...
impl TemperatureMonitor {
    ...
    fn add_sensor(&mut self, sensor: Box<dyn TemperatureSensor<Error = DS18B20Error>>) -> Result<(), &'static str> {
        if self.sensors.is_full() {
            return Err("Too many sensors");
        }
        
        self.sensors.push(sensor).map_err(|_| "Failed to add sensor")?;
        self.filters.push(TemperatureFilter::new(0.1)).map_err(|_| "Failed to add filter")?;
    ...
    fn get_statistics(&self) -> TemperatureStatistics {
        ...
        if stats.count > 0 {
            ...
            stats.std_deviation = ((stats.sum_squares / stats.count as f32) - 
                                  (stats.average * stats.average)).sqrt();
        }
...
// 格式化温度数据
fn format_temperature_data(buffer: &mut [u8], sensor_name: &str, temperature: f32, timestamp: u32) -> Result<usize, ()> {
    use heapless::String;
    ...
    use core::fmt::Write;
    write!(output, "{},{:.2},{}\r\n", sensor_name, temperature, timestamp).map_err(|_| ())?;
...
// 格式化统计信息
fn format_statistics(buffer: &mut [u8], readings: &[&TemperatureReading], timestamp: u32) -> Result<usize, ()> {
    use heapless::String;
    ...
    use core::fmt::Write;
    write!(output, "STATS,AVG={:.2},COUNT={},TIME={}\r\n", average, count, timestamp).map_err(|_| ())?;
