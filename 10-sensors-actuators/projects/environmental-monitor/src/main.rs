    ...
    + embedded_hal::blocking::i2c::WriteRead<Error = E>,
{
  ...
  fn compensate_temperature(&self, adc_t: i32) -> (f32, i32) {
    let var1 = (((adc_t >> 3) - ((self.calibration_data.dig_t1 as i32) << 1))
      * (self.calibration_data.dig_t2 as i32))
      >> 11;
    let var2 = (((((adc_t >> 4) - (self.calibration_data.dig_t1 as i32))
      * ((adc_t >> 4) - (self.calibration_data.dig_t1 as i32)))
      >> 12)
      * (self.calibration_data.dig_t3 as i32))
      >> 14;
...
impl DataProcessor {
  ...
  fn apply_low_pass_filter(&mut self, data: &EnvironmentalData) {
    ...
    self.filtered_data.temperature =
      alpha * data.temperature + (1.0 - alpha) * self.filtered_data.temperature;
    self.filtered_data.humidity =
      alpha * data.humidity + (1.0 - alpha) * self.filtered_data.humidity;
    self.filtered_data.pressure =
      alpha * data.pressure + (1.0 - alpha) * self.filtered_data.pressure;
    self.filtered_data.light_intensity =
      alpha * data.light_intensity + (1.0 - alpha) * self.filtered_data.light_intensity;

    // 对于整数值使用简单平均
    self.filtered_data.co2_level = ((alpha * data.co2_level as f32)
      + ((1.0 - alpha) * self.filtered_data.co2_level as f32))
      as u16;
    self.filtered_data.pm25 =
      ((alpha * data.pm25 as f32) + ((1.0 - alpha) * self.filtered_data.pm25 as f32)) as u16;
    self.filtered_data.pm10 =
      ((alpha * data.pm10 as f32) + ((1.0 - alpha) * self.filtered_data.pm10 as f32)) as u16;
  }
...
{
  pub fn new(i2c1: I2C1, i2c2: I2C2, spi: SPI, sampling_interval: u32) -> Self {
    Self {
