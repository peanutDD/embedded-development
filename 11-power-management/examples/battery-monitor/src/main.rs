...
impl BatteryMonitorSystem {
  ...
  fn calculate_derived_values(&mut self) {
    // 计算功率
    self.monitor_state.power =
      ((self.monitor_state.voltage as i32 * self.monitor_state.current as i32) / 1000) as i16;
  ...
  fn assess_voltage_health(&self) -> BatteryHealth {
    ...
    if voltage_variance < 50 {
      BatteryHealth::Excellent
    } else if voltage_variance < 100 {
      BatteryHealth::Good
    } else if voltage_variance < 200 {
      BatteryHealth::Fair
    } else if voltage_variance < 300 {
      BatteryHealth::Poor
    } else {
  ...
  fn assess_capacity_health(&self) -> BatteryHealth {
    // 基于实际容量与标称容量的比较
    let capacity_ratio =
      (self.monitor_state.remaining_capacity * 100) / self.battery_config.capacity;

    if capacity_ratio > 90 {
      BatteryHealth::Excellent
    } else if capacity_ratio > 70 {
      BatteryHealth::Good
    } else if capacity_ratio > 50 {
      BatteryHealth::Fair
    } else if capacity_ratio > 30 {
      BatteryHealth::Poor
    } else {
      BatteryHealth::Critical
    }
  ...
  fn assess_resistance_health(&self) -> BatteryHealth {
    ...
    let resistance_increase = if estimated_resistance > self.battery_config.internal_resistance {
      ((estimated_resistance - self.battery_config.internal_resistance) * 100)
        / self.battery_config.internal_resistance
    } else {
      0
    };

    if resistance_increase < 20 {
      BatteryHealth::Excellent
    } else if resistance_increase < 50 {
      BatteryHealth::Good
    } else if resistance_increase < 100 {
      BatteryHealth::Fair
    } else if resistance_increase < 200 {
      BatteryHealth::Poor
    } else {
      BatteryHealth::Critical
    }
  ...
  fn calculate_voltage_variance(&self) -> u16 {
    ...
    let variance: u32 = self
      .voltage_buffer
      .iter()
      .map(|&x| {
        let diff = if x as u32 > avg {
          x as u32 - avg
        } else {
          avg - x as u32
        };
      ...
      .sum::<u32>()
      / self.voltage_buffer.len() as u32;
  ...
  fn estimate_internal_resistance(&self) -> u16 {
    ...
    // 简化的内阻估算：基于电压和电流的变化
    let voltage_change =
      self.voltage_buffer.iter().max().unwrap() - self.voltage_buffer.iter().min().unwrap();
    let current_change =
      self.current_buffer.iter().max().unwrap() - self.current_buffer.iter().min().unwrap();
  ...
  fn update_statistics(&mut self) {
    ...
    if self.monitor_state.current > 0 {
      // 放电
      self.statistics.total_charge_out += (self.monitor_state.current as u32) / 3600;
    // mAs to mAh
  ...
  fn send_status_report(&mut self) {
    ...
    report
      .push_str(&format!("Voltage: {}V\r\n", voltage_str))
      .ok();
    report
      .push_str(&format!("Current: {}A\r\n", current_str))
      .ok();
    report.push_str(&format!("Power: {}W\r\n", power_str)).ok();
    report
      .push_str(&format!("SOC: {}%\r\n", self.monitor_state.state_of_charge))
      .ok();
    report
      .push_str(&format!(
        "Remaining: {}mAh\r\n",
    ...
    // 统计数据
    report
      .push_str(&format!("Uptime: {}s\r\n", self.statistics.uptime))
      .ok();
    report
      .push_str(&format!(
        "Charge In: {}mAh\r\n",
        self.statistics.total_charge_in
      ))
      .ok();
    report
      .push_str(&format!(
        "Charge Out: {}mAh\r\n",
        self.statistics.total_charge_out
      ))
      .ok();
