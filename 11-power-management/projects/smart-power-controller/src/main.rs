...
use stm32f4xx_hal::{
    prelude::*,
    stm32,
    gpio::{gpioa::*, gpiob::*, gpioc::*, Output, PushPull, Input, PullUp, Analog},
...
use heapless::{Vec, String, FnvIndexMap, pool::{Pool, Node}};
...
pub struct SmartPowerController {
    // 硬件接口
    adc: Adc<stm32::ADC1>,
    i2c: I2c<stm32::I2C1, (PB8<stm32f4xx_hal::gpio::Alternate<4>>, PB9<stm32f4xx_hal::gpio::Alternate<4>>)>,
...
impl SmartPowerController {
    pub fn new(
        ...
        Self {
            ...
            power_manager: PowerManager {
                ...
                efficiency_curve: [
                    (10, 70), (20, 80), (30, 85), (40, 88), (50, 90),
                    (60, 91), (70, 90), (80, 88), (90, 85), (100, 80)
                ],
            },
            load_balancer: LoadBalancer {
            ...
            energy_optimizer: EnergyOptimizer {
                ...
                power_scheduling: PowerScheduling {
                    enabled: false,
                    time_slots: [TimeSlot { hour: 0, power_limit: 100000, priority_boost: [false; 4] }; 24],
                    current_slot: 0,
                },
                sleep_management: SleepManagement {
    ...
    fn read_all_sensors(&mut self) {
        ...
        for i in 0..4 {
            ...
            self.power_channels[i].power = 
                (self.power_channels[i].voltage as u32 * self.power_channels[i].current as u32 / 1000) as u16;
        }
    ...
    fn power_based_balancing(&mut self) {
        let total_power: u32 = self.power_channels.iter()
            .map(|ch| ch.power as u32)
            .sum();
        ...
        // 检查是否需要重新平衡
        let max_load = self.load_balancer.load_distribution.iter()
    ...
    fn apply_efficiency_control(&mut self, channel: usize, control_output: f32) {
        ...
        match channel_data.control_mode {
            ControlMode::PWM => {
                let new_duty = (channel_data.pwm_duty as f32 + control_output).max(0.0).min(100.0) as u8;
    ...
    fn send_status_report(&mut self) {
        ...
        report.push_str(&format!("System State: {}\r\n", state_str)).ok();
        
        // 总功率
        let total_power: u32 = self.power_channels.iter().map(|ch| ch.power as u32).sum();
        report.push_str(&format!("Total Power: {}W\r\n", total_power / 1000)).ok();
    ...
    // 其他辅助方法的简化实现
    fn system_self_test(&mut self) -> Result<(), &'static str> { Ok(()) }
    fn initialize_power_manager(&mut self) -> Result<(), &'static str> { Ok(()) }
    fn initialize_load_balancer(&mut self) -> Result<(), &'static str> { Ok(()) }
    fn initialize_energy_optimizer(&mut self) -> Result<(), &'static str> { Ok(()) }
    fn initialize_data_logger(&mut self) -> Result<(), &'static str> { Ok(()) }
