...
impl GatewayRequest {
  pub fn new(
    tcp_transaction_id: u16,
    tcp_connection_id: u16,
    rtu_slave_id: u8,
    function_code: u8,
  ) -> Self {
    Self {
...
impl ModbusGateway {
  ...
  /// 提取TCP帧
  fn extract_tcp_frame(
    &self,
    buffer: &mut Vec<u8, GATEWAY_BUFFER_SIZE>,
  ) -> Result<Option<ModbusTcpFrame>, &'static str> {
    if buffer.len() < 8 {
  ...
  fn send_tcp_exception(
    ...
    if let Some(connection) = self.get_tcp_connection_mut(connection_id) {
      ...
      connection
        .tx_buffer
        .push((tcp_frame.transaction_id >> 8) as u8)
        .ok();
      connection
        .tx_buffer
        .push((tcp_frame.transaction_id & 0xFF) as u8)
        .ok();
      ...
      connection
        .tx_buffer
        .push(tcp_frame.function_code | 0x80)
        .ok();
  ...
  fn check_rtu_response(&mut self) -> Result<Option<ModbusRtuFrame>, &'static str> {
    ...
    if self.rtu_rx_buffer.len() >= 4 {
      ...
      let received_crc =
        (self.rtu_rx_buffer[data_len + 1] as u16) << 8 | (self.rtu_rx_buffer[data_len] as u16);
  ...
  /// 转换RTU响应为TCP响应
  fn convert_rtu_to_tcp_response(
    &mut self,
    request: &mut GatewayRequest,
    rtu_response: ModbusRtuFrame,
  ) -> Result<(), &'static str> {
    // 检查从站ID和功能码
  ...
  fn send_tcp_response(&mut self, request: &GatewayRequest) -> Result<(), &'static str> {
    if let Some(connection) = self.get_tcp_connection_mut(request.tcp_connection_id) {
      ...
      connection
        .tx_buffer
        .push((request.tcp_transaction_id >> 8) as u8)
        .ok();
      connection
        .tx_buffer
        .push((request.tcp_transaction_id & 0xFF) as u8)
        .ok();
      connection.tx_buffer.push(0x00).ok(); // 协议ID高字节
      connection.tx_buffer.push(0x00).ok(); // 协议ID低字节
  ...
  pub fn get_statistics(&self) -> GatewayStatistics {
    GatewayStatistics {
      ...
      active_connections: self
        .tcp_connections
        .iter()
        .filter(|conn| conn.state == TcpConnectionState::Connected)
        .count() as u8,
