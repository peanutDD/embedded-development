  ...
  TOUCH_IRQ: InputPin,
{
  ...
  fn initialize_data_points(&mut self) -> Result<(), &'static str> {
    ...
    for (tag, value, unit) in data_points.iter() {
      ...
      self
        .data_points
        .push(data_point)
        .map_err(|_| "Too many data points")?;
    }
  ...
  fn read_touch(&mut self) -> Result<Option<TouchEvent>, &'static str> {
    ...
    if self
      .touch_irq
      .is_low()
      .map_err(|_| "Failed to read touch IRQ")?
    {
      // 读取触摸坐标
  ...
  fn update_trend_data(&mut self, tag: &str, value: f32) -> Result<(), &'static str> {
    for trend in &mut self.trends {
      if trend.tag.as_str() == tag {
        ...
        trend
          .timestamps
          .push(self.system_time)
          .map_err(|_| "Trend timestamps full")?;
  ...
  fn draw_main_page(&mut self) -> Result<(), &'static str> {
    ...
    for data_point in &self.data_points {
      ...
      display_text
        .push_str(&data_point.unit)
        .map_err(|_| "Text too long")?;

      let color = if data_point.quality {
        Rgb565::GREEN
      } else {
        Rgb565::RED
      };
  ...
  fn draw_trend_chart(
    ...
    if trend.values.len() > 1 {
      ...
      for i in 1..trend.values.len() {
        ...
        let y2 =
          position.y + size.height as i32 - ((trend.values[i] - trend.min_value) * y_scale) as i32;
  ...
  fn draw_alarm_status(&mut self) -> Result<(), &'static str> {
    ...
    if active_alarms > 0 {
      ...
      alarm_text
        .push_str("Active Alarms: ")
        .map_err(|_| "Text too long")?;

      // 简化的数字转换
      if active_alarms < 10 {
        alarm_text
          .push((b'0' + active_alarms as u8) as char)
          .map_err(|_| "Text too long")?;
      } else {
...
fn format_float(value: f32) -> String<16> {
  ...
  result.push('.').ok();
  result
    .push((b'0' + fractional_part.abs() as u8) as char)
    .ok();
