...
impl AesEncryptor {
  ...
  fn encrypt_ecb(&self, plaintext: &[u8]) -> Result<EncryptionResult, &'static str> {
    ...
    for chunk in padded_data.chunks(AES_BLOCK_SIZE) {
      ...
      ciphertext
        .extend_from_slice(&encrypted_block)
        .map_err(|_| "Ciphertext too large")?;
    }
  ...
  fn decrypt_cbc(
    ...
    ciphertext: &[u8],
    iv: [u8; AES_BLOCK_SIZE],
  ) -> Result<Vec<u8, MAX_DATA_SIZE>, &'static str> {
    if ciphertext.len() % AES_BLOCK_SIZE != 0 {
      return Err("Invalid ciphertext length");
    }
  ...
  fn add_pkcs7_padding(&self, data: &[u8]) -> Result<Vec<u8, MAX_DATA_SIZE>, &'static str> {
    ...
    padded
      ...
      .map_err(|_| "Data too large")?;

    let padding_len = AES_BLOCK_SIZE - (data.len() % AES_BLOCK_SIZE);

    for _ in 0..padding_len {
      padded
        .push(padding_len as u8)
        .map_err(|_| "Padding failed")?;
    }
...
impl AesEncryptionSystem {
  ...
  fn run_encryption_tests(&mut self) {
    ...
    match self
      .test_suite
      .benchmark_encryption(b"Performance test data", 10)
    {
      Ok(cycles) => {
...
fn main() -> ! {
  ...
  // 创建系统
  let mut system = AesEncryptionSystem::new(test_suite, led_pin, button_pin, serial);
