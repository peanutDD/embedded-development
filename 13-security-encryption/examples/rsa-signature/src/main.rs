...
impl SimpleRsa {
  ...
  pub fn generate_keypair(&mut self, rng: &mut dyn RngCore) -> Result<(), &'static str> {
    ...
    for _ in 0..128 {
      ...
      for byte in bytes {
        private_key
          .push(byte)
          .map_err(|_| "Private key too large")?;
      }
    }
  ...
  fn generate_signature(
    ...
    hash: &[u8; 32],
    private_key: &[u8],
  ) -> Result<Vec<u8, MAX_SIGNATURE_SIZE>, &'static str> {
...
impl CertificateValidator {
  ...
  pub fn add_trusted_key(&mut self, public_key: Vec<u8, 512>) -> Result<(), &'static str> {
    self
      .trusted_keys
      .push(public_key)
      .map_err(|_| "Too many trusted keys")
  }
...
impl RsaSignatureTestSuite {
  ...
  pub fn test_certificate_validation(&mut self) -> Result<bool, &'static str> {
    ...
    for i in 0..128 {
      root_cert
        .push(i as u8)
        .map_err(|_| "Certificate too large")?;
      intermediate_cert
        .push((i + 1) as u8)
        .map_err(|_| "Certificate too large")?;
      leaf_cert
...
fn main() -> ! {
  ...
  // 创建系统
  let mut system = RsaSignatureSystem::new(test_suite, led_pin, button_pin, serial);
