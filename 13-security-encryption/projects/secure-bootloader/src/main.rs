...
impl FirmwareDecryptor {
    ...
    /// 解密固件数据
    pub fn decrypt(&self, encrypted_data: &[u8], iv: &[u8; AES_BLOCK_SIZE]) -> Result<Vec<u8, MAX_FIRMWARE_SIZE>, &'static str> {
        ...
        for chunk in encrypted_data.chunks(AES_BLOCK_SIZE) {
            ...
            decrypted.extend_from_slice(&decrypted_block).map_err(|_| "Decrypted data too large")?;
...
impl FlashManager {
    ...
    /// 读取固件数据
    pub fn read_firmware_data(&self, address: u32, size: u32) -> Result<Vec<u8, MAX_FIRMWARE_SIZE>, &'static str> {
...
impl SecureBootloader {
    ...
    pub fn firmware_update(&mut self, new_firmware: &[u8]) -> Result<(), &'static str> {
        ...
        // 解析固件头部
        let header = unsafe {
            *(new_firmware.as_ptr() as *const FirmwareHeader)
        };
...
fn main() -> ! {
    ...
    // 创建系统
    let mut system = SecureBootSystem::new(
        bootloader,
        led_pin,
        button_pin,
    );
