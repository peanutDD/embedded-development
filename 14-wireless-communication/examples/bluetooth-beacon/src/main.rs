  ...
  UART: embedded_hal::serial::Read<u8> + embedded_hal::serial::Write<u8>,
{
  ...
  pub fn set_beacon_data(&mut self, config: &BeaconConfig) -> Result<(), &'static str> {
    ...
    command
      .push_str("AT+BLEADVDATA=\"")
      .map_err(|_| "Command too long")?;

    // 转换为十六进制字符串
    for &byte in &beacon_data {
      command
        .push_str(&format_hex_byte(byte))
        .map_err(|_| "Command too long")?;
    }
  ...
  fn build_ibeacon_data(&self, config: &BeaconConfig) -> Result<Vec<u8, 64>, &'static str> {
    ...
    // Major (2字节，大端序)
    data
      .push((config.major >> 8) as u8)
      .map_err(|_| "Data too long")?;
    data
      .push((config.major & 0xFF) as u8)
      .map_err(|_| "Data too long")?;

    // Minor (2字节，大端序)
    data
      .push((config.minor >> 8) as u8)
      .map_err(|_| "Data too long")?;
    data
      .push((config.minor & 0xFF) as u8)
      .map_err(|_| "Data too long")?;

    // TX Power (1字节，有符号)
    data
      .push(config.tx_power as u8)
      .map_err(|_| "Data too long")?;
  ...
  pub fn start_beacon(&mut self, config: &BeaconConfig) -> Result<(), &'static str> {
    ...
    adv_params
      .push_str("AT+BLEADVPARAM=")
      .map_err(|_| "Command too long")?;
    adv_params
      .push_str(&format_number(config.advertising_interval as u32))
      .map_err(|_| "Command too long")?;
    ...
    adv_params
      .push_str(",0,0,7,0")
      .map_err(|_| "Command too long")?; // 其他参数
  ...
  BT: BluetoothModule,
{
  ...
  fn add_or_update_device(&mut self, mut device: ScannedDevice) {
    ...
    if let Some(ref mut beacon_data) = device.beacon_data {
      beacon_data.distance =
        DistanceCalculator::calculate_distance(device.rssi, beacon_data.tx_power);
    }
  ...
  fn cleanup_old_devices(&mut self) {
    ...
    self
      .scanned_devices
      .retain(|device| current_time - device.last_seen < timeout);
  }
