...
impl Default for SensorConfig {
    fn default() -> Self {
        ...
        config.server_url.push_str("http://api.example.com/sensors").ok();
    ...
    UART: embedded_hal::serial::Read<u8> + embedded_hal::serial::Write<u8>,
{
        ...
        if response.contains("OK") {
            ...
            if let Some(start) = ip_response.find("STAIP,\"") {
                ...
                if let Some(end) = ip_response[ip_start..].find('"') {
                    ...
                    self.ip_address.push_str(ip_str).map_err(|_| "IP too long")?;
                }
            }
    ...
    pub fn connect_tcp(&mut self, host: &str, port: u16) -> Result<u8, &'static str> {
        let mut command = String::<128>::new();
        command.push_str("AT+CIPSTART=0,\"TCP\",\"").map_err(|_| "Command too long")?;
    ...
    pub fn send_tcp_data(&mut self, conn_id: u8, data: &[u8]) -> Result<(), &'static str> {
        ...
        command.push((b'0' + conn_id) as char).map_err(|_| "Command too long")?;
        command.push(',').map_err(|_| "Command too long")?;
        
        let data_len = format_number(data.len() as u32);
        command.push_str(&data_len).map_err(|_| "Command too long")?;
    ...
    pub fn close_tcp(&mut self, conn_id: u8) -> Result<(), &'static str> {
        ...
        command.push_str("AT+CIPCLOSE=").map_err(|_| "Command too long")?;
        command.push((b'0' + conn_id) as char).map_err(|_| "Command too long")?;
    ...
    WIFI: WiFiModule,
{
    ...
    pub fn get(&mut self, url: &str) -> Result<String<512>, &'static str> {
        ...
        request.push_str(&host).map_err(|_| "Request too long")?;
        request.push_str("\r\n").map_err(|_| "Request too long")?;
        request.push_str("User-Agent: ").map_err(|_| "Request too long")?;
        request.push_str(&self.user_agent).map_err(|_| "Request too long")?;
        request.push_str("\r\n").map_err(|_| "Request too long")?;
    ...
    // 发送POST请求
    pub fn post(&mut self, url: &str, data: &str, content_type: &str) -> Result<String<512>, &'static str> {
        // 解析URL
        let (host, path) = self.parse_url(url)?;
        ...
        request.push_str(&self.user_agent).map_err(|_| "Request too long")?;
        request.push_str("\r\n").map_err(|_| "Request too long")?;
        request.push_str("Content-Type: ").map_err(|_| "Request too long")?;
        request.push_str(content_type).map_err(|_| "Request too long")?;
        request.push_str("\r\n").map_err(|_| "Request too long")?;
        request.push_str("Content-Length: ").map_err(|_| "Request too long")?;
        
        let data_len = format_number(data.len() as u32);
        request.push_str(&data_len).map_err(|_| "Request too long")?;
        request.push_str("\r\n").map_err(|_| "Request too long")?;
        request.push_str("Connection: close\r\n").map_err(|_| "Request too long")?;
        request.push_str("\r\n").map_err(|_| "Request too long")?;
    ...
    fn parse_url(&self, url: &str) -> Result<(String<64>, String<128>), &'static str> {
        ...
        if let Some(slash_pos) = url_without_protocol.find('/') {
            ...
            host.push_str(url_without_protocol).map_err(|_| "Host too long")?;
    ...
    SENSORS: SensorManager,
{
    ...
    pub fn initialize(&mut self) -> Result<(), &'static str> {
        ...
        // 连接WiFi网络
        self.wifi.connect_wifi(&self.config.wifi_ssid, &self.config.wifi_password)?;
    ...
    fn build_json_payload(&self, data: &[SensorData]) -> Result<String<1024>, &'static str> {
        ...
        json.push_str("{\"device_id\":\"").map_err(|_| "JSON too long")?;
        json.push_str(&self.config.device_id).map_err(|_| "JSON too long")?;
        json.push_str("\",\"data\":[").map_err(|_| "JSON too long")?;
        ...
        for (i, sensor_data) in data.iter().enumerate() {
            ...
            json.push('{').map_err(|_| "JSON too long")?;
            json.push_str("\"timestamp\":").map_err(|_| "JSON too long")?;
            json.push_str(&format_number(sensor_data.timestamp)).map_err(|_| "JSON too long")?;
            json.push_str(",\"temperature\":").map_err(|_| "JSON too long")?;
            json.push_str(&format_float(sensor_data.temperature)).map_err(|_| "JSON too long")?;
            json.push_str(",\"humidity\":").map_err(|_| "JSON too long")?;
            json.push_str(&format_float(sensor_data.humidity)).map_err(|_| "JSON too long")?;
            json.push_str(",\"pressure\":").map_err(|_| "JSON too long")?;
            json.push_str(&format_float(sensor_data.pressure)).map_err(|_| "JSON too long")?;
            json.push_str(",\"light\":").map_err(|_| "JSON too long")?;
            json.push_str(&format_number(sensor_data.light as u32)).map_err(|_| "JSON too long")?;
            json.push_str(",\"battery\":").map_err(|_| "JSON too long")?;
            json.push_str(&format_float(sensor_data.battery_voltage)).map_err(|_| "JSON too long")?;
            json.push('}').map_err(|_| "JSON too long")?;
        }
