...
use stm32f4xx_hal::{
    ...
    serial::{Serial, Config as SerialConfig},
...
impl LoRaController {
    ...
    pub fn get_status(&self) -> (bool, u32, u8, u32) {
        (self.enabled, self.frequency, self.spreading_factor, self.packet_count)
    }
}
...
impl DeviceManager {
    ...
    pub fn get_online_devices(&self, current_time: u32, timeout_ms: u32) -> Vec<&DeviceInfo, MAX_DEVICES> {
...
impl MessageRouter {
    ...
    pub fn route_message(&mut self, message: GatewayMessage) -> Result<(), &'static str> {
        // 查找目标设备的协议
        let target_protocol = self.routing_table.get(&message.target_id)
            .copied()
    ...
    fn convert_protocol(&self, mut message: GatewayMessage, target_protocol: Protocol) -> Result<GatewayMessage, &'static str> {
        // 协议转换逻辑
...
impl IoTGateway {
    ...
    pub fn run_cycle(&mut self) {
        ...
        // 发送心跳消息
        if self.system_time % 30000 == 0 { // 每30秒
            self.send_heartbeat();
        }
        
        // 清理离线设备
        if self.system_time % 60000 == 0 { // 每60秒
            self.device_manager.cleanup_offline_devices(self.system_time, 120000); // 2分钟超时
        }
