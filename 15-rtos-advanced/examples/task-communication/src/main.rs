...
mod app {
    ...
    fn sensor_data_processor(ctx: sensor_data_processor::Context, msg: Message) {
        ...
        if sensor_reading.value > 1000 {
            let error_data = [0xFF, sensor_reading.sensor_id, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00];
    ...
    fn error_handler(mut ctx: error_handler::Context, msg: Message) {
        ...
        cortex_m_log::println!(
            ...
            error_code, error_source, msg.timestamp
        );
    ...
    fn data_analyzer(ctx: data_analyzer::Context) {
        if !ctx.local.sensor_data_buffer.is_empty() {
            let avg_value: u32 = ctx.local.sensor_data_buffer
                .iter()
                .map(|reading| reading.value as u32)
                .sum::<u32>() / ctx.local.sensor_data_buffer.len() as u32;
    ...
    fn communication_test(_ctx: communication_test::Context) {
        // 发送测试消息到各个队列
        let test_messages = [
            Message::new(MessageType::NetworkPacket, 2, [0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08]),
            Message::new(MessageType::SystemEvent, 1, [0xBB, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]),
            Message::new(MessageType::SensorData, 2, [0xFF, 0x03, 0xE8, 0x01, 0x00, 0x00, 0x00, 0x00]),
        ];
