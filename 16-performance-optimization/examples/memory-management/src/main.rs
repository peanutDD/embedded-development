  ...
  fn calculate_fragmentation(&mut self, free_blocks: u32, largest_free_block: u32) {
    if free_blocks > 0 && largest_free_block > 0 {
      self.fragmentation_ratio =
        1.0 - (largest_free_block as f32 / self.current_bytes_allocated as f32);
    }
  }
}
...
fn test_static_allocator() {
  ...
  cortex_m_log::println!(
    "Allocator stats: {} allocations, {} bytes",
...
fn test_cache_optimization() {
  ...
  cortex_m_log::println!(
    ...
    align_of::<CacheAlignedBuffer<u32, 256>>()
  );
}
...
fn run_memory_benchmarks() {
  ...
  cortex_m_log::println!(
    "Static allocation: {:.2} cycles/op, {:.0} ops/sec",
  ...
  benchmark.benchmark_allocation(1000, || pool_manager.allocate(64).is_some());

  cortex_m_log::println!(
    "Pool allocation: {:.2} cycles/op, {:.0} ops/sec",
    benchmark.get_cycles_per_operation(),
    benchmark.get_operations_per_second(168_000_000)
  );
}
