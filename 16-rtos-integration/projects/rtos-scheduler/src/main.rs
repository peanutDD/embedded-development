...
mod app {
    ...
    fn scheduler_tick(mut ctx: scheduler_tick::Context) {
        ...
        let next_task = match algorithm {
            SchedulingAlgorithm::PriorityBased => {
                priority_based_scheduler(&mut ctx)
            }
            SchedulingAlgorithm::RoundRobin => {
                round_robin_scheduler(&mut ctx)
            }
            SchedulingAlgorithm::EarliestDeadlineFirst => {
                edf_scheduler(&mut ctx)
            }
            SchedulingAlgorithm::RateMonotonic => {
                rate_monotonic_scheduler(&mut ctx)
            }
            SchedulingAlgorithm::Preemptive => {
                preemptive_scheduler(&mut ctx)
            }
            SchedulingAlgorithm::Cooperative => {
                cooperative_scheduler(&mut ctx)
            }
        };
        ...
        ctx.shared.task_queue.lock(|queue| {
            if let Some(task) = queue.iter_mut().find(|t| t.task_id == task_id) {
                ...
                cortex_m_log::println!(
                    ...
                    task.task_id, task.priority, task.deadline, task.remaining_time
                );
            }
        });
    ...
    fn performance_monitor(mut ctx: performance_monitor::Context) {
        (ctx.shared.scheduler_stats, ctx.shared.performance_metrics, ctx.shared.task_queue).lock(|stats, metrics, queue| {
            // 计算系统负载
            ...
            for task in queue.iter() {
                cortex_m_log::println!(
                    ...
                    task.task_id, task.state, task.priority, task.cpu_usage
                );
            }
        });
    ...
    fn deadlock_detector(mut ctx: deadlock_detector::Context) {
        ...
        if blocked_tasks > 2 {
            cortex_m_log::println!("WARNING: Potential deadlock detected - {} blocked tasks", blocked_tasks);
