...
pub mod sensor {
    ...
    impl TemperatureSensor {
        ...
        pub fn is_temperature_stable(&self, threshold: f32) -> bool {
            ...
            recent_readings.iter().all(|&reading| {
                (reading - avg).abs() < threshold
            })
        }
...
pub mod protocol {
    ...
    impl Message {
        pub fn new(msg_type: MessageType, sequence: u8, payload: &[u8]) -> Result<Self, &'static str> {
            ...
            for &byte in payload {
                msg_payload.push(byte).map_err(|_| "Failed to add payload")?;
            }
    ...
    impl ProtocolHandler {
        ...
        pub fn create_data_message(&mut self, payload: &[u8]) -> Result<Message, &'static str> {
            ...
            self.pending_acks.push(self.next_sequence)
...
fn run_basic_tests() {
    ...
    led_sm.handle_event(state_machine::LedEvent::TurnOn).unwrap();
...
mod tests {
    ...
    fn test_led_state_machine() {
        ...
        sm.handle_event(state_machine::LedEvent::StartBlinking).unwrap();
