...
pub mod system_integration {
      ...
      + embedded_hal::blocking::i2c::WriteRead,
  {
    ...
    pub fn test_device_scan(&mut self) -> Result<Vec<u8, 128>, TestError> {
      ...
      for addr in 0x08..0x78 {
        if self.i2c.write(addr, &[]).is_ok() {
          found_devices
            .push(addr)
            .map_err(|_| TestError::BufferFull)?;
        }
      }
    ...
    pub fn test_register_access(&mut self) -> Result<(), TestError> {
      ...
      self
        .i2c
        .write(self.device_address, &[register, test_value])
      ...
      self
        .i2c
        .write_read(self.device_address, &[register], &mut buffer)
        .map_err(|_| TestError::I2cError)?;
    ...
    pub fn test_multi_byte_transfer(&mut self) -> Result<(), TestError> {
      ...
      self
        .i2c
        .write_read(self.device_address, &[register], &mut read_buffer)
        .map_err(|_| TestError::I2cError)?;
...
pub mod communication_integration {
  ...
  impl TestMessage {
    ...
    pub fn serialize_json(&self) -> Result<String<256>, &'static str> {
      ...
      let serialized =
        serde_json_core::to_slice(self, &mut buffer).map_err(|_| "Serialization failed")?;

      let json_str = core::str::from_utf8(serialized).map_err(|_| "Invalid UTF-8")?;
    ...
    pub fn serialize_postcard(&self) -> Result<Vec<u8, 64>, &'static str> {
      ...
      let serialized =
        postcard::to_slice(self, &mut buffer).map_err(|_| "Postcard serialization failed")?;
    ...
    pub fn deserialize_postcard(data: &[u8]) -> Result<Self, &'static str> {
      postcard::from_bytes(data).map_err(|_| "Postcard deserialization failed")
    }
  }
  ...
  impl ProtocolStackTest {
    ...
    pub fn test_throughput(
      &mut self,
      message_count: usize,
    ) -> Result<u32, system_integration::TestError> {
      let start_cycles = cortex_m::peripheral::DWT::cycle_count();

      for i in 0..message_count.min(16) {
        let data = format!("Message {}", i);
        let msg = TestMessage::new(self.sequence_number, start_cycles, data.as_bytes())
          .map_err(|_| system_integration::TestError::ConfigurationError)?;
...
pub mod system_level_tests {
  ...
  impl SystemTestSuite {
    ...
    pub fn run_all_tests(
      &mut self,
      delay: &mut stm32f4xx_hal::delay::Delay,
    ) -> Result<(), TestError> {
      ...
      // 运行协议测试
      self.run_test("Message Creation", || {
        self.protocol_test.test_message_creation()
      });
      self.run_test("JSON Serialization", || {
        self.protocol_test.test_json_serialization()
      });
      self.run_test("Postcard Serialization", || {
        self.protocol_test.test_postcard_serialization()
      });
      self.run_test("Message Integrity", || {
        self.protocol_test.test_message_integrity()
      });
      ...
      F: FnOnce() -> Result<(), TestError>,
    {
      ...
      let result = match test_fn() {
        Ok(()) => TestResult {
          ...
          passed: true,
          execution_time_cycles: cortex_m::peripheral::DWT::cycle_count()
    ...
    pub fn generate_report(&self) -> TestReport {
      ...
      let total_execution_time: u32 = self
        .test_results
        .iter()
...
fn main() -> ! {
  ...
  // 创建GPIO集成测试
  let gpio_test = system_integration::GpioIntegrationTest::new(input_pin, output_pin, led_pin);
