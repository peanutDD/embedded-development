...
impl RecoveryManager {
  ...
  fn set_default_strategies(&mut self) {
    ...
    self
      .strategies
      .insert(CrossPlatformError::GpioError, RecoveryStrategy::Retry(3))
      .ok();
    ...
    self
      .strategies
      .insert(CrossPlatformError::SensorError, RecoveryStrategy::Reset)
      .ok();

    // 超时错误 - 重试2次
    self
      .strategies
      .insert(CrossPlatformError::TimeoutError, RecoveryStrategy::Retry(2))
      .ok();

    // 硬件错误 - 停止执行
    self
      .strategies
      .insert(CrossPlatformError::HardwareError, RecoveryStrategy::Halt)
      .ok();

    // 内存错误 - 停止执行
    self
      .strategies
      .insert(CrossPlatformError::MemoryError, RecoveryStrategy::Halt)
      .ok();
    ...
    self
      .strategies
      .insert(
        CrossPlatformError::InvalidParameter,
