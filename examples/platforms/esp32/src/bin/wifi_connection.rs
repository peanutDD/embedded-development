...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("HTTP服务器已启动，访问: http://{}", wifi_manager.get_ip_info()?.ip);
    ...
    let sensor_thread = thread::spawn(move || {
        ...
        loop {
            ...
            info!("传感器数据更新: 温度={:.1}°C, 湿度={:.1}%, 气压={:.1}hPa", 
    ...
    loop {
        // 更新HTTP服务器数据
        {
            ...
            server_guard.update_data(
                *counter_guard,
                sensor_data_guard.temperature,
                status,
            );
        }
        ...
        if loop_counter % 600 == 0 {
            ...
            info!("网络状态: {}", if wifi_manager.is_connected().unwrap_or(false) { "已连接" } else { "未连接" });
            
            let sensor_data_guard = sensor_data.lock().unwrap();
            info!("当前传感器数据: 温度={:.1}°C, 湿度={:.1}%, 光照={}", 
                  sensor_data_guard.temperature, 
                  sensor_data_guard.humidity, 
                  sensor_data_guard.light_level);
        }
    }
}
