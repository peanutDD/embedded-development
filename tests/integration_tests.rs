...
impl IntegrationTestManager {
    ...
    async fn test_rust_basics(&self) -> TestResult {
        ...
        // 尝试编译基础示例
        match self.compile_rust_project(&format!("{}/01-rust-basics", self.base_path)).await {
            Ok(_) => TestResult {
                name: name.to_string(),
                passed: true,
    ...
    async fn test_hal_layer(&self) -> TestResult {
        ...
        let hal_path = format!("{}/03-embedded-concepts/projects/custom_hal", self.base_path);
    ...
    async fn test_timer_pwm(&self) -> TestResult {
        ...
        let timer_path = format!("{}/06-timers-interrupts/projects/servo_control", self.base_path);
    ...
    async fn test_uart_communication(&self) -> TestResult {
        ...
        let uart_path = format!("{}/05-serial-communication/projects/data_logger", self.base_path);
    ...
    async fn test_i2c_communication(&self) -> TestResult {
        ...
        let i2c_path = format!("{}/08-communication-protocols-i2c-spi/projects/sensor_hub", self.base_path);
    ...
    async fn test_spi_communication(&self) -> TestResult {
        ...
        let spi_path = format!("{}/08-communication-protocols-i2c-spi/projects/display_controller", self.base_path);
    ...
    async fn test_multitasking(&self) -> TestResult {
        ...
        let task_path = format!("{}/11-power-management/projects/task_scheduler", self.base_path);
    ...
    async fn test_interrupt_handling(&self) -> TestResult {
        ...
        let int_path = format!("{}/12-industrial-projects/projects/interrupt_manager", self.base_path);
    ...
    async fn test_power_management(&self) -> TestResult {
        ...
        let power_path = format!("{}/13-security-encryption/projects/low_power_sensor", self.base_path);
    ...
    async fn test_industrial_iot(&self) -> TestResult {
        ...
        let iot_path = format!("{}/16-performance-optimization/projects/industrial_gateway", self.base_path);
    ...
    async fn test_wireless_communication(&self) -> TestResult {
        ...
        let wireless_path = format!("{}/14-wireless-communication/projects/mesh_network", self.base_path);
    ...
    async fn test_machine_learning_ai(&self) -> TestResult {
        ...
        let ai_path = format!("{}/17-machine-learning-ai/projects/smart_camera", self.base_path);
    ...
    async fn test_system_integration(&self) -> TestResult {
        ...
        let integration_path = format!("{}/19-system-integration-deployment/projects/iot_platform", self.base_path);
    ...
    fn generate_report(&self) {
        ...
        for suite in &self.test_suites {
            ...
            for test in &suite.tests {
                ...
                println!("  {} {} ({:.2}s)", status, test.name, test.duration.as_secs_f64());
                if !test.passed {
            ...
            println!("  📈 成功率: {:.1}% ({}/{} 通过)", success_rate, passed, passed + failed);
        ...
        println!("  总成功率: {:.1}%", (total_passed as f64 / total_tests as f64) * 100.0);

        if total_failed == 0 {
            println!("\n🎉 所有测试都通过了！嵌入式开发教程质量良好。");
        } else {
            println!("\n⚠️  有 {} 个测试失败，请检查相关章节的代码。", total_failed);
        }
    }
}
